...
namespace ConsoleInterface
{
	...
	{
		return (TargetState)mOwner->ConsoleSupport->GetConsoleState(mHandle);
	}
	...
	{
		return mOwner->ConsoleSupport->GetIsTargetConnected(mHandle);
	}

	/// <summary>
	/// Gets whether or not the target is the default target.
	/// </summary>
	bool PlatformTarget::IsDefault::get()
	{
		return mHandle.mHandle.ToPointer() == mOwner->ConsoleSupport->GetDefaultTarget();
	}
	...
	{
		String ^TMName = gcnew String(mOwner->ConsoleSupport->GetTargetManagerNameForConsole(mHandle));
	...
	bool PlatformTarget::MakeDirectory(String ^DirectoryName)
	{
		if(DirectoryName == nullptr)
		{
			throw gcnew ArgumentNullException(L"DirectoryName");
		}

		pin_ptr<const wchar_t> Ptr = PtrToStringChars(DirectoryName);

		return mOwner->ConsoleSupport->MakeDirectory(mHandle, Ptr);
	}
	...
	bool PlatformTarget::MakeDirectoryHierarchy(String ^DirectoryHierarchy)
	{
		// find the last slash, if there is one
		int LastSlash = DirectoryHierarchy->LastIndexOf("\\");

		// make sure there was a slash, and it wasn't the first character
		if(LastSlash > 0)
		{
			// if there was a valid slash, make the outer directory
			bool bResult = MakeDirectoryHierarchy(DirectoryHierarchy->Substring(0, LastSlash));
		...
		if(DirectoryHierarchy->LastIndexOf(L"\\") == DirectoryHierarchy->Length - 1)
		{
			DirectoryHierarchy = DirectoryHierarchy->Substring(0, DirectoryHierarchy->Length - 1);
		}

		return MakeDirectory(DirectoryHierarchy);
	}
	...
	bool PlatformTarget::CopyFileToTarget(String ^SourceFilename, String ^DestFileName)
	{
		...
		return mOwner->ConsoleSupport->CopyFile(mHandle, NativeSrcFile, NativeDestFile);
	}
	...
	bool PlatformTarget::RetrieveFileFromTarget(String ^SourceFilename, String ^DestFileName)
	{
		...
		return mOwner->ConsoleSupport->RetrieveFile(mHandle, NativeSrcFile, NativeDestFile);
	}
	...
	{
		return mOwner->ConsoleSupport->Reboot(mHandle);
	}
	...
	bool PlatformTarget::RebootAndRun(String ^Configuration, String ^BaseDirectory, String ^GameName, String ^URL, bool bForceGameName)
	{
		...
		return mOwner->ConsoleSupport->RebootAndRun(mHandle, NativeConfig, NativeBaseDir, NativeGameName, NativeURL, bForceGameName);
	}
	...
	void PlatformTarget::Sync(TOCSettings ^BuildSettings, TOCInfo ^Entry)
	{
		...
		if(!ExistingDirectories->ContainsKey(DestDir))
		{
			if(!MakeDirectoryHierarchy(DestDir))
			{
				BuildSettings->WriteLine(Color::Orange, L"Failed to create directory \'{0}\'", DestDir);
			}
			else
			{
				ExistingDirectories[DestDir] = DestDir;
			}
		}
