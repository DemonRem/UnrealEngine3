...
namespace UnrealDVDLayout
{
    public partial class UnrealDVDLayout : Form
    {
        ...
        public const long XboxSectorsPerLayer = 1783936;
        ...
        public bool Init()
        {
			...
			string SettingsFile = Path.Combine( Application.StartupPath, "UnrealDVDLayout.Options.xml" );
			Options = ReadXml<SettableOptions>( SettingsFile );
        ...
        {
			string SettingsFile = Path.Combine( Application.StartupPath, "UnrealDVDLayout.Options.xml" );
			WriteXml<SettableOptions>( SettingsFile, Options, "" );
        ...
        private void ImportTOCFile( string[] FileNames, bool Report )
        {
			...
			{
			case "xbox360":
				TableOfContents.GetXboxSystemUpdateFile();
			...
			{
			case "xbox360":
				CreateXGDFile( Report );
				break;

			case "ps3":
				CreateGP3File( Report );
				break;

			case "pcconsole":
				CreateInstallDesignerProject( Report );
				break;
			}
        ...
        private void ImportTOCMenu_Click( object sender, EventArgs e )
        {
            ...
            if( GenericOpenFileDialog.ShowDialog() == DialogResult.OK )
            {
                ImportTOCFile( GenericOpenFileDialog.FileNames, true );
            }
        }
        ...
        public void DoPopulateListBoxes()
        {
            ...
            if( TableOfContents.Groups != null )
            {
                ...
                if( UpdateLayers )
                {
					...
					{
					case "xbox360":
						long Layer0Free = XboxSectorsPerLayer;
						long Layer1Free = XboxSectorsPerLayer;
						...
						float Layer0Percent = ( float )( XboxSectorsPerLayer - Layer0Free ) * 100.0f / XboxSectorsPerLayer;
						...
						float Layer1Percent = ( float )( XboxSectorsPerLayer - Layer1Free ) * 100.0f / XboxSectorsPerLayer;
        ...
        public void HandleCommandLine( string[] Arguments )
        {
            ...
            {
				Error( "Not enough parameters; usage 'UnrealDVDLayout <Game> <Platform> <LayoutName> <Language> [Language] [Language] [-image ImageName]'" );
                return;
            }
            ...
            ImportTOCFile( TOCFileNames.ToArray(), false );
			...
			case "xbox360":
				...
				WriteXml<XboxGameDiscLayout>( XGDFileName, XboxDiscLayout, "http://www.w3.org/2001/XMLSchema-instance" );
			...
			case "ps3":
				...
				WriteXml<psproject>( GP3FileName, PS3DiscLayout, "" );
			...
			case "pcconsole":
				...
				// Create a new GP3 with the updated TOC sizes
				CreateInstallDesignerProject( true );

				// Write out the GP3 file
				string IDPFileName = Path.GetFullPath( Path.ChangeExtension( IDPTemplateName, ".InstallDesignerProject" ) );
				WriteXml<InstallDesignerDataSet>( IDPFileName, PCDiscLayout, "urn:schemas-microsoft-com:InstallDesignerProject.v1" );
        ...
        private T ReadXml<T>( string FileName ) where T: new()
        {
            ...
            try
            {
                using( Stream XmlStream = new FileStream( FileName, FileMode.Open, FileAccess.Read, FileShare.None, 1024 * 1024, false ) )
                {
        ...
        private bool WriteXml<T>( string FileName, T Instance, string DefaultNameSpace )
        {
            ...
            try
            {
                using( XmlStream = new FileStream( FileName, FileMode.Create, FileAccess.Write, FileShare.None, 256 * 1024, false ) )
                {
                    ...
                    ObjSer.Serialize( XmlStream, Instance, EmptyNamespace );
                }
            }
            catch( Exception E )
            {
