...
struct FTriangleSOA
{
	...
	/** All bits are 1 for opaque triangles, and 0 otherwise. */
	VectorRegister	OpaqueMask;
 ...
 */
FORCEINLINE INT appLineCheckTriangleSOA(const FVector3SOA& Start, const FVector3SOA& End, const FVector3SOA& Dir, VectorRegister MeshIndex, VectorRegister LODIndex, const FTriangleSOA& Triangle4, UBOOL bOpaqueOnly, UBOOL bTwoSidedCollision, UBOOL bFlipSidedness, FLOAT& InOutIntersectionTime)
{
	...
	if (bOpaqueOnly)
	{
		// Only allow collision with opaque triangles if bOpaqueOnly is TRUE
		TriangleMask = VectorBitwiseAND(TriangleMask, Triangle4.OpaqueMask);
	}
		...
		kDOPArray<NodeType>& Nodes)
	{
		...
		else
		{
			...
			for ( DWORD SOAIndex=0; SOAIndex < t.NumTriangles; ++SOAIndex )
			{
				...
				SOA.OpaqueMask = MakeVectorRegister(
					(DWORD)(Tris[0]->bOpaque ? 0xFFFFFFFF : 0), 
					(DWORD)(Tris[1]->bOpaque ? 0xFFFFFFFF : 0),
					(DWORD)(Tris[2]->bOpaque ? 0xFFFFFFFF : 0),
					(DWORD)(Tris[3]->bOpaque ? 0xFFFFFFFF : 0));
...
#define PLAIN_C 0
	...
	UBOOL LineCheckTriangles(TkDOPLineCollisionCheck<COLL_DATA_PROVIDER,KDOP_IDX_TYPE>& Check, const TTraversalHistory<KDOP_IDX_TYPE>& History) const
	{
		...
		for ( KDOP_IDX_TYPE SOAIndex = t.StartIndex; SOAIndex < (t.StartIndex + t.NumTriangles); SOAIndex++ )
		{
			...
			INT SubIndex = appLineCheckTriangleSOA( Check.StartSOA, Check.EndSOA, Check.DirSOA, Check.MeshIndexRegister, Check.LODIndexRegister, TriangleSOA, Check.bOpaqueOnly, Check.bTwoSidedCollision, Check.bFlipSidedness, Check.Result->Time );
			if ( SubIndex >= 0 )
			{
				bHit = TRUE;
