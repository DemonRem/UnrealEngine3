			...
			if (TexelToVertex.TotalSampleWeight > 0.0f)
			{
					...
					|| !PhotonMappingSettings.bUseFinalGathering && GeneralSettings.NumIndirectLightingBounces >= 0)
				{
					...
					else
					{
						...
						const FLinearColor AccumulatedDirectIllumination = .5f * CalculateIncidentDirectIllumination(TextureMapping, CurrentVertex, 1, MappingContext, RandomStream, bDebugThisTexel);
			...
			for (INT LightIndex = 0; LightIndex < TextureMapping->Mesh->RelevantLights.Num(); LightIndex++)
			{
				...
				else 
				{
					...
					else
					{
						...
						CalculateDirectAreaLightingTextureMapping(TextureMapping, MappingContext, LightMapData, ShadowMaps, TexelToVertexMap, bDebugThisMapping, Light);
					}
				}
			}
		}
				...
				for (INT X = 0;X < TextureMapping->CachedSizeX;X++)
				{
					...
					if ((*FilteredShadowMapData)(X,Y).bIsMapped)
					{
						...
						if (ShadowFactor > 0.0f)
						{
							...
							const FStaticLightingVertex CurrentVertex = TexelToVertex.GetVertex();
							const FLinearColor LightIntensity = Light->GetDirectIntensity(CurrentVertex.WorldPosition);
	...
	for (INT Y = 0; Y < TextureMapping->CachedSizeY; Y++)
	{
		for (INT X = 0; X < TextureMapping->CachedSizeX; X++)
		{
			...
			if ( CurrentLightSample.bIsMapped )
			{
				...
				const FVector4 Normal = MaterialSettings.bUseNormalMapsForSimpleLightMaps ?
					Vertex.TransformTangentVectorToWorld(TextureMapping->Mesh->EvaluateNormal(Vertex.TextureCoordinates[0], TexelToVertex.ElementIndex)) :
				...
				if (bLightIsInFrontOfTriangle || TextureMapping->Mesh->IsTwoSided(TexelToVertex.ElementIndex))
				{
					...
					if (bTraceShadowRays)
					{
						...
						LightIntensity = Light->GetDirectIntensity(CurrentVertex.WorldPosition);
