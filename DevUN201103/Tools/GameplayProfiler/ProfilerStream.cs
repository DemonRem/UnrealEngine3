...
namespace GameplayProfiler
{
	...
	class ProfilerStream
	{
		 ...
		 * @param	ClassNameIndex			The object's outermost name's index into the name table
		 * 
		 * @return	Name in Class Outermost.Outer.Object_ObjectInstance format
		 */
		public string GetObjectName(int ObjectInstanceIndex, int ObjectNameIndex, int ClassNameIndex, int OuterNameIndex, int OutermostNameIndex)
		{
			string FullName = "";
			if(ClassNameIndex >= 0)
			{
