...
namespace CrashReport.Models
{
    ...
    public partial class Bugg
    {
        ...
        public IList<string> GetFunctionCalls(int max)
        {
            ...
            var FunctionCallIds = GetFunctionCallIdsByPattern(Pattern);


            int count = FunctionCallIds.Count();
            int total = count;
            ...
            {
                int id;
                bool converted = int.TryParse(FunctionCallId, out id);
                if (converted)
                {
                    Ids.Add((id));
                }
            ...
            Results = mBuggRepository.GetFunctionCalls(Ids);
