		...
		static void ValidatingProc()
		{
			...
			while( true )
			{
				...
				try
				{
					...
					// Search for builds that have been hidden for more than 5 days
					using( DataSet OldBuilds = DataHelper.GetDataSet( "SELECT TOP( 1 ) * FROM [PlatformBuilds] " +
																		"WHERE ( StatusID = " + StatusHidden.ToString() + " ) " +
																		"AND ( DATEDIFF( day, DiscoveryTime, GETDATE() ) > 5 ) " +
																		"AND ( Special = 0 )" ) )
					{
						if( OldBuilds.Tables[0].Rows.Count > 0 )
						{
							DataRow Row = OldBuilds.Tables[0].Rows[0];
							long PlatformBuildID = Convert.ToInt64( Row["ID"] );
							DataHelper.PlatformBuild_Delete( PlatformBuildID );
						}
					}
