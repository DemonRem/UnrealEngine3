...
namespace UnrealProp
{
    ...
    public static class TaskScheduler
    {
        ...
        static public void Init()
        {
            ...
            Log.WriteLine( "UPMS TASK SCHEDULER", Log.LogType.Important, "Initialized!" );
        }
        ...
        static private int GetActiveTasks( string UPDSName )
        {
            ...
            Log.WriteLine( "UPMS TASK SCHEDULER", Log.LogType.Debug, UPDSName + " has " + ActiveTasks.ToString() + " active tasks" );
        ...
        static void SchedulerProc()
        {
            ...
            while( true )
            {
                ...
                try
                {
                    ...
                    {
                        if( DataHelper.PlatformBuild_GetStatus( Task.PlatformBuildID ) != BuildStatus.Ready )
                        {
                        ...
                        if( DateTime.Now > Task.ScheduleTime )
                        {
                            ...
                            if( UPMS_Service.UPDSList.Count > 0 )
                            {
                                ...
                                lock( SyncWorkingTasks )
                                {
                                    ...
                                    if( InUse )
                                    {
                                        Log.WriteLine( "UPMS TASK SCHEDULER", Log.LogType.Important, "Target machine for task: " + Task.ID.ToString() + " is currently in use by another task!" );
                                        continue;
                                    }
                                }
                                ...
                                else
                                {
                                    ...
                                    Log.WriteLine( "UPMS TASK SCHEDULER", Log.LogType.Important, BestUPDSName + " is too busy to execute task ID: " + Task.ID.ToString() + "!" );
                                }
                            }
                            else
                            {
                                Log.WriteLine( "UPMS TASK SCHEDULER", Log.LogType.Important, "Cannot find any UPDS to execute task ID: " + Task.ID.ToString() + "!" );
                            }
                        }
                    }
