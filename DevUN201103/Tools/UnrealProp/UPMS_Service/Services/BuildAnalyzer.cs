...
namespace UnrealProp
{
    ...
    public static class BuildAnalyzer
    {
        ...
        static public void Init()
        {
            ...
            Thread.Start();
            Log.WriteLine( "UPMS BUILD ANALYZER", Log.LogType.Important, "Initialised!" );
        }
        ...
        static string GetSHA1Hash( string PathName )
        {
            ...
            {
                Log.WriteLine( "UPMS BUILD ANALYZER", Log.LogType.Error, Ex.ToString() );
            }
#endif
        ...
        static void AnalyzeBuild( DataRow Build )
        {
            ...
            {
#endif
                Log.WriteLine( "UPMS BUILD ANALYZER", Log.LogType.Important, "started analyzing new build :" + Build["Path"].ToString().Trim() );
				...
				if( !DirInfo.Exists )
				{
					...
					Log.WriteLine( "UPMS BUILD ANALYZER", Log.LogType.Important, "build does not exist; aborting" );
					return;
				}
                ...
                Log.WriteLine( "UPMS BUILD ANALYZER", Log.LogType.Important, "finished analyzing new build :" + Build["Path"].ToString().Trim() );
#if !DEBUG
            }
            catch( Exception Ex )
            {
                Log.WriteLine( "UPMS BUILD ANALYZER", Log.LogType.Error, "Unhandled exception: " + Ex.ToString() );
            }
#endif
        }
