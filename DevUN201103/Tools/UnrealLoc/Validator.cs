        ...
        private ValidationError MatchPairs( ref string Value, ValidationScheme Scheme )
        {
            ...
            foreach( char C in Value )
            {
				...
				case ValidationScheme.WesternEuro:
					if( !CheckAlphaNumeric( C ) && !CheckAdditional( C, Main.Options.AllowedWesternEuroCharacters ) )
					{
						BadChar = C;
            ...
            if( ReturnCode != ValidationError.NoError )
            {
                ...
                switch( ReturnCode )
                {
                    case ValidationError.IllegalCharacter:
						ReturnString += " '" + BadChar + "' (0x" + ( ( int )BadChar ).ToString( "X" ) + ") at offset " + Offset.ToString() + " in '" + Value + "' in '" + LE.Owner.Owner.Owner.Owner.FileName + "' at line " + LE.LocEntryLineNumber.ToString();
                        break;
                    ...
                    case ValidationError.LocalisedTags:
						ReturnString += " in '" + Value + "' in '" + LE.Owner.Owner.Owner.Owner.FileName + "' at line " + LE.LocEntryLineNumber.ToString();
                        break;
                }
