 ...
 * @param IndentCount the number of indents to use
 */
inline void ToXml(FString& OutXmlString,const TArray<FOnlineProfileSetting>& Settings,DWORD IndentCount = 0)
{
	FString Indent = BuildIndentString(IndentCount);
	OutXmlString += Indent;
	...
	{
		ToXml(OutXmlString,Settings(SettingsIndex).ProfileSetting,Indent);
	}
	OutXmlString += Indent;
	OutXmlString += TEXT("</ProfileSettings>\r\n");
}
