...
{
	if (GSocketSubsystem == NULL || !GIpDrvInitialized)
	{
		return FALSE;
	}
	...
	ListenSocket = GSocketSubsystem->CreateStreamSocket(TEXT("FDebugServer tcp-listen"));
	if(!ListenSocket)
	{
	...
	PingSocket = GSocketSubsystem->CreateDGramSocket(TEXT("FDebugServer ping-pong"), TRUE);
	check(PingSocket);
	...
	if (!PingSocket->Bind(PingAddr))
	{
		...
		PingSocket->Close();
		GSocketSubsystem->DestroySocket(PingSocket);
		PingSocket = NULL;
	}
