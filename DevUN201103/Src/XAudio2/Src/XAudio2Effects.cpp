...
UBOOL FXAudio2EffectsManager::CreateEQPremasterVoices( UXAudio2Device* XAudio2Device )
{
	...
	const FLOAT* OutputMatrix = UXAudio2Device::GetOutputMatrix( XAudio2Device->DeviceDetails.OutputFormat.dwChannelMask, NumChannels );

	if( !AudioDevice->ValidateAPICall( TEXT( "SetOutputMatrix (EQPremaster)" ), 
		EQPremasterVoice->SetOutputMatrix( NULL, SPEAKER_COUNT, NumChannels, OutputMatrix ) ) )
	{
		return( FALSE );
	}

	if( !AudioDevice->ValidateAPICall( TEXT( "SetOutputMatrix (DryPremaster)" ), 
		DryPremasterVoice->SetOutputMatrix( NULL, SPEAKER_COUNT, NumChannels, OutputMatrix ) ) )
	{
		return( FALSE );
	}
...
UBOOL FXAudio2EffectsManager::CreateRadioVoice( class UXAudio2Device* XAudio2Device )
{
	...
	if( !AudioDevice->ValidateAPICall( TEXT( "SetOutputMatrix (Radio)" ), 
		RadioEffectVoice->SetOutputMatrix( NULL, SPEAKER_COUNT, NumChannels, OutputMatrix ) ) )
	{
		return FALSE;
	}
