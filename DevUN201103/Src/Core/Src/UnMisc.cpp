...
void FMapPackageFileCache::CachePath( const TCHAR* InPath)
{
	...
	if (GUseCoderMode)
	{
		FString CoderPath = InPath;
		if (CoderPath.InStr(TEXT("\\Cooked"), FALSE, TRUE) != INDEX_NONE)
		{
			CoderPath += TEXT("_Coder");
		}
		appFindFilesInDirectory(Packages, *CoderPath, TRUE, FALSE);
	}
	else
#endif
	{
		FString CookedPath(InPath);

		if (appGetPlatformType() & UE3::PLATFORM_WindowsConsole)
		{
			CookedPath = CookedPath.Replace(TEXT("\\CookedPC"),TEXT("\\CookedPCConsole"),TRUE);
		}
		else if (appGetPlatformType() & UE3::PLATFORM_WindowsServer)
		{
			CookedPath = CookedPath.Replace(TEXT("\\CookedPC"),TEXT("\\CookedPCServer"),TRUE);
		}
		
		appFindFilesInDirectory(Packages, *CookedPath, TRUE, FALSE);
	}
...
UBOOL appGetCookedContentPath(UE3::EPlatformType InPlatform, FString& OutPath)
{
	...
	FString AppendPath(TEXT(""));

	if (GUseCoderMode)
	{
		AppendPath = TEXT("_Coder");
	}

	FString PlatformString = appPlatformTypeToString(InPlatform);
	if (PlatformString != TEXT(""))
	{
		OutPath = FString::Printf(TEXT("%s%s"), *(appGameDir() * TEXT("Cooked") + PlatformString), *AppendPath);
		return TRUE;
	}
	return FALSE;
}
