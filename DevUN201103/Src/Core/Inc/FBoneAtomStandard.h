...
FORCEINLINE FBoneAtom FBoneAtom::Inverse() const
{
	...
	FBoneAtom Result(Rotation.Inverse(), FVector(0.f), 1/Scale);

	FQuat QuatTrans = Result.Rotation*FQuat(Result.Scale*Translation.X, Result.Scale*Translation.Y, Result.Scale*Translation.Z, 0.f)*Result.Rotation.Inverse();
