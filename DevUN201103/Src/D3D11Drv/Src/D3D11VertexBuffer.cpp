...
FVertexBufferRHIRef FD3D11DynamicRHI::CreateVertexBuffer(UINT Size,FResourceArrayInterface* ResourceArray,DWORD InUsage)
{
	...
	Desc.ByteWidth = Size;
	Desc.Usage = (InUsage & RUF_AnyDynamic) ? D3D11_USAGE_DYNAMIC : D3D11_USAGE_DEFAULT;
	Desc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	Desc.CPUAccessFlags = (InUsage & RUF_AnyDynamic) ? D3D11_CPU_ACCESS_WRITE : 0;
	Desc.MiscFlags = 0;
	...
	TRefCountPtr<FD3D11VertexBuffer> VertexBuffer;
	VERIFYD3D11RESULT(Direct3DDevice->CreateBuffer(&Desc,pInitData,(ID3D11Buffer**)VertexBuffer.GetInitReference()));
...
void* FD3D11DynamicRHI::LockVertexBuffer(FVertexBufferRHIParamRef VertexBufferRHI,UINT Offset,UINT Size,UBOOL bReadOnlyInsteadOfWriteOnly)
{
	...
	FD3D11LockedKey LockedKey(VertexBuffer);
	...
	if(bIsDynamic)
	{
		...
		VERIFYD3D11RESULT(Direct3DDeviceIMContext->Map(VertexBuffer,0,D3D11_MAP_WRITE_DISCARD,0,&MappedSubresource));
	...
	else
	{
		if(bReadOnlyInsteadOfWriteOnly)
		{
			...
			Direct3DDeviceIMContext->CopyResource(StagingVertexBuffer,VertexBuffer);
...
void FD3D11DynamicRHI::UnlockVertexBuffer(FVertexBufferRHIParamRef VertexBufferRHI)
{
	...
	if(bIsDynamic)
	{
		...
		Direct3DDeviceIMContext->Unmap(VertexBuffer,0);
	}
	else
	{
		...
		else 
		{
			...
			Direct3DDeviceIMContext->UpdateSubresource(VertexBuffer,LockedKey.Subresource,NULL,LockedData->Data,LockedData->Pitch,0);
