...
UBOOL AGameInfo::GetSupportedGameTypes(const FString& InFilename,struct FGameTypePrefix& OutGameType,UBOOL bCheckExt) const
{
	...
	if( DefaultMapPrefixes.Num() > 0 || CustomMapPrefixes.Num() > 0 )
	{
		...
		if (NewMapName.StartsWith(PLAYWORLD_PACKAGE_PREFIX) ||
			NewMapName.StartsWith(FString(PLAYWORLD_CONSOLE_BASE_PACKAGE_PREFIX) + TEXT("IOS")) ||
			NewMapName.StartsWith(FString(PLAYWORLD_CONSOLE_BASE_PACKAGE_PREFIX) + TEXT("PS3")) ||
			NewMapName.StartsWith(FString(PLAYWORLD_CONSOLE_BASE_PACKAGE_PREFIX) + TEXT("360")))
		{
			NewMapName = NewMapName.Right(MapNameString.Len() - 6);
		}
		else if (NewMapName.StartsWith(FString(PLAYWORLD_CONSOLE_BASE_PACKAGE_PREFIX) + TEXT("PC")))
		{
			NewMapName = NewMapName.Right(MapNameString.Len() - 5);
		}
