...
#if defined(_MSC_VER)
#define USE_TOOTLE (!CONSOLE && !DEDICATED_SERVER && !UE3_LEAN_AND_MEAN)
#else
#define USE_TOOTLE 0
#endif
...
void SortTriangles_Tootle_FIRST( INT NumTriangles, INT NumVertices, const FSoftSkinVertex* Vertices, DWORD* Indices )
{
	// Tootle wants 32-bit indices
	TArray<UINT> NewIndices(NumTriangles*3);
	for( INT i=0;i<NumTriangles*3;i++ )
		NewIndices(i) = Indices[i];

	TootleInit();

	TootleOptimize(  &Vertices->Position, &NewIndices(0), NumVertices, NumTriangles, sizeof(FSoftSkinVertex), 24,
		NULL, 0, // use the default viewpoints
		TOOTLE_CCW, // the mesh has counter-clockwise front face winding
		&NewIndices(0), NULL,
	...
	// Copy indices back in reverse order
	for( INT i=0;i<NumTriangles;i++ )
	{
		Indices[i*3+0] = (DWORD)NewIndices((NumTriangles-i-1)*3+0);
		Indices[i*3+1] = (DWORD)NewIndices((NumTriangles-i-1)*3+1);
		Indices[i*3+2] = (DWORD)NewIndices((NumTriangles-i-1)*3+2);
	}
}

/**
* Sort triangles using Tootle.
*
* @param SkelMesh - Skeletal mesh
* @param NumTriangles - Number of triangles
* @param Vertices - pointer to the skeletal mesh's FSoftSkinVertex array.
* @param Indices - pointer to the index buffer data to be reordered
*/
void SortTriangles_Tootle( INT NumTriangles, INT NumVertices, const FSoftSkinVertex* Vertices, DWORD* Indices )
{
	// Get connected triangle sets
	TArray<UINT> TriSet;
	INT MaxTriSet = GetConnectedTriangleSets( NumTriangles, Indices, TriSet );
	TArray<TArray<INT> > Strips;
	Strips.AddZeroed(MaxTriSet);
	for( INT TriIndex=0;TriIndex<TriSet.Num();TriIndex++ )
	{
		Strips(TriSet(TriIndex)).AddItem(TriIndex);
	}

	// Tootle wants 32-bit indices. build with triangles from the same set adjacent
	TArray<UINT> NewIndices;
	NewIndices.Empty(NumTriangles*3);
	for( INT s=0;s<Strips.Num();s++ )
	{
		for( int TriIndex=0;TriIndex<Strips(s).Num();TriIndex++ )
		{
			INT tri = Strips(s)(TriIndex);
			DWORD i1 = Indices[tri*3+0];
			DWORD i2 = Indices[tri*3+1];
			DWORD i3 = Indices[tri*3+2];
			NewIndices.AddItem(i1);
			NewIndices.AddItem(i2);
			NewIndices.AddItem(i3);
		}
	}

	TootleInit();

	FLOAT Viewpoints[] = { 0, 0.707f, 0.707f };

	TootleOptimizeOverdraw(&Vertices->Position, &NewIndices(0), NumVertices, NumTriangles, sizeof(FSoftSkinVertex),
		//NULL, 0,
		Viewpoints, 1,
		TOOTLE_CW,
		&TriSet(0),
	...
	{
		Indices[t*3+0] = (DWORD)NewIndices((NumTriangles-t-1)*3+0);
		Indices[t*3+1] = (DWORD)NewIndices((NumTriangles-t-1)*3+1);
		Indices[t*3+2] = (DWORD)NewIndices((NumTriangles-t-1)*3+2);
	}
}
