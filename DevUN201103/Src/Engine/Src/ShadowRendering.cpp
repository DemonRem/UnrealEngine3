...
class FShadowDepthShaderParamters
{
public:
	...
	/** Serializer. */
	friend FArchive& operator<<(FArchive& Ar,FShadowDepthShaderParamters& P)
	{
...
private:
	...
	FShadowDepthShaderParamters ShadowParameters;
};
...
void DrawStencilingSphere(const FSphere& Sphere, const FVector& PreViewTranslation)
{
	...
	TArray<WORD, TInlineAllocator<NumSides * NumRings * 6> > Indices;
...
void FProjectedShadowInfo::RenderProjection(const FViewInfo* View, BYTE DepthPriorityGroup, UBOOL bRenderingBeforeLight) const
{
#if WANTS_DRAW_MESH_EVENTS
	FString EventName;
	if (GEmitDrawEvents)
	...
	else
	{
			...
			&& (!ShouldUseDeferredShading() || LightSceneInfo->LightingChannels.GetDeferredShadingChannelMask() == 0))
		{
			...
			//@todo - handle the masked subject primitives on 360
			...
			// Making other SM3 platforms do DDM_NonMaskedOnly as well to be consistent
			const EDepthDrawingMode DrawingMode = DDM_NonMaskedOnly;
		...
		else
		{
			...
			if (bForegroundCastingOnWorld)
			{
				...
				// Draw the subject's dynamic opaque elements
				const EDepthDrawingMode DrawingMode = GRHIShaderPlatform == SP_PCD3D_SM5 ? DDM_AllOccluders : DDM_NonMaskedOnly;
	...
	else
	{
		...
		else
		{
			...
			if (GRHIShaderPlatform == SP_PCD3D_SM5 && GSystemSettings.UsesMSAA())
			{
				...
				PerFragmentPixelShader->SetParameters(*View,this);
	...
	if (bCameraInsideShadowFrustum)
	{
		// If the camera is inside the shadow frustum (or the near plane is clipping), use a full screen stencil clear as that will be faster
...
void FProjectedShadowInfo::RenderOnePassPointLightProjection(const FViewInfo& View, BYTE DepthPriorityGroup) const
{
...
void FProjectedShadowInfo::RenderPlanarProjection(const class FSceneRenderer* SceneRenderer, BYTE DPGIndex) const
{
	...
	if( GUsingMobileRHI )
	{
		...
		const FLinearColor ModShadowColor = Lerp(FLinearColor::White, LightSceneInfo->ModShadowColor, FadeAlpha);
	...
	if (bDominantShadowsActive)
	{
		...
		for (INT TextureIndex = ARRAY_COUNT(LightsPerAttenuationTexture) - 1; TextureIndex >= 0; TextureIndex--)
		{
			if (LightsPerAttenuationTexture[TextureIndex].Num() > 0)
			{
				...
				if (bHasInheritDominantShadowMaterials && TextureIndex == DominantDirectionalLightTexture)
				{
	...
	for (INT ShadowIndex = 0; ShadowIndex < Shadows.Num(); ShadowIndex++)
	{
		...
		}
		{
			...
			for (INT ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
			{
				...
				CurrentShadow->RenderOnePassPointLightProjection(View, DPGIndex);
			}
		}
		bDirty = TRUE;
	}
	...
	for (INT ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		for (INT ShadowIndex = 0; ShadowIndex < Shadows.Num(); ShadowIndex++)
		{
			...
			if (ProjectedShadowInfo->bAllocated)
			{
				...
				if ((ProjectedShadowInfo->ViewMaskOut & (1 << ViewIndex)) == 0)
				{
					ProjectedShadowInfo->RenderProjection(&View, DPGIndex, bRenderingBeforeLight);
				}
			}
		}
	...
	if (!GUsingMobileRHI)
	{
		...
		while (NumShadowsRendered < Shadows.Num())
		{
			...
			if (PassNumber > 0 || !(GOnePassDominantLight && Shadows(0)->IsPrimaryWholeSceneDominantShadow()))
			{
				...
				GSceneRenderTargets.FinishRenderingShadowDepth(bRenderingWholeSceneDominantShadow, FResolveParams(ResolveRect));
			}
