...
void FDynamicSortableSpriteEmitterDataBase::Render(FParticleSystemSceneProxy* Proxy, FPrimitiveDrawInterface* PDI,const FSceneView* View,UINT DPGIndex)
{
...
void FDynamicMeshEmitterData::Render(FParticleSystemSceneProxy* Proxy, FPrimitiveDrawInterface* PDI,const FSceneView* View,UINT DPGIndex)
{
	...
	if (Source.EmitterRenderMode == ERM_Normal)
	{
		...
		{
			// replace the motion blur transform data with the
			// new transforms from the current frame
			...
			for (INT LODIndex = 0; LODIndex < 1; LODIndex++)
			{
				for (INT ElementIndex = 0; ElementIndex < LODModel.Elements.Num(); ElementIndex++)
				{
					...
					if (ElementIndex < LODInfo->Elements.Num())
					{
						...
						{
							NewMIRes[0]->Parent = LODInfo->Elements(ElementIndex).MaterialInterface->GetRenderProxy(FALSE);
							if (GIsGame == FALSE)
							{
								NewMIRes[1]->Parent = LODInfo->Elements(ElementIndex).MaterialInterface->GetRenderProxy(TRUE);
							}
							else
							{
...
void FDynamicTrailsEmitterData::Render(FParticleSystemSceneProxy* Proxy, FPrimitiveDrawInterface* PDI, const FSceneView* View, UINT DPGIndex)
{
	SCOPE_CYCLE_COUNTER(STAT_TrailRenderingTime);
	INC_DWORD_STAT(STAT_TrailParticlesRenderCalls);
...
void UParticleSystemComponent::SetLightEnvironment(ULightEnvironmentComponent* NewLightEnvironment)
{
	...
	check(!NewLightEnvironment || NewLightEnvironment->IsA(UParticleLightEnvironmentComponent::StaticClass()));
