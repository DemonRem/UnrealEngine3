...
UBOOL UGameEngine::Exec( const TCHAR* Cmd, FOutputDevice& Ar )
{
	...
	else if( ParseCommand( &Cmd, TEXT("MemLeakCheckPostGC") ) )
	{
		...
		const UBOOL OldGPrintLogTimes = GPrintLogTimes;
...
//		GParticleDataManager.ResetParticleMemoryMaxValues();
		...
		GPrintLogTimes = OldGPrintLogTimes;
	...
	else if( ParseCommand(&Str,TEXT("MEM")) )
	{
		...
		if( !bSummaryOnly || bDetailed )
		{
			...
			ENQUEUE_UNIQUE_RENDER_COMMAND_TWOPARAMETER(
				RetrieveRenderThreadMemstackUsage,
				INT*, UsedMemory, &UsedMemory,
				INT*, UnusedMemory, &UnusedMemory,
			{
				*UsedMemory = GRenderingThreadMemStack.GetByteCount();
				*UnusedMemory = GRenderingThreadMemStack.GetUnusedByteCount();
			});
			FlushRenderingCommands();
			Ar.Logf( TEXT("GRenderingThreadMemStack allocation size [used/ unused] = [%i / %i] KByte"), UsedMemory / 1024, UnusedMemory / 1024 );
...
UBOOL UGameEngine::LoadMap( const FURL& URL, UPendingLevel* Pending, FString& Error )
{
	...
	// Stream in all textures. Block for 5 seconds or until they're all streamed in.
	if (GStreamingManager != NULL)
	{
		FLOAT TimeLimit = 5.0f;
		GConfig->GetFloat(TEXT("TextureStreaming"), TEXT("LoadMapTimeLimit"), TimeLimit, GEngineIni);
