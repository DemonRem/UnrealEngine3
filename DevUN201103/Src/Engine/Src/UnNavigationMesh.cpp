...
FLOAT ExpansionEdgeSmoothingConvexTolerance = 20.f;
...
// do 3->2 merging?
UBOOL ExpansionDoThreeToTwoMerge = TRUE;
							  ...
							  FNavMeshPolyBase* ParentPoly)
{
	...
	if(STEPHEIGHT_TEST_STEP_SIZE/StepSize<0.5f)
	{
		...
		FVector ParentPerpOffset = GetExpansionStepSize(Perp*(ParentStepSize-STEPHEIGHT_TEST_STEP_SIZE));	
...
void APylon::CheckForErrors()
{
	...
	{
		GWarn->MapCheck_Add( MCTYPE_ERROR, this, TEXT("Pylon is outside its expansion bounds!"), MCACTION_NONE, TEXT("PylonOutsideBounds"));
	}
...
UBOOL FindCheckStartPos(const FVector& Start,const FVector& End,const FVector& Extent,const TArray<FNavMeshPolyBase*>& ObstacleMeshPolys, TArray<APylon*>* PylonsToCheckPtr, FVector& out_StartCheckPos)
{
	const FVector MidPt = (Start + End)*0.5f;
	if( !DoesBoxIntersectPolys(MidPt,Extent,ObstacleMeshPolys) )
	{
		out_StartCheckPos = MidPt+FVector(0.f,0.f,-Extent.Z);
		return TRUE;
	}

	// divide and try again!
	if( (Start-MidPt).SizeSquared() > MIN_CHECK_START_SIZE_SQ && FindCheckStartPos(Start,MidPt,Extent,ObstacleMeshPolys, PylonsToCheckPtr, out_StartCheckPos) )
	{
		return TRUE;
	}

	if( (MidPt-End).SizeSquared() > MIN_CHECK_START_SIZE_SQ && FindCheckStartPos(MidPt,End,Extent,ObstacleMeshPolys, PylonsToCheckPtr, out_StartCheckPos) )
	{
		return TRUE;
	}
...
UBOOL LineCheckAgainstSpecificPolys(const FVector& Start, const FVector& End, const FVector& Extent, TArray<FNavMeshPolyBase*>& Polys, FVector& out_HitLoc)
{
 ...
 */
UBOOL FindEndPointsForSize(const FVector& ValidStartPos, FVector& out_SpanStart, FVector& out_SpanEnd, const FVector& Extent, TArray<FNavMeshPolyBase*>& ObstacleMeshPolys)
{
	...
	if( LineCheckAgainstSpecificPolys(StartPosWIthZOffset,StartWithZOffset,Extent,ObstacleMeshPolys,out_SpanStart ))
	{
		out_SpanStart -= HeightOffset;
	}

	if ( LineCheckAgainstSpecificPolys(StartPosWIthZOffset,EndWithZOffset,Extent,ObstacleMeshPolys,out_SpanEnd ) )
	{
...
void UNavigationMeshBase::BuildEdgesFromSegmentSpan(const FVector& Start, const FVector& End, TArray<FEdgeWidthPair>& out_EdgePairs, TArray<APylon*>* PylonsToCheck/*=NULL*/, BYTE EdgeGroupID/*=0*/, TArray<FNavMeshPolyBase*>* ObstacleMeshPolysPtr/*=NULL*/)
{
	...
	// slide along the length of the segment and try to find the largest size supported 

	const FVector Seg_Delta = End-Start;
	const FLOAT Seg_Dist = Seg_Delta.Size();
	const FVector Seg_Dir = Seg_Delta/Seg_Dist;

	static TArray<APylon*> PylonsToCheck_Real;

	// legwork to get a list of pylons who we need to query for obstacle mesh geo (avoid multiple octree hits)
	TArray<APylon*>* PylonsToCheckPtr;
	if( PylonsToCheck != NULL )
	{
		PylonsToCheckPtr = PylonsToCheck;
	}
	else
	{
		PylonsToCheck_Real.Reset(PylonsToCheck_Real.Num());
		FBox NewBox(1);
		NewBox += Start;
		NewBox += End;
		UNavigationHandle::GetAllPylonsFromPos(NewBox.GetCenter(),NewBox.GetExtent(),PylonsToCheck_Real);

		PylonsToCheckPtr = &PylonsToCheck_Real;
	}
	...
	for( INT i = Scout->PathSizes.Num()-1; i >= 0; --i)
	{
		...
		if( ObstacleMeshPolysPtr == NULL && i == Scout->PathSizes.Num()-1)
		{
			ObstacleMeshPolysPtr = &ObstacleMeshPolys;

			ObstacleMeshPolys.Reset(ObstacleMeshPolys.Num());

			FBox TraceBox = FBox::BuildAABB(Start,Extent);
			TraceBox += FBox::BuildAABB(End,Extent);

			UNavigationHandle::GetAllObstaclePolysFromPos(TraceBox.GetCenter(), TraceBox.GetExtent(),*ObstacleMeshPolysPtr,PylonsToCheckPtr);
		}
		...
		const FVector StartWithZOffset = Start + HeightOffset;
		const FVector EndWithZOffset = End + HeightOffset;

		// start out with the ends of the segmetn we're building spans for
		FVector NewStart = Start;
		FVector NewEnd = End;

		// quickly find a valid spot form which to start our linechecks
		FVector StartCheckPos(0.f);
		if( FindCheckStartPos(StartWithZOffset,EndWithZOffset,Extent,ObstacleMeshPolys, PylonsToCheckPtr, StartCheckPos) )
		{
			if( FindEndPointsForSize(StartCheckPos,NewStart,NewEnd,Extent,*ObstacleMeshPolysPtr) )
			{
				// then we had a succesful run, and now we need to actually add the edge
				out_EdgePairs.AddItem(FEdgeWidthPair(NewStart,NewEnd,Extent.X,EdgeGroupID));
				...
				if ( !bAtStart || !bAtEnd )
				{
					for( INT j = i-1; j >= 0; --j) // start with the size after theone we just added
					{
						const FVector InnerExtent = FVector(Scout->PathSizes(j).Radius,Scout->PathSizes(j).Radius,Scout->PathSizes(j).Height);					

						HeightOffset.Z = InnerExtent.Z;		
						NewStart = Start;
						NewEnd = End;
						
						if( FindEndPointsForSize(StartCheckPos,NewStart,NewEnd,InnerExtent,*ObstacleMeshPolysPtr) )
						{
							out_EdgePairs.AddItem(FEdgeWidthPair(NewStart,NewEnd,InnerExtent.X,EdgeGroupID));
						}
					}

					// clip off remainder and call on each
					if( !bAtStart )
					{
						// since we've already gone over start -> newstart with current size
						BuildEdgesFromSegmentSpan(Start,NewStart,out_EdgePairs,PylonsToCheckPtr,++EdgeGroupID,ObstacleMeshPolysPtr);
					}

					if( !bAtEnd )
					{
						BuildEdgesFromSegmentSpan(NewEnd,End,out_EdgePairs,PylonsToCheckPtr,++EdgeGroupID,ObstacleMeshPolysPtr);
					}
				}
...
/**
 * returns TRUE if the poly matches the passed PolyRef,
 * minimizes poly lookups for speed
 * @param Poly - poly to match
 * @param Ref - ref we're matching against
 * @return TRUE if poly matches the poly ref
 */
UBOOL DoesRefMatchPoly(FNavMeshPolyBase* Poly, FPolyReference& Ref)
{
	if( Ref.OwningPylon.Actor == Poly->NavMesh->GetPylon() )
	{
		WORD SubPolyID = Ref.GetSubPolyId();
		WORD TLPolyID = Ref.GetTopLevelPolyId();
	...
	for(INT EdgeIdx=0;EdgeIdx<NumEdges;EdgeIdx++)
	{
		...
		if( Edge != NULL )
		{
			if( Edge->IsCrossPylon() )
			{
				...
				if( ( DoesRefMatchPoly(Poly1,CPEdge->Poly0Ref) && DoesRefMatchPoly(Poly2,CPEdge->Poly1Ref) ) ||
					( DoesRefMatchPoly(Poly2,CPEdge->Poly0Ref) && DoesRefMatchPoly(Poly1,CPEdge->Poly1Ref) ) )
				{
					return TRUE;
				}
			}
			...
			for(INT EdgeIdx=0;EdgeIdx<CurPoly->GetNumEdges();++EdgeIdx)
			{
				...
				if( IsPointCloseToOnSegment(EdgeCtr,SegmentEndPoint0,SegmentEndPoint1, FALSE ) )
				{
					...
					for(INT OuterEdgeIdx=0;OuterEdgeIdx<OuterEdges.Num();++OuterEdgeIdx)
					{
						...
						if( CurOuterEdge.BuildTempEdgePolys(0) == OtherPoly )
						{
					...
					if( !bAlreadyHaveDummyLinkToPoly )
					{
						...
						CurEdge->GetAllEdgesInGroup(CurPoly,EdgesInGroup);
						FLOAT BestLen = 0.f;
						INT BestIdx=-1;
	...
	for(INT Idx=0;Idx<NormalMesh->Polys.Num();Idx++)
	{
		...
		if( bSecondPass )
		{
			...
			for(INT EdgeIdx=0;EdgeIdx<CurPoly.GetNumEdges();EdgeIdx++)
			{
				...
				if(CurPolyEdge != NULL && !CurPolyEdge->ShouldBeConsideredForObstacleGeo())
				{
		...
		{			
			...
			//    the segments of the poly that are associated with the edges linked to it
			{
				...
				for(INT EdgePolyIdx=0;EdgePolyIdx<CurPoly.PolyEdges.Num();++EdgePolyIdx)
				{
					...
					CurEdge->GetAllEdgesInGroup(Poly0, EdgesInGroup);
...
{
	APylon* Pylon = (APylon*)(*OwningPylon);
	...
	if(Pylon != NULL && Pylon->NavMeshPtr != NULL && (bEvenIfPylonDisabled || !Pylon->bDisabled))
	{
		...
		if(TLPoly->NumObstaclesAffectingThisPoly > 0 && SubPolyId != MAXWORD)
		{
			...
			return SubMesh->GetPolyFromId(SubPolyId);
		}
...
FNavMeshPolyBase* FNavMeshCrossPylonEdge::GetPoly0()
{
	if( bPendingDelete )
	{
		return NULL;
...
FNavMeshPolyBase* FNavMeshCrossPylonEdge::GetPoly1()
{
	if( bPendingDelete )
	{
		return NULL;
											 ...
											 UBOOL bFirstPass )
{
	...
	{
		const FLOAT PathLaneRadius = Min<FLOAT>((EntityRadius * 2.f) * PathParams.SearchLaneMultiplier, EffectiveEdgeLength);
...
// 		GWorld->GetWorldInfo()->DrawDebugBox( EntityPosition, FVector(3), 0, 0, 255, TRUE );
	...
	{
		// DEBUG DEBUG
		//GWorld->GetWorldInfo()->DrawDebugLine( InEntityPosition, ClosestEdgePoint,0,255,0,TRUE);
	...
	else if( DistFromEdgeRey < PathParams.SearchExtent.X && DistToEdge > PathParams.SearchExtent.X )
	{
		...
		// DEBUG DEBUG
		//GWorld->GetWorldInfo()->DrawDebugLine( InEntityPosition, DestinationPosition,0,0,255,TRUE);
	}
...
FVector FNavMeshEdgeBase::GetEdgeNormal(UBOOL bWorldSpace/* =WORLD_SPACE */)
{
	...
	if(GetPoly0() != NULL && GetPoly1() != NULL)
	{
		Up = GetPoly0()->GetPolyNormal(bWorldSpace) + GetPoly1()->GetPolyNormal(bWorldSpace);
		Up *= 0.5f;
	}
...
UBOOL FNavMeshDropDownEdge::PrepareMoveThru( AController* C, FVector& out_MovePt )
{
	if( C != NULL && C->Pawn != NULL )
	{
		...
		FLOAT ArrivalOffset = 2.0f*C->Pawn->GetCylinderExtent().X;
		...
		out_MovePt = Closest;
		out_MovePt.Z += C->Pawn->GetCylinderExtent().Z;
...
void UNavigationMeshBase::UnMarkEdgeAsActive(FNavMeshEdgeBase* Edge, UNavigationHandle* Handle)
{
	...
	if( ActiveInterface != NULL )
	{
		...
		for(INT EdgeIdx=0;EdgeIdx<EdgesInGroup.Num();++EdgeIdx)
		{
			Edge->ExtraEdgeCost = Max<INT>( Edge->ExtraEdgeCost - ActiveInterface->ExtraEdgeCostToAddWhenActive(), 0 );
		}		
...
//		debugf(TEXT("%s UnMarkActiveEdge %d/%d"), *Handle->GetOuter()->GetName(), ActiveInterface->ExtraEdgeCostToAddWhenActive(), Edge->ExtraEdgeCost );
	...
	if(Edge->IsCrossPylon() || IsSubMesh())
	{
		ActiveEdgeToHandleMap.RemoveSinglePair(Edge,Handle);
	}	
}
...
FLOAT ExpansionMinPolyDotForAbsorption = 0.9f;
#define MIN_ABSORPTION_DOT ExpansionMinPolyDotForAbsorption
FLOAT FindOptimalExpansionForRectanglePoly(FNavMeshPolyBase* Poly, UNavigationMeshBase* Mesh, UBOOL bBailOnCrappyNodes, INT& NumMerged, ControlPointMap& CtrlPtMap)
{
	...
	if(Poly->CalcArea() < FNavMeshPolyBase::CalcArea(BestConfig.Verts,Mesh))
	{
		...
		for(INT AbsorbedIdx=0;AbsorbedIdx<AbsorbedPolys.Num();AbsorbedIdx++)
		{
			...
			//if(!FPlaneAABBIsect(CurPoly_Plane,Poly->GetPolyBounds(WORLD_SPACE)))
			...
			if( (CurPoly->GetPolyNormal(LOCAL_SPACE) | Poly->GetPolyNormal(LOCAL_SPACE)) < MIN_ABSORPTION_DOT )
			{
				continue;	
			}
...
INT UNavigationMeshBase::FindStartingIndex( FNavMeshPolyBase* Poly, FVector EdgePt0, FVector EdgePt1, VERTID SharingEdgeVert_0, VERTID SharingEdgeVert_1 )
{
	...
	UBOOL bShare0_Next_OnEdgePt = FPointsAreNear(GetVertLocation(NextVert_0,WORLD_SPACE),EdgePt0,NEAR_EDGE_PT_TOL)||FPointsAreNear(GetVertLocation(NextVert_0,WORLD_SPACE),EdgePt1,NEAR_EDGE_PT_TOL);
	UBOOL bShare1_Next_OnEdgePt = FPointsAreNear(GetVertLocation(NextVert_1,WORLD_SPACE),EdgePt0,NEAR_EDGE_PT_TOL)||FPointsAreNear(GetVertLocation(NextVert_1,WORLD_SPACE),EdgePt1,NEAR_EDGE_PT_TOL);
	...
	{
// 		UBOOL bShare0_OnEdgePt = FPointsAreNear(GetVertLocation(SharingEdgeVert_0,WORLD_SPACE),EdgePt0,NEAR_EDGE_PT_TOL)||FPointsAreNear(GetVertLocation(SharingEdgeVert_0,WORLD_SPACE),EdgePt1,NEAR_EDGE_PT_TOL);
// 		UBOOL bShare1_OnEdgePt = FPointsAreNear(GetVertLocation(SharingEdgeVert_1,WORLD_SPACE),EdgePt0,NEAR_EDGE_PT_TOL)||FPointsAreNear(GetVertLocation(SharingEdgeVert_1,WORLD_SPACE),EdgePt1,NEAR_EDGE_PT_TOL);
...
UBOOL UNavigationMeshBase::PerformEdgeSmoothingStep( 				
													...
													UBOOL bConvexPoly)
	{
	static UBOOL bTrace = FALSE;// MT->VerifyTrajectoryForEdgeSimplification is better at checking this anyway
	static UBOOL bIsect = TRUE;
	static UBOOL bLegCheck = TRUE;
	...
	if(NAVMESHGEN_DO_EDGESMOOTHING)
	{
			...
			)
		{
			...
			FLOAT LegADist = (CurrentAct-PrevAct).Size();
			FLOAT LegBDist = (CurrentAct-NextAct).Size();
			...
			if(    (!bLegCheck || (OffDist < LegADist && OffDist < LegBDist))
				&& (!bTrace || 
								(
								   GWorld->SingleLineCheck(Hit,NULL,WS_Prev+2.0f*WS_PolyNorm,WS_Next+2.0f*WS_PolyNorm,EXPANSION_TRACE_FLAGS,FVector(1.f))
				                && GWorld->SingleLineCheck(Hit,NULL,WS_Prev+NAVMESHGEN_MAX_STEP_HEIGHT*WS_PolyNorm,WS_Next+NAVMESHGEN_MAX_STEP_HEIGHT*WS_PolyNorm,EXPANSION_TRACE_FLAGS,FVector(1.f))
								)
					)
				&& (!bSlopeStepCheck || VerifyTrajectoryForEdgeSimplification(WS_Prev+FVector(0.f,0.f,NAVMESHGEN_ENTITY_HALF_HEIGHT),LegCNorm,LegCDist,FPathBuilder::GetScout(),HeightOffset,&Extent))
...
UBOOL FNavMeshPolyBase::IntersectsPoly(const TArray<FVector>& OtherPolyVerts,UBOOL bWorldSpace, const FLOAT ExpandDist)
{
	...
	FPlane NewPolyPlane(OtherPolyVerts(0),OtherPolyVerts(1),OtherPolyVerts(2));
	FPlane ThisPolyPlane = GetPolyPlane(bWorldSpace);
	// if the poly's plane does not clip this poly's AABB, throw it out
	FBox TransformedBBox = GetPolyBounds(bWorldSpace);
	if( ! FPlaneAABBIsect(  NewPolyPlane, TransformedBBox) )
	{
		return FALSE;
	}
...
UBOOL FNavMeshPolyBase::IntersectsPoly2D(const FVector& In_SegPt0, const FVector& In_SegPt1, FVector& out_EntryPoint, FVector& out_ExitPoint,UBOOL bWorldSpace/*=LOCAL_SPACE*/)
{
	...
	const FVector Ctr = GetPolyCenter(bWorldSpace);
	...
	for(INT VertIdx=0;VertIdx<PolyVerts.Num();VertIdx++)
	{
		const FVector CurEdgePt0 = NavMesh->Verts(PolyVerts(VertIdx));
		const FVector CurEdgePt1 = NavMesh->Verts(PolyVerts(( VertIdx + 1 ) % PolyVerts.Num()));

		// find outward normal of this edge 
		const FVector CurEdgeNormal = ((CurEdgePt1 - CurEdgePt0) ^ Norm).SafeNormal();
...
*/
UBOOL FNavMeshPolyBase::GetBestLocationForCyl(const FVector& WS_CylIdealLoc, FLOAT CylRadius, FVector& out_BestLoc, UBOOL bForceInteriorPosition)
{
#define BESTCYLLOCISECT_THRESH 5.0f
	...
	{
		AdjustPositionToDesiredHeightAbovePoly(ClosestEdgePt,CylRadius);
	...
	if( BestCornerDist < BIG_NUMBER )
	{
		...
		AdjustPositionToDesiredHeightAbovePoly(ClosestCornerPt,CylRadius);
