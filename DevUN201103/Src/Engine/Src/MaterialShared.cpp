...
{
	checkAtCompileTime(5 == SF_NumFrequencies, Bad_EShaderFrequency);
	switch(Frequency)
	{
...
UBOOL FMaterialResource::RenderTwoSidedSeparatePass() const { return Material->TwoSidedSeparatePass; }
...
UBOOL FMaterialResource::TranslucencyReceiveDominantShadowsFromStatic() const { return Material->bTranslucencyReceiveDominantShadowsFromStatic; }
...
UBOOL FMaterialResource::CastLitTranslucencyShadowAsMasked() const { return Material->bCastLitTranslucencyShadowAsMasked; }
...
FMaterial::~FMaterial()
{
	...
	if (ShaderMap)
	{
		ShaderMap->RemovePendingMaterial(this);
		ShaderMap->BeginRelease();
	}
...
protected:
	...
	/** The current material property being compiled.  This affects the behavior of all compiler funtions except GetFixedParameterCode. */
