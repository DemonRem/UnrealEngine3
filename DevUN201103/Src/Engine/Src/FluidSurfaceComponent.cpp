...
void UFluidSurfaceComponent::Tick(FLOAT DeltaTime)
{
	...
	if (GIsGame && FluidSimulation)
	{
		// Find the closest player to the fluid surface, use that player's view location as the detail position
		...
		for( INT PlayerIndex = 0; PlayerIndex < GEngine->GamePlayers.Num(); PlayerIndex++ )
		{
			if( GEngine->GamePlayers(PlayerIndex) )
			{
				...
				const FLOAT Distance = CalcDistance( PlayerViewLocation );
				if ( Distance < ViewDistance )
				{
					ClosestPlayerViewLocation = PlayerViewLocation;
					ViewDistance = Distance;
				}
			}
		}
	}
	else if (GIsEditor && FluidSimulation)
	{
		ClosestPlayerViewLocation = EditorViewPosition;
	}
