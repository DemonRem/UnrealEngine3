...
void UAudioDevice::Flush( FSceneInterface* SceneToFlush )
{
	...
	for( INT ComponentIndex = AudioComponents.Num() - 1; ComponentIndex >= 0; ComponentIndex-- )
	{
		...
		if( AudioComponent )
		{
			...
			if( ( AudioComponent->bIgnoreForFlushing ) && ( GIsEditor == FALSE ) )
			{
...
UBOOL FSoundSource::SetReverbApplied( void )
{
	...
	bReverbApplied = WaveInstance->bReverb;
...
/**
 * Set the bRadioDistortionApplied variable
 */
UBOOL FSoundSource::SetRadioDistortionApplied( void )
{
	// By default, do not apply radio distortion because it usually won't be applied. 
	bRadioDistortionApplied = FALSE;

	// Do not apply radio distortion to music or multichannel sounds.
	if( !WaveInstance->bIsMusic
	// Do not apply radio distortion to multichannel sounds.
	 &&  WaveInstance->WaveData->NumChannels > 0
	 &&  WaveInstance->WaveData->NumChannels < 3
	// Must have radio volume set in the sound class.
	 &&  WaveInstance->RadioFilterVolumeThreshold > KINDA_SMALL_NUMBER )
	{
		bRadioDistortionApplied = TRUE;
	}
	...
	return( bRadioDistortionApplied );
}
