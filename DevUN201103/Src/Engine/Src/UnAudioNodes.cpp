...
INT USoundNodeWave::GetResourceSize( UE3::EPlatformType Platform )
{
	...
	else if( ( Platform & UE3::PLATFORM_Xenon ) != 0 )
	{
		ResourceSize += CompressedXbox360Data.GetBulkDataSize();
	}
...
void USoundNodeWave::StripData(UE3::EPlatformType PlatformsToKeep, UBOOL bStripLargeEditorData)
{
	...
	if (!(PlatformsToKeep & UE3::PLATFORM_Xenon))
	{
		CompressedXbox360Data.RemoveBulkData();
	}
...
void USoundNodeWave::PostLoad( void )
{
	...
	if( LocalizedSubtitles.Num() > 0 )
	{
		if (GLanguageExtensionIndex == -1)
		{
			...
			if (GLanguageExtensionIndex == -1)
			{
				debugfSlow(NAME_Warning, TEXT("\tDid not find language extenstion %s"), UObject::GetLanguage());
		...
		if( GUseFullLocalizationFiles == FALSE )
		{
...
void USoundNodeAttenuation::Serialize( FArchive& Ar )
{
	...
	if( Ar.Ver() < VER_DEPRECATE_SOUND_RANGES )
	{
		...
		MinRadius_DEPRECATED.GetOutRange( Min, Max );
		RadiusMin = ( Max + Min ) / 2.0f;

		MaxRadius_DEPRECATED.GetOutRange( Min, Max );
		RadiusMax = ( Max + Min ) / 2.0f;

		LPFMinRadius_DEPRECATED.GetOutRange( Min, Max );
		LPFRadiusMin = ( Max + Min ) / 2.0f;

		LPFMaxRadius_DEPRECATED.GetOutRange( Min, Max );
		LPFRadiusMax = ( Max + Min ) / 2.0f;

		bAttenuateWithLPF = bAttenuateWithLowPassFilter_DEPRECATED;
	}
}
 ...
 */
void USoundNodeAttenuation::PostLoad( void )
{
	Super::PostLoad();
	...
	if( Linker )
	{
		...
		// This version deprecated the DistanceModel variable for the DistanceAlgorithm variable so 
		// that the default algorithm would change from ATTENUATION_Logarithmic to ATTENUATION_Linear.
		// We need to preserve the algorithm type to prevent data loss. 
		if( Version < VER_SOUNDNODEATTENUATION_DISTANCEMODEL_CHANGE )
		{
			DistanceAlgorithm = DistanceModel_DEPRECATED;
		}
	}
}
...
void USoundNodeAmbient::Serialize( FArchive& Ar )
{
	Super::Serialize( Ar );
	...
	if( Ar.Ver() < VER_AMBIENT_SOUND_UPDATE )
	{
		FAmbientSoundSlot SoundSlot;
		SoundSlot.Wave = Wave_DEPRECATED;

		SoundSlots.AddItem( SoundSlot );
	}
...
UBOOL USoundNodeConcatenatorRadio::ApplyChirpSound( UAudioDevice* AudioDevice, UAudioComponent* AudioComponent, USoundNodeWave* Chirp )
{
	...
	if( AudioComponent->CurrentVolume * Chirp->Volume < AudioComponent->CurrentRadioFilterVolumeThreshold )
	{
		return( TRUE );
	}
...
void USoundNodeConcatenatorRadio::ParseNodes( UAudioDevice* AudioDevice, USoundNode* Parent, INT ChildIndex, UAudioComponent* AudioComponent, TArray<FWaveInstance*>& WaveInstances )
{
	...
	case 0:
		...
		{
			// If no chirp in sound, or radio not applied, play the dialog line straight away
			NodeIndex = 1;
	...
	case 2:
		if( ApplyChirpSound( AudioDevice, AudioComponent, AudioDevice->ChirpOutSoundNodeWave ) )
		{
