	...
	) const
{
	...
	SetVertexShaderValue(VertexShader->GetVertexShader(), VertexShaderParameters->LightDirectionParameter, FVector4(-(LightSceneInfo->GetDirection()), PointLightInfo ? 0.0f : 1.0f));
	SetVertexShaderValue(
		VertexShader->GetVertexShader(),
		VertexShaderParameters->LightPositionAndInvRadiusParameter,
		FVector4(
		LightSceneInfo->GetOrigin() + View->PreViewTranslation,
		PointLightInfo ? PointLightInfo->InvRadius : 1.0f
		)
		);
