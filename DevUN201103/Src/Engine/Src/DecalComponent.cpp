...
public:
	...
	*/
	...
	virtual EMemoryStats GetMemoryStatType( void ) const { return( STAT_GameToRendererMallocDecalSP ); }
...
void UDecalComponent::SetParentToWorld(const FMatrix& ParentToWorld)
{
	...
	if( !bHasBeenAttached )
	{
		if (DecalTransform == DecalTransform_SpawnRelative)
		{
			...
			FMatrix ParentToWorldInverse = ParentToWorld.Inverse();
			OriginalParentRelativeLocation = ParentToWorldInverse.TransformFVector(Location);
			OriginalParentRelativeOrientationVec = ParentToWorldInverse.TransformNormal(Orientation.Vector()).SafeNormal();
		}
		else if (DecalTransform == DecalTransform_OwnerRelative)
		{
			...
			OriginalParentRelativeLocation = ParentRelativeLocation;
			OriginalParentRelativeOrientationVec = ParentRelativeOrientation.Vector();
		}
	}
	...
	else
	{
		...
		Location = ParentToWorld.TransformFVector(OriginalParentRelativeLocation);
		Orientation = ParentToWorld.TransformNormal(OriginalParentRelativeOrientationVec).Rotation();
	}
}
...
void UDecalComponent::UpdateTransform()
{
	...
	if( bMovableDecal &&
		Owner && Owner->bMovable )
	{
		// only moveable decals can reattach to receivers dynamically
