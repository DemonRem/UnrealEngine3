...
void AActor::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyThatChanged != NULL && PropertyThatChanged->GetFName() == FName(TEXT("Group")))
	{
		// tell the world that the set of groups may have changed (and that this actor caused the change)
		GCallbackEvent->Send(CALLBACK_GroupsHaveChanged, this);
	}
...
void AInterpActor::SetShadowParentOnAllAttachedComponents()
{
	if (!Base)
	{
		...
		while (ProcessStack.Num() > 0)
		{
			...
			{
				UMeshComponent* MeshComp = Cast<UMeshComponent>(AttachedActor->Components(ComponentIndex));
				if (MeshComp != NULL)
				{
					...
					//warnf( TEXT("SetShadowParent: %s"), *MeshComp->GetFullName() );
					if (StaticMeshComponent != NULL && bOverrodeShadowOrDLE)
					{
