...
extern UBOOL TrackTextureEvent( UTexture2D* Texture, UBOOL bIsDestroying, UBOOL bEnableLogging, UBOOL bForceMipLevelsToBeResident );
	...
	for (INT MipIndex = 0; MipIndex < InTexture->Mips.Num(); MipIndex++)
	{
		if (MipIndex < InTexture->Mips.Num() && InTexture->Mips(MipIndex).Data.IsAvailableForUse())
		{
			...
			else			
			{
				...
				InTexture->Mips(MipIndex).Data.GetCopy(&MipDataPtr, TRUE);
			}
		}
	}
}
