...
DECLARE_CYCLE_STAT(TEXT("Light visibility time"),STAT_LightVisibilityTime,STATGROUP_DLE);
DECLARE_CYCLE_STAT(TEXT("UpdateStaticEnvironment time"),STAT_UpdateStaticEnvironmentTime,STATGROUP_DLE);
DECLARE_CYCLE_STAT(TEXT("Interpolate indirect time"),STAT_InterpolateIndirectTime,STATGROUP_DLE);
DECLARE_CYCLE_STAT(TEXT("UpdateDynamicEnvironment time"),STAT_UpdateDynamicEnvironmentTime,STATGROUP_DLE);
DECLARE_CYCLE_STAT(TEXT("UpdateEnvironmentInterpolation time"),STAT_UpdateEnvironmentInterpolationTime,STATGROUP_DLE);
DECLARE_CYCLE_STAT(TEXT("Attach time"),STAT_AttachLightEnvironmentTime,STATGROUP_DLE);
DECLARE_CYCLE_STAT(TEXT("CreateLights time"),STAT_CreateLightsTime,STATGROUP_DLE);
DECLARE_DWORD_COUNTER_STAT(TEXT("Light Environments"),STAT_NumEnvironments,STATGROUP_DLE);
DECLARE_DWORD_COUNTER_STAT(TEXT("Environment Updates"),STAT_StaticEnvironmentUpdates,STATGROUP_DLE);
DECLARE_DWORD_COUNTER_STAT(TEXT("Environment Updates"),STAT_DynamicEnvironmentUpdates,STATGROUP_DLE);
...
void FDynamicLightEnvironmentState::Tick(FLOAT DeltaTime)
{
	...
	if (!GLightEnvironmentDebugInfo.Component || GLightEnvironmentDebugInfo.Component == Component)
	{
		if (GLightEnvironmentDebugInfo.bShowBounds)
		{
			...
			Component->Owner->DrawDebugSphere(OwnerBounds.Origin, OwnerBounds.SphereRadius, 12, 200, 200, 255, FALSE);
		}
		...
		if (GLightEnvironmentDebugInfo.bShowStaticUpdates)
		{
			for (INT i = 0; i < Component->State->DebugStaticUpdates.Num(); i++)
			{
				...
				Component->Owner->DrawDebugSphere(CurrentPosition, 50.0f, 12, 155, 155, 100, FALSE);
			}
		}
		if (GLightEnvironmentDebugInfo.bShowCreateLights)
		{
...
UPointLightComponent* FDynamicLightEnvironmentState::CreateRepresentativeShadowLight(const FLinearColor& Intensity)
{
	// Construct a point light to represent the brightest direction of the remaining light environment.
...
void FDynamicLightEnvironmentState::CreateEnvironmentLightList(ULightComponent* NewAffectingDominantLight, FLOAT NewDominantShadowTransitionDistance, UBOOL bForceUpdate)
{
	...
	if(	LightError > ErrorThreshold ||
		...
		(CompositeShadowInfo.ShadowDirection | CurrentShadowInfo.ShadowDirection) < .99996f ||
		...
		bForceUpdate)
	{
			...
			&& CompositeShadowInfo.ShadowDirection.SizeSquared() > DELTA)
		{
			...
			DominantShadowIntensityRatio = Lerp(FLinearColor::White, DominantShadowIntensityRatio, PrimaryLightWeight);
			...
			CurrentShadowInfo.ShadowDirection.Z = Max(CurrentShadowInfo.ShadowDirection.Z, appCos(AngleFromZAxis));
			CurrentShadowInfo.ShadowDirection = CurrentShadowInfo.ShadowDirection.UnsafeNormal();
	
			CurrentRepresentativeShadowLight = CreateRepresentativeShadowLight(DominantShadowIntensityRatio);
		}
	}
	...
	if(CurrentRepresentativeShadowLight)
	{
		...
		const FVector LightPosition = OwnerBounds.Origin + CurrentShadowInfo.ShadowDirection * LightDistance;
...
UBOOL FDynamicLightEnvironmentState::IsLightVisible(const ULightComponent* Light, const FVector& OwnerPosition, UBOOL bIsDynamic, FLOAT& OutVisibilityFactor) const
{
	...
	if(!Light->CastShadows || !Light->CastStaticShadows || !Component->bShadowFromEnvironment)
	{
		OutVisibilityFactor = 1.0f;
		return TRUE;
	}
...
{
	SCOPE_CYCLE_COUNTER(STAT_AttachLightEnvironmentTime);
...
void UDynamicLightEnvironmentComponent::CheckForErrors()
{
	...
	if (Owner && IsValidComponent() && IsEnabled() && State)
	{
		...
		if (GWorld && GWorld->PersistentLevel)
		{
			...
			FPrecomputedLightVolume* PrecomputedLightVolume = GWorld->PersistentLevel->PrecomputedLightVolume;
			if (!bAlreadyWarned && PrecomputedLightVolume && PrecomputedLightVolume->IsInitialized())
			{
				const FBox& PLVBounds = PrecomputedLightVolume->GetBounds();
				const FBox DLEBounds = State->GetOwnerBounds().GetBox();
				if (!(DLEBounds.Min == FVector::ZeroVector && DLEBounds.Max == FVector::ZeroVector) && !PLVBounds.Intersect(DLEBounds))
				{
					GWarn->MapCheck_Add( MCTYPE_WARNING, Owner, *FString::Printf(TEXT("Dynamic Light Environment is outside of the Lightmass Importance Volume, will have black indirect lighting until moved inside.") ), MCACTION_NONE, TEXT("") );
				}
			}
		}
	}
