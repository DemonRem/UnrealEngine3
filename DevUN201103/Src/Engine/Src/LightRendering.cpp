...
public:
	...
	void SetParameters(const FSceneView& View, const FLightSceneInfo* LightSceneInfo)
	{
		...
		SetPixelShaderValue(GetPixelShader(), SpecularScaleParameter, LightSceneInfo->bUseImageReflectionSpecular ? 0.0f : 1.0f);
	}
...
UBOOL FSceneRenderer::RenderLights(UINT DPGIndex,UBOOL bAffectedByModulatedShadows, UBOOL bWasSceneColorDirty)
{
	...
	for(TSparseArray<FLightSceneInfoCompact>::TConstIterator LightIt(Scene->Lights);LightIt;++LightIt)
	{
		...
		if(bLightIsVisibleInAnyView)
		{
			...
			SCOPED_DRAW_EVENT(EventLightPass)(DEC_SCENE_ITEMS,TEXT("%s Dynamic channel %u"),*LightSceneInfo->GetLightName().ToString(), LightSceneInfo->LightingChannels.Dynamic);
			...
			if (bUseAttenuationBuffer)
			{
				...
				if (bHasInheritDominantShadowMaterials && !GOnePassDominantLight && LightSceneInfo->LightType == LightType_DominantDirectional)
				{
