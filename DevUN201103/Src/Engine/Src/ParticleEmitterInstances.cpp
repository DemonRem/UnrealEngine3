		...
		for (INT i=0; i<ActiveParticles; i++)
		{
			...
			{	
				...
				else
				{
					...
					ParticleBoundingBox += FBox(NewLocation - ParticleExtent, NewLocation + ParticleExtent);
				}
			}
		}
...
void FParticleEmitterInstance::CheckSpawnCount(UBOOL bIsSubUV, INT InNewCount, INT InMaxCount)
{
	...
	if (WorldInfo)
	{
		...
		FString ErrorMessage = 
			FString::Printf(TEXT("Emitter %2d spawn vertices: %10d (%8.3f kB of verts), clamp to %10d (%8.3f kB): %s"),
			MyIndex,
			InNewCount, 
