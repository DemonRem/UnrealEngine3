...
UBOOL UPhysicsAsset::LineCheck(FCheckResult& Result, class USkeletalMeshComponent* SkelComp, const FVector& Start, const FVector& End, const FVector& Extent, UBOOL bPerPolyShapes)
{
	...
	{
		debugf( TEXT("UPhysicsAsset::LineCheck : Non-uniform scale factor. You will not be able to collide with it.  Turn off collision and wrap it with a blocking volume. SkelComp %s (%s) Scale %f Scale3D %s Owner %s Scale %f Scale3D %s"), 
			*SkelComp->GetName(),
