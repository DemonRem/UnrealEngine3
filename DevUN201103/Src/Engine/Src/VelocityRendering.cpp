...
public:
	...
	{
		return PrevViewProjectionMatrixParameter.IsBound();
	}
...
public:
	...
	UBOOL SupportsVelocity() const
	{
		return PreviousLocalToWorldParameter.IsBound() && VelocityParameters.SupportsVelocity();
	}
...
public:
	...
	UBOOL SupportsVelocity() const
	{
		return VelocityParameters.SupportsVelocity();
	}
...
public:
	...
	void SetMesh(const FPrimitiveSceneInfo* PrimitiveSceneInfo,const FMeshElement& Mesh,const FViewInfo& View,UBOOL bBackFace,FLOAT InstanceMotionBlurScale, UBOOL bFullMotionBlur)
	{
		...
		if(!ForegroundDPG)
		{
			...
			// for now hard coded to not affect the result (to find where the fix is actually needed)
			InstanceMotionBlurScale = 1.0f;

			FLOAT ObjectMotionBlurScale	= InstanceMotionBlurScale * ViewMotionBlurScale;
	...
	) const
{
	...
	PixelShader->SetMesh(PrimitiveSceneInfo,Mesh,View,bBackFace,PrimitiveSceneInfo->Component->MotionBlurScale, bFullMotionBlur);
...
UBOOL FVelocityDrawingPolicy::HasVelocity(const FViewInfo& View, const FPrimitiveSceneInfo* PrimitiveSceneInfo)
{
	...
	if ( !View.bRequiresVelocities || !PrimitiveSceneInfo->Proxy->IsMovable() )
	{
		return FALSE;
	}
	...
	if ( Abs(PrimitiveSceneInfo->Component->MotionBlurScale - 1.0f) < 0.0001f )
	{
		...
		// determine if the primitive needs per bone motion blur update
		const USkeletalMeshComponent* SkeletalMeshComponent = ConstCast<USkeletalMeshComponent>(Component);
