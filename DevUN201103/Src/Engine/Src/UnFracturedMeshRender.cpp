 ...
 */
void FFracturedStaticMeshSceneProxy::SetIndexSource(INT LODIndex, INT ElementIndex, INT FragmentIndex, FMeshElement& OutMeshElement, UBOOL bWireframe) const
{
	...
	if (bWireframe)
	{
		if (LODIndex == 0 && bUseDynamicIndexBuffer)
		{
			...
			OutMeshElement.IndexBuffer = &FracturedIndexBuffer;
			OutMeshElement.NumPrimitives = FracturedIndexBuffer.Indices.Num() / 3;
		}
		else
		{
	...
	else
	{
		...
		if (LODIndex == 0)
		{
			if (bUseDynamicIndexBuffer)
			{
				...
				OutMeshElement.FirstIndex = ElementRange.BaseIndex;
				OutMeshElement.NumPrimitives = ElementRange.NumPrimitives;
			}
			else
			{
...
void FFracturedStaticMeshSceneProxy::BuildMissingDecalStaticMeshElements_RenderThread()
{
	...
	for( INT DecalIdx=0; DecalIdx < Decals.Num(); DecalIdx++ )
	{
		FDecalInteraction* DecalInteraction = Decals(DecalIdx);
		if( DecalInteraction && 
			!DecalInteraction->DecalStaticMesh )
		{
...
void FFracturedSkinnedMeshSceneProxy::DrawDynamicElements(FPrimitiveDrawInterface* PDI,const FSceneView* View,UINT DPGIndex,DWORD Flags)
{
	...
	if (GetDepthPriorityGroup(View) == DPGIndex)
	{
		...
		for(INT ElementIndex = 0;ElementIndex < LODModel.Elements.Num();ElementIndex++)
		{
			...
			checkSlow(bUseDynamicIndexBuffer);
			const FRawIndexBuffer& FracturedIndexBuffer = ComponentBaseResources->InstanceIndexBuffer;
			MeshElement.IndexBuffer = &FracturedIndexBuffer;
			...
			for (INT ChunkIndex = 0; ChunkIndex < ElementRanges(ElementIndex).Num(); ChunkIndex++)
			{
				...
				MeshElement.FirstIndex = ElementRange.BaseIndex;
				MeshElement.NumPrimitives = ElementRange.NumPrimitives;
