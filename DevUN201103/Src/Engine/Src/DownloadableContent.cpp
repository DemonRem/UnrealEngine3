 ...
 * @return true if the bundle could be installed correctly, false if it failed
 */
UBOOL UDownloadableContentManager::GetDLCTextureCachePath(FName TextureCacheName,FString& Path)
{
    FString* Result = TextureCachePathMap.Find(TextureCacheName);
	if (Result != NULL)
	{
		Path = *Result;
		return TRUE;
...
{
	// Update the TFC map with any entries
	InstallTFCFiles(DLCBundle);
 ...
 */
void UDownloadableContentManager::InstallTFCFiles(const FOnlineContent& DLCBundle)
{
	...
	{
		if (DLCBundle.ContentFiles(FileIndex).Right(4) == TEXT(".tfc"))
		{
			const FFilename FullFilename = FFilename(DLCBundle.ContentFiles(FileIndex));
			debugf(NAME_DevDlc,TEXT("Caching DLC texture file cache: %s/%s"),*FullFilename.GetBaseFilename(),*FullFilename);
			// Add the file to our map
			TextureCachePathMap.Set(FName(*FullFilename.GetBaseFilename()),FullFilename);
		}
	}
}
	...
	for (INT FileIndex = 0; FileIndex < DLCBundle.ContentFiles.Num(); FileIndex++)
	{
			...
			Ext == TEXT("int"))
		{
			...
			else
			{
				...
				ConfigFileName = GSys->LocalizationPaths(0) * Ext * ContentFile;
			}
			...
			if (bIsINI)
			{
				...
				AddPackagesToFullyLoad(ConfigFileName);
			}
		}
	}
	// Reload config/loc for any objects affected
...
void UDownloadableContentManager::AddPackagesToFullyLoad(EFullyLoadPackageType LoadType,const TCHAR* Section,const TCHAR* KeyOne,const TCHAR* KeyN,const TCHAR* FileName)
{
	...
	{
		GameEngine->AddPackagesToFullyLoad(LoadType,It.Key().ToString(),It.Value(),TRUE);
	}
}
