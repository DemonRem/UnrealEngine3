...
/**
 * The shadow FadeAlpha calculated for an individual view has to be higher than this to be projected in that view
 * (set to a negative value to project in all views, assuming any view is large enough to render depths).
 */
static const FLOAT GMinimumPerViewShadowAlphaCutoff = 0.0f;
			...
			else
			{
				...
				for (INT MeshIndex = 0; MeshIndex < PrimitiveSceneInfo->StaticMeshes.Num(); MeshIndex++)
				{
					...
					if (StaticMesh.CastShadow 
						...
						&& DistanceSquared < StaticMesh.MaxDrawDistanceSquared)
					{
						StaticMeshWholeSceneShadowDepthMap(StaticMesh.Id) = TRUE;
						bDrawingStaticMeshes = TRUE;
					}
 ...
 */
void FSceneRenderer::GenerateChildSceneInfos(const FPrimitiveSceneInfo* InPrimitiveSceneInfo, TArray <FPrimitiveSceneInfo*, SceneRenderingAllocator>& OutChildSceneInfos) const
{
	...
	if(ShadowGroup)
	{
		for(INT ChildIndex = 0;ChildIndex < ShadowGroup->Primitives.Num();ChildIndex++)
		{
			...
			GenerateChildSceneInfos(ShadowChild, OutChildSceneInfos);
		}
	}
}
	...
	)
{
	...
	if(!PrimitiveSceneInfo->ShadowParent)
	{
		...
		GenerateChildSceneInfos(PrimitiveSceneInfo, ChildSceneInfos);
		...
		for(INT ViewIndex = 0;ViewIndex < Views.Num();ViewIndex++)
		{
			...
			FVector4 ScreenPosition = View.WorldToScreen(OriginalBounds.Origin);
			...
			// Determine if the shadow and/or preshadow is visible in this particular view
			// If it's visible in any view, we need to do the depth render, but the projection may be pointless in some views
			const INT CShadowFadeResolution = (LightSceneInfo->ShadowFadeResolution > 0) ? LightSceneInfo->ShadowFadeResolution : GSystemSettings.ShadowFadeResolution;
			const FLOAT ViewSpecificAlpha = CalculateShadowFadeAlpha( UnclampedResolution, CShadowFadeResolution, MinShadowResolution );
			const FLOAT ViewSpecificPreShadowAlpha = CalculateShadowFadeAlpha( appTrunc(UnclampedResolution * GSystemSettings.PreShadowResolutionFactor), GSystemSettings.PreShadowFadeResolution, GSystemSettings.MinPreShadowResolution );

			if (ViewSpecificAlpha <= GMinimumPerViewShadowAlphaCutoff)
			{
				ViewMask |= (1 << ViewIndex);
			}

			if (ViewSpecificPreShadowAlpha <= GMinimumPerViewShadowAlphaCutoff)
			{
				ViewMaskPreshadow |= (1 << ViewIndex);
			}
		}
		...
		const FLOAT ResolutionFadeAlpha = CalculateShadowFadeAlpha( MaxUnclampedResolution, ShadowFadeResolution, MinShadowResolution );
		const FLOAT ResolutionPreShadowFadeAlpha = CalculateShadowFadeAlpha( appTrunc(MaxUnclampedResolution * GSystemSettings.PreShadowResolutionFactor), GSystemSettings.PreShadowFadeResolution, GSystemSettings.MinPreShadowResolution );
		...
		if ((ResolutionFadeAlpha > 1.0f / 256.0f || bRenderPreShadow && ResolutionPreShadowFadeAlpha > 1.0f / 256.0f)
			&& LightSceneInfo->GetProjectedShadowInitializer(Bounds,ShadowInitializer))
		{
			...
			const FLOAT FadeAlpha = ResolutionFadeAlpha * ElapsedTimeFadeFraction;
				...
				&& (!IsDominantLightType(LightSceneInfo->LightType) || LightSceneInfo->bStaticShadowing))
			{
				...
				ProjectedShadowInfo = new(GRenderingThreadMemStack,1,16) FProjectedShadowInfo(
					...
					SizeX,
					SizeY,
					PrimitiveSceneInfo->bAllowShadowFade ? FadeAlpha : 1.0f
					);

				// Store the view-specific mask for this shadow
				ProjectedShadowInfo->ViewMaskOut = ViewMask;
				...
				&& (bOpaqueRelevance || bTranslucentRelevance))
			{
				...
				if (bTranslucentRelevance || bForegroundRelevance || bIsOutsideWholeSceneShadow)
				{
					...
					if (ProjectedPreShadowInfo)
					{
						...
						ProjectedPreShadowInfo->FadeAlpha = PrimitiveSceneInfo->bAllowShadowFade ? PreFadeAlpha : 1.0f;
					}
					else
					{
						...
						ProjectedPreShadowInfo = new FProjectedShadowInfo(
							...
							PreshadowSizeX,
							PreshadowSizeY,
							PrimitiveSceneInfo->bAllowShadowFade ? PreFadeAlpha : 1.0f
							);
					}

					// Store the view-specific mask for this preshadow
					ProjectedPreShadowInfo->ViewMaskOut = ViewMaskPreshadow;
