 ...
 */
UBOOL TrackTextureEvent( UTexture2D* Texture, UBOOL bIsDestroying, UBOOL bEnableLogging, UBOOL bForceMipLevelsToBeResident )
{
			...
			if ( appStristr(TextureName, *TrackedTextureName) != NULL )
			{
				if ( bEnableLogging )
				{
						 ...
						 bIsDestroying )
					{
						...
						debugf( NAME_DevStreaming, TEXT("Texture: \"%s\", ResidentMips: %d/%d, RequestedMips: %d, StreamingStatus: %d, Forced: %d (%s)"), TextureName, LastEvent->NumResidentMips, Texture->Mips.Num(), bIsDestroying ? 0 : LastEvent->NumRequestedMips, LastEvent->StreamingStatus, bForceMipLevelsToBeResident, bIsDestroying ? TEXT("DESTROYED") : TEXT("updated") );
					}
				}
				return TRUE;
			}
...
{
	FTextureStreamingStats( UTexture2D* InTexture2D, ETextureStreamingType InType, INT InResidentMips, INT InWantedMips, INT InResidentSize, INT InWantedSize, INT InMaxSize )
		:	Texture( InTexture2D )
		,	StreamType( InType )
...
struct FStreamingTexture
{
	...
	void UpdateCachedInfo( )
	{
		...
		DynamicWantedMips = INDEX_NONE;
		DynamicMinDistance = FLT_MAX;
	}
...
//		StreamingManager.CalcDynamicWantedMips();
		...
		for ( INT Index=0; Index < StreamingManager.StreamingTextures.Num() && !IsAborted(); ++Index )
		{
			...
			{
				UBOOL bTrackedTexture = TrackTextureEvent( StreamingTexture.Texture, FALSE, ENABLE_TEXTURE_LOGGING, StreamingTexture.bForceFullyLoad );
 ...
 */
void FStreamingManagerCollection::UpdateAllResources( FLOAT TimeLimit/*=0.0f*/, UBOOL bUseMipFading/*=FALSE*/ )
{
	// Disable mip-fading for upcoming texture updates, if requested.
	FLOAT PrevMipLevelFadingState = GEnableMipLevelFading;
	if ( bUseMipFading == FALSE )
	{
	...
	BlockTillAllRequestsFinished( TimeLimit );
 ...
 */
void FStreamingManagerCollection::BlockTillAllRequestsFinished( FLOAT TimeLimit /*= 0.0f*/ )
{
	// Route to streaming managers.
	for( INT ManagerIndex=0; ManagerIndex<StreamingManagers.Num(); ManagerIndex++ )
	{
		FStreamingManagerBase* StreamingManager = StreamingManagers(ManagerIndex);
		StreamingManager->BlockTillAllRequestsFinished( TimeLimit );
	}
}
...
/**
 * Legacy: Updates streaming, taking into all views infos.
 *
 * @param DeltaTime				Time since last call in seconds
 * @param bProcessEverything	[opt] If TRUE, process all resources with no throttling limits
 */
void FStreamingManagerTexture::UpdateResourceStreaming( FLOAT DeltaTime, UBOOL bProcessEverything/*=FALSE*/ )
{
	...
	while ( (IndividualStreamingTexture || CurrentStreamableLink) && Context.MaxTexturesToProcess-- )
	{
		...
		UBOOL bTrackedTexture = TrackTextureEvent( Texture, FALSE, ENABLE_TEXTURE_LOGGING, FALSE );
		...
		if ( FStreamingTexture::IsStreamingTexture(Texture) )
		{
			...
			if ( Texture->IsReadyForStreaming() )
			{
				...
				{
					BeginUpdateMipCount( StreamingTexture, Context, FALSE );
				}
			}
			...
			// This section can be used to track how specific textures are streamed.
			TrackTextureEvent( Texture, FALSE, ENABLE_TEXTURE_LOGGING, StreamingTexture.bForceFullyLoad );
		}
	}
...
void FStreamingManagerTexture::StreamTextures( UBOOL bProcessEverything )
{
	...
	if ( Context.TextureStats.Num() > 0 )
	{
		...
		INT TotalMaxSize		= 0;
		debugf( TEXT(",Current Width,Current Height,Wanted Width,Wanted Height,Max Width,Max Height,Current Size,Wanted Size,Max Size,Streaming Type, Name") );
		for( INT TextureIndex=0; TextureIndex<Context.TextureStats.Num(); TextureIndex++ )
		{
			...
			INT LODBias				= TextureStat.Texture->GetCachedLODBias();
			INT CurrDroppedMips		= TextureStat.Texture->Mips.Num() - TextureStat.ResidentMips;
			INT WantedDroppedMips	= TextureStat.Texture->Mips.Num() - TextureStat.WantedMips;
			debugf( TEXT(",%i,%i,%i,%i,%i,%i,%i,%i,%i,%s,%s"),
				TextureStat.Texture->SizeX >> CurrDroppedMips,
				TextureStat.Texture->SizeY >> CurrDroppedMips,
				TextureStat.Texture->SizeX >> WantedDroppedMips,
				TextureStat.Texture->SizeY >> WantedDroppedMips,
				TextureStat.Texture->SizeX >> LODBias,
				TextureStat.Texture->SizeY >> LODBias,
				TextureStat.ResidentSize/1024,
				TextureStat.WantedSize/1024,
				TextureStat.MaxSize/1024,
				GStreamTypeNames[TextureStat.StreamType],
				*TextureStat.Texture->GetName()
				);
			TotalCurrentSize	+= TextureStat.ResidentSize;
			TotalWantedSize		+= TextureStat.WantedSize;
			TotalMaxSize		+= TextureStat.MaxSize;
		}
		debugf(TEXT("Total size: Current= %d  Wanted=%d  Max= %d"), TotalCurrentSize/1024, TotalWantedSize/1024, TotalMaxSize/1024 );
		bCollectTextureStats = FALSE;
	}
#endif
	...
	static UBOOL bSkipStreaming = FALSE;

	if ( bLimitedPoolSize && !bSkipStreaming )
	{
		...
		while ( HighPrioIndex < LowPrioIndex && TempMemoryUsed < MaxTempMemoryUsed )
		{
			...
			if ( HighPrioTexture.Texture != NULL )
			{
				...
				if ( HighPrioTexture.bInFlight && HighPrioTexture.RequestedMips < HighPrioTexture.ResidentMips && HighPrioTexture.RequestedMips < HighPrioTexture.WantedMips )
				{
					...
					{
						// Try to cancel.
						UBOOL bCancelled = HighPrioTexture.Texture->CancelPendingMipChangeRequest();
						if ( bCancelled )
						{
				...
				if ( HighPrioTexture.bInFlight == FALSE && HighPrioTexture.WantedMips > HighPrioTexture.ResidentMips )
				{
					...
					if ( StreamSize <= AvailableNow && TempMemoryUsed < MaxTempMemoryUsed )
					{
						...
						bStreamInTexture = TRUE;
					}
				}
			}

			// Do we need to stream something out?
			while ( AvailableLater < 0 && HighPrioIndex < LowPrioIndex && TempMemoryUsed < MaxTempMemoryUsed )
			{
				const FTexturePriority& TexturePriority = PrioritizedTextures( LowPrioIndex );
				FStreamingTexture& LowPrioTexture = StreamingTextures( TexturePriority.Value );

				// Check that this texture hasn't been marked for removal.
				if ( LowPrioTexture.Texture != NULL )
				{
					// Cancel load operation?
					if ( LowPrioTexture.bInFlight && LowPrioTexture.RequestedMips > LowPrioTexture.ResidentMips )
					{
						// Try to cancel.
						UBOOL bCancelled = LowPrioTexture.Texture->CancelPendingMipChangeRequest();
						if ( bCancelled )
						{
							INT CurrentSize = LowPrioTexture.GetSize( LowPrioTexture.ResidentMips );
							INT StreamSize = LowPrioTexture.GetSize( LowPrioTexture.RequestedMips ) - CurrentSize;
							AvailableLater += StreamSize;
						}
					}

					// Can we stream out this texture?
					if ( LowPrioTexture.bInFlight == FALSE && LowPrioTexture.ResidentMips > LowPrioTexture.MinAllowedMips )
					{
						//@TODO: Temp memory used=0 when using in-place reallocation.
						INT CurrentSize = LowPrioTexture.GetSize( LowPrioTexture.ResidentMips );
						INT StreamSize = CurrentSize - LowPrioTexture.GetSize( LowPrioTexture.MinAllowedMips );
						AvailableLater += StreamSize;
						TempMemoryUsed += CurrentSize;
						LowPrioTexture.WantedMips = LowPrioTexture.MinAllowedMips;
						BeginUpdateMipCount( LowPrioTexture, Context, TRUE );
					}
				}
			...
			if ( bStreamInTexture )
			{
				BeginUpdateMipCount( HighPrioTexture, Context, TRUE );
			}

			HighPrioIndex++;
		}
	}
	else if ( !bSkipStreaming )
	{
...
void FStreamingManagerTexture::StreamTexturesUnlimited( FStreamingContext& Context, const TArray<FTexturePriority>& PrioritizedTextures, INT TempMemoryUsed )
{
	for ( INT PrioIndex=0; PrioIndex < PrioritizedTextures.Num() && TempMemoryUsed < MaxTempMemoryUsed; ++PrioIndex )
	{
		...
		else if ( StreamingTexture.bInFlight && StreamingTexture.RequestedMips > StreamingTexture.ResidentMips && StreamingTexture.RequestedMips > StreamingTexture.WantedMips )
		{
			if (!GNeverStreamOutTextures)
			{
				StreamingTexture.Texture->CancelPendingMipChangeRequest();
			}
		}
		...
		else if ( StreamingTexture.bInFlight == FALSE && StreamingTexture.ResidentMips != StreamingTexture.WantedMips )
		{
			if (!GNeverStreamOutTextures || StreamingTexture.ResidentMips < StreamingTexture.WantedMips)
			{
				...
				TempMemoryUsed += CurrentSize;
				BeginUpdateMipCount( StreamingTexture, Context, TRUE );
			}
		}
	}
}
...
void FStreamingManagerTexture::NewUpdateResourceStreaming( FLOAT DeltaTime, UBOOL bProcessEverything/*=FALSE*/ )
{
	...
	// Start async task after the last data collection stage.
	if ( ProcessingStage == NumDataCollectionStages - 1 )
	{
 ...
 * @param TimeLimit		Optional time limit for processing, in seconds. Specifying 0 means infinite time limit.
 */
void FStreamingManagerTexture::BlockTillAllRequestsFinished( FLOAT TimeLimit /*= 0.0f*/ )
{
	...
	const INT MaxTexturesToProcess = UTexture2D::GetNumStreamableTextures();
	...
	do 
	{
		...
		NumPendingUpdates = 0;
		NumIterations++;
		for ( INT TextureCount = 0; CurrentStreamableLink && TextureCount < MaxTexturesToProcess; ++TextureCount )
		{
			UTexture2D* Texture = **CurrentStreamableLink;
			CurrentStreamableLink = CurrentStreamableLink->Next();
			if ( Texture->UpdateStreamingStatus() )
			{
				NumPendingUpdates++;
...
//	debugf( NAME_DevStreaming, TEXT("Block on streaming: %.1f ms (%d textures updated, %d iterations)"), ElapsedTime*1000.0f, MaxPendingUpdates, NumIterations );
}
...
void FStreamingManagerTexture::UpdateFrameStats( FStreamingTexture& StreamingTexture, FStreamingContext& Context )
{
	...
	if ( Context.bCollectTextureStats )
	{
		...
		{
			new (Context.TextureStats) FTextureStreamingStats( StreamingTexture.Texture, StreamType, StreamingTexture.ResidentMips, PerfectWantedMips, ResidentSize, PerfectWantedSize, PotentialSize );
		}
	}
...
{
	STAT_FAST(INT PerfectWantedMips = INDEX_NONE);
...
UBOOL FStreamingManagerTexture::ConditionallyCancelTextureStreaming( FStreamingTexture& StreamingTexture, FStreamingContext& Context )
{
	...
	if( bCanCancel && (StreamingTexture.RequestedMips != StreamingTexture.WantedMips) )
	{
		...
		{
			bCancelledStreaming = StreamingTexture.Texture->CancelPendingMipChangeRequest();
		}
 ...
 */
void FStreamingManagerTexture::BeginUpdateMipCount( FStreamingTexture& StreamingTexture, FStreamingContext& Context, UBOOL bIgnoreMemoryConstraints )
{
	UTexture2D* Texture = StreamingTexture.Texture;
						...
						for ( INT PartialIndex=0; PartialIndex < 4; ++PartialIndex )
						{
							if ( CenterX[PartialIndex] < 3.402823466e+30F )
							{
								...
								for( INT ViewIndex=0; ViewIndex < ThreadNumViews(); ViewIndex++ )
								{
									...
									FLOAT DistSqMinusRadiusSq = ClampMeshToCameraDistanceSquared(MinDistance * MinDistance);
...
void FStreamingManagerTexture::DumpTextureInstances( const UPrimitiveComponent* Primitive, FSpawnedPrimitiveData& PrimitiveData, UTexture2D* Texture2D )
{
	...
	for ( INT InstanceIndex=0; InstanceIndex < PrimitiveData.TextureInstances.Num(); ++InstanceIndex )
	{
		...
		if ( Instance.Texture2D == Texture2D )
		{
			...
			WantedMipCount = Clamp( WantedMipCount, StreamingTexture.MinAllowedMips, StreamingTexture.MaxAllowedMips );
			INT WantedMipIndex = Max(Texture2D->Mips.Num() - WantedMipCount, 0);
			debugf( TEXT("%s: Wanted=%dx%d, Distance=%.1f, TexelFactor=%.2f, CurrentRadius=%5.1f, OriginalRadius=%5.1f, Position=(%d,%d,%d), IsAttached=%d, Mesh=\"%s\""),
