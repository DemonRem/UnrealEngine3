...
void UAnimTree::DrawAnimNode( FCanvas* Canvas, const TArray<UAnimObject*>& SelectedNodes, UBOOL bShowWeight)
{
	...
	const FColor& BorderColor = bSelected ? SelectedColor : FColor(0, 0, 200);
	...
	if(ChildNode)
	{
		...
		const FColor& ChildNodeDrawColor = ( bSelected || SelectedNodes.ContainsItem( ChildNode ) ) ? SelectedColor : GetNodeColorFromWeight( 1.0f );
	...
	for(INT i=0; i<RootMorphNodes.Num(); i++)
	{
		...
		if(MorphNode)
		{
			...
			FLOAT Tension = Abs<INT>(Start.X - End.X);
			const FColor& MorphNodeDrawColor = ( bSelected || SelectedNodes.ContainsItem( MorphNode ) ) ? SelectedColor : FColor( 50, 50, 100 );
	...
	for(INT i=0; i<SkelControlLists.Num(); i++)
	{
		...
		if(Control)
		{
			...
			FLOAT Tension = Abs<INT>(Start.X - End.X);
			const FColor& LineColor = ( bSelected || SelectedNodes.ContainsItem( Control ) ) ? SelectedColor : SkelControlTitleColorDisabled;
...
void UAnimNodeBlendBase::DrawAnimNode( FCanvas* Canvas, const TArray<UAnimObject*>& SelectedNodes, UBOOL bShowWeight)
{
	...
	// Generate border color
	const UBOOL bSelected = SelectedNodes.ContainsItem( this );
	const FColor& BorderColor =  bSelected ? SelectedColor : GetNodeColorFromWeight( NodeTotalWeight );
	...
	for(INT i=0; i<Children.Num(); i++)
	{
		...
		if(ChildNode)
		{
			...
			FLOAT Tension = Abs<INT>(Start.X - End.X);
			const FColor& ChildNodeDrawColor = ( SelectedNodes.ContainsItem( ChildNode ) ) ? SelectedColor : GetNodeColorFromWeight( NodeTotalWeight * Children(i).Weight );
			FLinkedObjDrawUtils::DrawSpline(Canvas, End, -Tension * FVector2D(1,0), Start, -Tension * FVector2D(1,0), ChildNodeDrawColor, TRUE);
		}
	}
}
...
void USkelControlBase::DrawSkelControl(FCanvas* Canvas, const TArray<UAnimObject*>& SelectedNodes)
{
	...
	if( NextControl )
	{
		...
		const FColor& LineColor	= ( bSelected || SelectedNodes.ContainsItem( NextControl ) ) ? SelectedColor : SkelControlTitleColorDisabled;
