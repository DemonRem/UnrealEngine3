...
void UNavigationMeshBase::DrawMesh(FDebugRenderSceneProxy* DRSP, APylon* Pylon)
{
	...
	{
		FColor Color = FColor::MakeRedToGreenColorFromScalar((float)PolyIdx/float(Polys.Num()));
	...
	for( PolyList::TIterator It(BuildPolys.GetHead());It;++It)
	{
		...
		FColor Color = FColor::MakeRedToGreenColorFromScalar((float)Poly->Item/float(BuildPolys.Num()));
		Color.B = PointerHash(Pylon);
