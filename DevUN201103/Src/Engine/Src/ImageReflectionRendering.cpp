...
void FImageReflectionShaderParameters::Set(const FPixelShaderRHIRef& PixelShaderRHI, const FSceneView& View, FScene& Scene, const FReflectionPlanarShadowInfo* ShadowInfo) const
{
		...
		&& GRHIShaderPlatform == SP_PCD3D_SM5)
	{
		...
		const FLOAT CalculateVolumeShadowing = Scene.PrecomputedDistanceFieldVolumeTexture.TextureRHI && GSystemSettings.RenderThreadSettings.bAllowImageReflectionShadowing ? 1.0f : 0.0f;
...
UBOOL FSceneRenderer::RenderImageReflections(UINT DPGIndex)
{
	...
	if (DPGIndex == SDPG_World 
		&& GRHIShaderPlatform == SP_PCD3D_SM5 
		&& GSystemSettings.RenderThreadSettings.bAllowImageReflections
