	...
	SourceMesh(SrcMesh)
{
	...
	Indices.Empty(LODModel.DynamicIndexBuffer.Num());
	for(INT Index = 0;Index < LODModel.DynamicIndexBuffer.Num();Index++)
	{
		Indices.AddItem(LODModel.DynamicIndexBuffer.Get(Index));
	}
