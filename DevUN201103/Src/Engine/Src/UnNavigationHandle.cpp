...
void FNavMeshWorld::VerifyPathObjects()
{
	...
	for( APylon* ListPylon = GWorld->GetWorldInfo()->PylonList; ListPylon != NULL; ListPylon = ListPylon->NextPylon )
	{
		...
		if(Mesh != NULL)	
		{
			...
			// edges in submeshes
			TArray<FPolyObstacleInfo> Infos;
			Mesh->PolyObstacleInfoMap.GenerateValueArray(Infos);
			UNavigationMeshBase* SubMesh = NULL;
			for(INT Idx=0;Idx<Infos.Num();++Idx)
			{
				SubMesh = Infos(Idx).SubMesh;
				if(SubMesh != NULL)
				{
...
void APylon::OnPylonStatusChange()
{
	if( NavMeshPtr != NULL )
	{
		...
		PolysNeedingRebuild.Reset(PolysNeedingRebuild.Num());
		...
		for(INT Idx=0;Idx<NavMeshPtr->CrossPylonEdges.Num();++Idx)
		{
			...
			Query_Polys.Reset(Query_Polys.Num());
							...
							INT OverrideHeuristicCost/*=-1*/)
{
	...
	for( INT EdgeIdx = 0; EdgeIdx < CurrentPoly->GetNumEdges(); EdgeIdx++ )
	{
		STAT(EdgeIterations++);
		...
		FNavMeshEdgeBase* CurrentNeighborEdge = CurrentPoly->GetEdgeFromIdx(EdgeIdx);
		...
		checkSlowish(CurrentNeighborEdge->GetPoly0() != CurrentNeighborEdge->GetPoly1());
		FNavMeshPolyBase* CurrentNeighborPoly = CurrentNeighborEdge->GetOtherPoly(CurrentPoly);
		...
		{
			if( !ApplyConstraints( CurrentNeighborEdge, CurrentPoly, CurrentNeighborPoly, InitialCost, HeuristicCost, UsedEdgePoint ) )
			{
				continue; 
			}
		}
		...
		if( bIsOnClosed || bIsOnOpen )
		{
			...
			if( CurrentNeighborEdge->VisitedPathWeight <= InitialCost + PredecessorEdge->VisitedPathWeight )
			{
...
{
	static FLOAT Thresh = 0.06f;	
	#define PERP_THRESH Thresh
	const FVector thisEdgeMovePt = PreviousPosition;
	FVector nextEdgeMovePt(0.f);
	PointDistToSegment( thisEdgeMovePt, NextEdge->GetVertLocation(0), NextEdge->GetVertLocation(1), nextEdgeMovePt );

	const FVector TrajDir = (nextEdgeMovePt - thisEdgeMovePt).SafeNormal();


	const FVector ThisPerp = GetEdgePerpDir(WORLD_SPACE);
	const FVector NextPerp = NextEdge->GetEdgePerpDir(WORLD_SPACE);

	// if trajectory is close to perp to both edges.. we're good! let it through
	if( Abs<FLOAT>(ThisPerp | TrajDir) > PERP_THRESH && Abs<FLOAT>(NextPerp | TrajDir) > PERP_THRESH )
	{
		return TRUE;
	}

	
	// decide which direction to push out by entity width ( we want the direction toward the center of the poly	
	
	// get dir rom prev poly to current poly
	FNavMeshPolyBase* PrevPoly = GetOtherPoly(PolyToMoveThru);

	const FVector PrevTocurDir = (PolyToMoveThru->GetPolyCenter(WORLD_SPACE) - PrevPoly->GetPolyCenter(WORLD_SPACE)).SafeNormal();
	
	const FVector PolyNorm = PolyToMoveThru->GetPolyNormal(WORLD_SPACE);

	FVector TrajPerp = (TrajDir ^ PolyNorm).SafeNormal();
	
	if( (TrajPerp | PrevTocurDir) < 0.f)
	{
		TrajPerp *= -1.0f;
		checkSlowish((TrajPerp|PrevTocurDir) >= 0.f);
	}
	const FLOAT EntWidth = PathParams.SearchExtent.X*1.414f; // account for sqrt(2) off axis cases
	// need to offset by entity width from center of trajectory
	FVector start = thisEdgeMovePt + (TrajPerp*EntWidth);
	FVector end = nextEdgeMovePt + (TrajPerp*EntWidth);

	if (DoesTrajectoryLeavePoly(PolyToMoveThru,start,end))
	{
		VERBOSE_LOG_PATH_SUPPORTSMOVETOEDGEFAIL(PathParams.Interface,NextEdge,this,nextEdgeMovePt);
		return FALSE;
	}
...
UBOOL UNavigationHandle::GeneratePath( AActor** out_DestActor, INT* out_DestItem )
{
	...
	for(INT Idx=0;Idx<ConstraintProfileData.Num();++Idx)
	{
		const FName ConstraintName = ConstraintProfileData(Idx).ConstraintName;
	...
	const DWORD SamplingError = (CallTotal - RealTotal);
	debugf(TEXT("OVERALL TIME:  %3.3fms (%3.3fms sampling time) (per call %3.3fms avg, %3.3fms max), CallCount: %i")
		...
		,CallAvg * GSecondsPerCycle * 1000.f
		,CallMax * GSecondsPerCycle * 1000.f
		,CallCount
		);
	debugf(TEXT("-------------------------------------------------------------------------------------------------"));
#endif
 ...
 */
UBOOL UNavigationHandle::ApplyConstraints( FNavMeshEdgeBase* Edge, FNavMeshPolyBase* SrcPoly, FNavMeshPolyBase* DestPoly, INT& EdgeCost, INT& HeuristicCost, const FVector& EdgePoint )
{
...
#endif 
		if( !Constraint->EvaluatePath( Edge, SrcPoly, DestPoly, CachedPathParams, EdgeCost, HeuristicCost, EdgePoint ) )
		{
...
FString UNavigationHandle::GetPathCacheDebugText()
{
	FString Result;
	for( INT Idx = 0; Idx < PathCache.Num(); Idx++ )
	{
		FNavMeshEdgeBase* Edge = PathCache(Idx);
		Result = FString::Printf(TEXT("%s \n %s"),*Result, *Edge->GetDebugText());
	}
	Result = FString::Printf(TEXT("%s\n --- %i edges total ---"),*Result,PathCache.Num());
 ...
 */
#define CLOSE_THRESHOLD 5.0f
#define CLOSE_THRESHOLD_SQ 25.0f
												   ...
												   FLOAT ArrivalDistance)
{
	...
	FVector EdgeNorm = CurEdge->GetEdgeNormal();

	// remove 'up' component from the delt to get 2D values relative to this edge's polys
	FLOAT Dist2dSq = GetFlattenedDistanceBetweenVectsSq(CurHandlePos,out_MovePosition,EdgeNorm);
	...
	if(Dist2dSq < BumpThresh*BumpThresh )
	{
		...
		FLOAT Dist = PointDistToSegment(CurHandlePos,CurEdge->GetVertLocation(0),CurEdge->GetVertLocation(1),ClosestPt);

		// remove 'up' component of the delta
		Dist = GetFlattenedDistanceBetweenVects(ClosestPt,CurHandlePos,EdgeNorm);
		...
		FVector NewPos(0.f);
		FLOAT DistToNextSq = GetFlattenedDistanceBetweenVectsSq(NextMovePoint,CurHandlePos,EdgeNorm);
		...
		if(bGotValidDirection)
		{
			...
			FlattenVectAlongPassedAxis(Dir,EdgeNorm);
			Dir = Dir.SafeNormal();
		...
		if(!bGotValidDirection || !OtherPoly->IntersectsPoly2D(out_MovePosition,NewPos,EntryPt,ExitPt,WORLD_SPACE))
		{
			FVector EdgePerp = CurEdge->GetEdgePerpDir();

			checkSlowish(OtherPoly);
			...
			FVector Delta = ((OtherPoly!=NULL)?OtherPoly->GetPolyCenter():*FinalDestination) - ClosestPt;
	...
	else // then we were not really close to the next move point, so move to a compensated (bumped) destination as usual
	{
		...
		out_MovePosition += (out_MovePosition - CurHandlePos).SafeNormal() * BumpAmt;
		checkSlow(!out_MovePosition.ContainsNaN());
	}
}
...
UBOOL UNavigationHandle::ComputeValidFinalDestination(FVector& out_ComputedPosition)
{
	...
	PylonsToCheck.Reset(PylonsToCheck.Num());
...
UBOOL UNavigationHandle::HandleNotOnPath( FLOAT ArrivalDistance, FVector& out_Dest )
{
	...
	if(CurrentEdge != NULL && PathCache.Num() > 0)
	{
		...
		else if( DistFromEdge < Extent.X*1.5f )
		{
			...
			UBOOL bFoundPoly0Pos = (CurrentEdge->GetPoly0() && CurrentEdge->GetPoly0()->GetBestLocationForCyl(SearchStart,Extent.X,BestLocPoly0));
			UBOOL bFoundPoly1Pos = (CurrentEdge->GetPoly1() && CurrentEdge->GetPoly1()->GetBestLocationForCyl(SearchStart,Extent.X,BestLocPoly1));
		...
		// are too close to the wall
	{
		...
		else
		{
			...
			else
			{
				...
				if(!AnchorPoly->GetBestLocationForCyl(SearchStart,Extent.X,NewWalkLoc,TRUE))
				{
			...
			// if we're already close to adjust point, then just go toward finaldest
			if(Delta.SizeSquared() < 25.0f)
			{

				NewWalkLoc = *FinalDestination;
				checkSlow(!NewWalkLoc.ContainsNaN());
				Delta = NewWalkLoc-SearchStart;
				DeltaDir = Delta.SafeNormal();

				//GWorld->GetWorldInfo()->DrawDebugLine(NewWalkLoc,NewWalkLoc+FVector(0.f,0.f,100.f),255,128,0,TRUE);

				//GWorld->GetWorldInfo()->DrawDebugLine(SearchStart,NewWalkLoc,255,128,0,TRUE);
			}
...
UBOOL IsValidAnchorPos(UNavigationHandle* AskingHandle, const FVector& Point, FNavMeshPolyBase* PolyContainingPoint)
{
	if ( PolyContainingPoint != NULL )
	{
		return PolyContainingPoint->IsEscapableBy(AskingHandle->CachedPathParams);
	}
	return FALSE;
}
/** 
...
UBOOL UNavigationHandle::GetValidatedAnchorPosition(FVector& out_NewAnchorLoc, FVector OverrideStartLoc/*=FVector()*/)
{
	...
	GetValidPositionsForBoxEx(StartLoc,CachedPathParams.SearchExtent.X*4.0f,CachedPathParams.SearchExtent,TRUE,Points,1,0.f,CachedPathParams.SearchExtent,&IsValidAnchorPos);
...
UBOOL UNavigationHandle::GetPylonAndPolyFromPos(const FVector& Pos, FLOAT MinWalkableFloorZ, APylon*& out_Pylon, struct FNavMeshPolyBase*& out_Poly)
{
...
UBOOL UNavigationHandle::GetAllPolysFromPos( const FVector& Pos, const FVector& Extent, TArray<struct FNavMeshPolyBase*>& out_PolyList, UBOOL bIgnoreDynamic, UBOOL bReturnBothDynamicAndStatic  )
{
	FPylonOctreeType* Octree = FNavMeshWorld::GetPylonOctree();
...
// 						GWorld->GetWorldInfo()->DrawDebugCoordinateSystem(Hit.Location,FRotator::ZeroRotator,15.f,TRUE);
			...
			if( bValidTrajectoryToPosFromStart && UNavigationHandle::StaticObstaclePointCheck(Hit,FloorSnappedPos,Extent,NULL,&PylonsWeCareAbout))
			{
				...
				if( FitNessFunc==NULL || (*FitNessFunc)(Handle,FloorSnappedPos,HitPoly) )
				{
...
*/
	...
	// Crowds use the navmesh A LOT so it makes really hard to see anything from this debug logging so ignore them
// 	if( Cast<ACrowdAgentBase>(GetOuter()) == NULL )
// 	{
// 		warnf( TEXT("--Find Path: %s"), *GetOuter()->GetName() );
// 
// 		UNavMeshPathConstraint* Constraint = PathConstraintList;
// 		while( Constraint != NULL )
// 		{
// 			warnf( TEXT("  Constraint Name: %s"), *Constraint->GetName() );
// 			Constraint = Constraint->NextConstraint;
// 		}
// 
// 		UNavMeshPathGoalEvaluator* Goal = PathGoalList;
// 		while( Goal != NULL )
// 		{
// 			warnf( TEXT("  Goal Name: %s"), *Goal->GetName() );
// 			Goal = Goal->NextEvaluator;
// 		}
// 	}
	...
	// If world has no navigation points
	if( !GWorld->GetFirstNavigationPoint() || !CachedPathParams.bAbleToSearch )
	{
		VERBOSE_LOG_PATH_MESSAGE(this,TEXT("FINDPATH RETURNING FALSE! No navigation points in level, or bAbleToSearch is FALSE"))
		return FALSE;
	}
	...
	if(TotalTime >= PATH_PROFILING_MESH_THRESHOLD)
	{
		...
		debugf(TEXT(">> FindPath(OVERALL) took %3.3fms (%i nodes in path) for %s"),TotalTime,PathCache.Num(),*GetOuter()->GetName());
	}
#endif
...
FNavMeshPolyBase* GetAdjacentPolyContainingPoint(FNavMeshPolyBase* BasePoly, const FVector& Pt, const FVector& Extent,  const FVector& DesiredMoveDir)
{
static FLOAT Ep = 0.001f;
#define ADJACENT_EPSILON Ep
	TArray<FNavMeshPolyBase*> AdjacentPolys;
	BasePoly->GetAdjacentPolys(AdjacentPolys);
...
#define MAX_STACK 10
UBOOL NavMeshWalkingObstacleCheck( APawn* Pawn, UNavigationHandle* Handle, FCheckResult& Hit,const FVector& Start,const FVector& End, const FVector& Extent, const FVector& Floor, INT StackCount=0)
{
	FNavMeshPolyBase* HitPoly=NULL;
	UBOOL bTrajectoryClear = Handle->PointReachableLineCheck(Handle,Hit,Start,End,Extent,TRUE,&HitPoly);
	...
	if(!bTrajectoryClear && Handle->CurrentEdge != NULL && HitPoly != NULL)
	{
		FVector VertLoc0 = Handle->CurrentEdge->GetVertLocation(0);
		FVector VertLoc1 = Handle->CurrentEdge->GetVertLocation(1);
		FVector PolyNorm = HitPoly->GetPolyNormal();
	
		FVector IsectPt(0.f);
		if( StackCount < MAX_STACK && (FPointPlaneDist(VertLoc0,HitPoly->GetPolyCenter(),PolyNorm) < CLOSE_THRESHOLD || FPointPlaneDist(VertLoc1,HitPoly->GetPolyCenter(),PolyNorm) < CLOSE_THRESHOLD) )
		{ 
			FVector ProjPt = FPointPlaneProject(Hit.Location,HitPoly->GetPolyCenter(),PolyNorm);
			...
			if(ProjToEdge_T > -Edge_T_Tolerance && ProjToEdge_T < 1.f+Edge_T_Tolerance)
			{
				...
				{
					bTrajectoryClear=NavMeshWalkingObstacleCheck(Pawn,Handle,Hit,NewStart,End,Extent,Floor,StackCount+1);
				}
				else
				{
...
void NavMeshWalkingDropToFloor(const FVector& OldLoc, FVector& out_NewLoc, FNavMeshPolyBase* Poly, APawn* Pawn )
{
	Poly->AdjustPositionToDesiredHeightAbovePoly(out_NewLoc,Pawn->CylinderComponent->CollisionHeight);
}
...
void APawn::physNavMeshWalking(FLOAT deltaTime)
{
	...
	{
		if(!Handle->GetPylonAndPolyFromPos(Location,WalkableFloorZ,Py,CurrentPoly))
		{
 			Velocity=FVector(0.f);
	...
	/*
	while more desired move left
	CLIP:
	take  move and find where it leaves the current poly
	-if it doesn't leave the current poly then go to TEST
	-if it does leave current poly clip to poly exit point goto TEST
	TEST:
	obstacle linecheck from last position to current position
	-if no hit move there, done
	-if hit stop move (wall adjust?)
	*/
	...
	while((DistMoved+0.01f) < DesiredMag && (Iterations++ < 7))
	{
		...
		FVector HeightOffset = CylinderComponent->CollisionHeight * Floor;

		FLOAT NewMag = (ProjectedDesired - Location).Size();

		// CLIP against poly
		FVector ClipStart = Working_location-HeightOffset;
		FVector ClipEnd = ProjectedDesired-HeightOffset;
		FVector ExitPt(0.f),EntryPt(0.f);
		UBOOL bIntersect = CurrentPoly->IntersectsPoly2D(ClipStart,ClipEnd,EntryPt,ExitPt,WORLD_SPACE);

		NewMag = (ExitPt - ClipStart).Size();
		
		FNavMeshPolyBase* OldPoly = CurrentPoly;
		FVector OldPoly_Normal = OldPoly->GetPolyNormal();

		// if the delta is not in the current poly at all, or the clipped delta starts and ends at the same point, or we're trying to move away from the surface of the current poly
		// check to see if 
		UBOOL bMovingAwayFromSurface = Abs<FLOAT>((AccelDir | OldPoly_Normal)) > 0.707f;
		
		if(!bIntersect || NewMag < KINDA_SMALL_NUMBER)
		{			
			CurrentPoly = GetAdjacentPolyContainingPoint(CurrentPoly,ProjectedDesired,GetCylinderExtent(),AccelDir);
			check( CurrentPoly == NULL || CurrentPoly->NumObstaclesAffectingThisPoly==0 );


			if(CurrentPoly == NULL)
			{
				// if our new position is not quite in any of our adjacent polys, but containspoint reports true 
				// for the point then we're probably in betweeen two polys whose normals are facing away from each other
				// so keep moving! 
				if(OldPoly->ContainsPoint(ProjectedDesired,WORLD_SPACE))
				{
					CurrentPoly = OldPoly;
					check( CurrentPoly == NULL || CurrentPoly->NumObstaclesAffectingThisPoly==0 );
				...
				{
					// test current point against adjacent polys to see if we just need to hop to another one
					CurrentPoly = GetAdjacentPolyContainingPoint(OldPoly,OldLocation,GetCylinderExtent(),AccelDir);
					if( CurrentPoly == NULL )
					{
						// if we're still not inside something useful just ephing bail.. wtf mate?
		...
		else if(bMovingAwayFromSurface)
		{
			FNavMeshPolyBase* NewPoly = GetAdjacentPolyContainingPoint(CurrentPoly,ClipEnd,GetCylinderExtent(),AccelDir);

			if (NewPoly != NULL && NewPoly->ContainsBox(FBox::BuildAABB(Working_location,GetCylinderExtent()),WORLD_SPACE))
			{
				CurrentPoly = NewPoly;
				check( CurrentPoly->NumObstaclesAffectingThisPoly==0 );
		...
		{
			ProjectedDesired=ExitPt+HeightOffset;
		}
		...
		if(OldPoly != CurrentPoly)
		{
			...
			// back to start of loop to try again with new poly
			continue;
		}

		// TEST destination
		FCheckResult Hit(1.f);
		if(!NavMeshWalkingObstacleCheck(this,Handle,Hit,Working_location,ProjectedDesired,Extent,Floor))
		{
			...
			if( (NewDelta | Delta) >= 0.f )
			{
				...
				if (!NavMeshWalkingObstacleCheck(this,Handle,Hit,Working_location,Working_location+NewDelta,Extent,Floor))
				{
		...
		NavMeshWalkingDropToFloor(Working_location,ProjectedDesired,CurrentPoly,this);
	...
	{
		FVector Loc = Working_location;
		NavMeshWalkingDropToFloor(Working_location,Loc,CurrentPoly,this);
		Working_location=Loc;
		Mag = GroundSpeed*deltaTime;
	}

	GWorld->FarMoveActor(this,Working_location,FALSE,TRUE,TRUE);	
