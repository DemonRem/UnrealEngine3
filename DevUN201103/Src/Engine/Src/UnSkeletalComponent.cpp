...
void USkeletalMeshComponent::Attach()
{
	if( SkeletalMesh )
	{
			...
			(appGetPlatformType() & UE3::PLATFORM_WindowsServer) == 0)
		{
			...
			if(bIsCPUSkinned)
			{
				if ( !GIsEditor && SkeletalMesh->bUsePackedPosition )
				{
					// This is warning - if CPU skin is on, and if PackedPosition is on, warn this
					// I'm not turning the boolean off because that's not going to solve problem. 
					// It needs resaving of skeletalmeshes.
					warnf(TEXT("'%s' can't use PackedPosition due to CPU skin. Setting it to FALSE."), *SkeletalMesh->GetFullName());
				}
					...
					if( ControlAlpha > ZERO_ANIMWEIGHT_THRESH && (bTransformingAffectedBones || bScalingAffectedBones) )
					{
						...
						if (bTransformingAffectedBones)
						{
							...
							for(INT AffectedIdx=0; AffectedIdx<AffectedBones.Num(); AffectedIdx++)
							{
								...
								// The above code basically reverts any scale parent is trying to set 
								// FIXMELH: if you want parent to be 0.5, this code will calculate child to compensate the change (2), and make it to 1. 
								// so copy back original LocalAtoms value
								// for now I check out commented 
								//RelativeTransform.SetScale(LocalAtoms(AffectedIdx).GetScale());
						...
						// also set 0 scale for bonevisibility == 0
						for(INT AffectedIdx=0; AffectedIdx<AffectedBones.Num(); AffectedIdx++)
						{
							const INT AffectedBoneIndex	= AffectedBones(AffectedIdx);
							if(BoneVisibility(AffectedBoneIndex) == 0)
							{
...
// 	}
	...
	if( !bRecentlyRendered && LowUpdateFrameRate > 0 && ((TickTag + (INT)SkeletalMesh->SkelMeshRUID) % LowUpdateFrameRate > 0) )
	{
	...
	if( LowUpdateFrameRate > 0 && !bRecentlyRendered )
	{
...
void USkeletalMeshComponent::EnableAltBoneWeighting(UBOOL bEnable, INT LOD)
{
	if (bEnable)
	{
		...
		ToggleInstanceVertexWeights(ForcedLodModel-1,TRUE);
	}
	...
	else if ( GIsEditor )
	{
		...
		ToggleInstanceVertexWeights(ForcedLodModel-1,FALSE);
	}
}
