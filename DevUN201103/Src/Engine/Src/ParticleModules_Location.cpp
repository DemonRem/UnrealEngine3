		...
		if (SourceType == BONESOCKETSOURCE_Sockets)
		{
			if (InstancePayload->SourceComponent && InstancePayload->SourceComponent->SkeletalMesh)
			{
				...
				if (Socket)
				{
					...
					{
						if ((InstancePayload->SourceComponent->BoneVisibility(BoneIndex) == 0) || 
...
void UParticleModuleLocationSkelVertSurface::Spawn(FParticleEmitterInstance* Owner, INT Offset, FLOAT SpawnTime)
{
	...
	else if(SourceType == VERTSURFACESOURCE_Surface)
	{
		...
		if(SourceIndex != -1)
		{
			...
			VertIndex[0] = InstancePayload->SourceComponent->SkeletalMesh->LODModels(0).DynamicIndexBuffer.Get( SourceIndex );
			VertIndex[1] = InstancePayload->SourceComponent->SkeletalMesh->LODModels(0).DynamicIndexBuffer.Get( SourceIndex+1 );
			VertIndex[2] = InstancePayload->SourceComponent->SkeletalMesh->LODModels(0).DynamicIndexBuffer.Get( SourceIndex+2 );
...
void UParticleModuleLocationSkelVertSurface::UpdateBoneIndicesList(FParticleEmitterInstance* Owner)
{
	...
	if (Owner->Component->GetActorParameter(SkelMeshActorParamName, ActorInst) && (ActorInst != NULL))
	{
		...
		if ((SkeletalMeshActor != NULL) && (SkeletalMeshActor->SkeletalMeshComponent != NULL))
		{
			for (INT FindBoneIdx = 0; FindBoneIdx < ValidAssociatedBones.Num(); FindBoneIdx++)
			{
				INT BoneIdx = SkeletalMeshActor->SkeletalMeshComponent->SkeletalMesh->MatchRefBone(ValidAssociatedBones(FindBoneIdx));
	...
	FVector& OutPosition, FQuat* OutRotation, UBOOL bSpawning /* = FALSE*/)
{
	...
	if (InSkelMeshComponent->SkeletalMesh)
	{
		...
		else if (SourceType == VERTSURFACESOURCE_Surface)
		{
			...
			VertIndex[0] = InSkelMeshComponent->SkeletalMesh->LODModels(0).DynamicIndexBuffer.Get( InPrimaryVertexIndex );
			VertIndex[1] = InSkelMeshComponent->SkeletalMesh->LODModels(0).DynamicIndexBuffer.Get( InPrimaryVertexIndex+1 );
			VertIndex[2] = InSkelMeshComponent->SkeletalMesh->LODModels(0).DynamicIndexBuffer.Get( InPrimaryVertexIndex+2 );
