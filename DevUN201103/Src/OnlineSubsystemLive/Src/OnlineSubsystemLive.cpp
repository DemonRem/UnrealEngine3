...
void FSaveGameDataAsyncTask::OpenFile(void)
{
	...
	UBOOL bIsValid = SaveGame.bIsValid || IsContentValid();
	// Make sure the content if valid before opening the file
	if (bIsValid)
	{
		SaveGame.bIsValid = FIRST_BITFIELD;
 ...
 * @return true if the async read was started successfully, false otherwise
 */
UBOOL UOnlineSubsystemLive::GetSaveGameData(BYTE LocalUserNum,INT DeviceId,const FString& FriendlyName,const FString& FileName,const FString& SaveFileName,TArray<BYTE>& SaveGameData)
{
	BYTE Return = OERS_Failed;
	if (LocalUserNum >= 0 && LocalUserNum < MAX_LOCAL_PLAYERS)
	{
