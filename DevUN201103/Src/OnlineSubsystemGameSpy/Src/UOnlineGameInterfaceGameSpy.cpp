...
void UOnlineGameInterfaceGameSpy::TickInternetTasks(FLOAT DeltaTime)
{
	...
	if (SBHandle != NULL)
	{
		// Request server info for any pending ones
		for (INT Index = 0; Index < ServersPendingInfo.Num(); Index++)
		{
			ServerBrowserAuxUpdateServer(SBHandle, ServersPendingInfo(Index).Server, SBTrue, SBTrue);
		}
		ServersPendingInfo.Empty();
 ...
 */
DWORD UOnlineGameInterfaceGameSpy::ReadPlatformSpecificInternetSessionInfo(const FOnlineGameSearchResult& DesiredGame,BYTE PlatformSpecificInfo[64])
{
	DWORD Return = E_FAIL;
