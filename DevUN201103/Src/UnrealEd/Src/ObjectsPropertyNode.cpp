...
void FObjectPropertyNode::InitChildNodes(void)
{
	...
	for( INT i = 0; i < GetNumObjects(); ++i )
	{
		...
		if( TempObject )
		{
			ClassesToConsider.AddUniqueItem( TempObject->GetClass() );
		}
	}
	...
	for( TFieldIterator<UProperty> It(BaseClass); It; ++It )
	{
		...
		{
			if(TopPropertyWindow->HasFlags(EPropertyWindowFlags::ShouldShowNonEditable) || (It->HasAnyPropertyFlags(CPF_Edit) && bDMCOnlyTest && !bHidden) )
			{
