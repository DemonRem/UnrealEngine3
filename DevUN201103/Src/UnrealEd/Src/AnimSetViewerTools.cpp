...
void WxAnimSetViewer::SetSelectedAnimSequence(UAnimSequence* InAnimSeq)
{
	...
	if ( PreviewSkelComp )
	{
		for (INT i = 0; i < PreviewSkelComp->Attachments.Num(); i++)
		{
			UArrowComponent* ArrowComp = Cast<UArrowComponent>(PreviewSkelComp->Attachments(i).Component);
			if (ArrowComp != NULL )
			{
				PreviewSkelComp->DetachComponent(ArrowComp);
			}
		}
	}
	...
	{
		for (INT i = 0; i < PreviewSkelCompRaw->Attachments.Num(); i++)
		{
			UArrowComponent* ArrowComp = Cast<UArrowComponent>(PreviewSkelCompRaw->Attachments(i).Component);
			if (ArrowComp != NULL )
			{
				PreviewSkelCompRaw->DetachComponent(ArrowComp);
			}
		}
	}
...
void WxAnimSetViewer::ImportMeshLOD(USkeletalMesh* InSkeletalMesh, INT DesiredLOD)
{
	...
	// Shut down the skeletal mesh component that is previewing this mesh.
	{
		...
		FDynamicIndexBufferData Data;
		NewLODModel.DynamicIndexBuffer.GetIndexBufferData( Data );

		// Assign new FStaticLODModel to desired slot in selected skeletal mesh.
		SelectedSkelMesh->LODModels(DesiredLOD) = NewLODModel;

		SelectedSkelMesh->LODModels(DesiredLOD).DynamicIndexBuffer.RebuildIndexBuffer( Data );
	...
	{
		PreviewSkelComp->ToggleInstanceVertexWeights(LODIdx, FALSE);
	}
