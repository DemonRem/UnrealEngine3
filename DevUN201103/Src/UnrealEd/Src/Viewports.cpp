...
UBOOL FViewportConfig_Data::LoadFromINI()
{
	...
	// used to identify new show flags, so we can default those to have the new SHOW_DefaultEditor state
	EShowFlags SavedDefaultEditor = SHOW_DefaultEditor & ~(SHOW_DepthOfField | SHOW_ImageReflections | SHOW_SubsurfaceScattering | SHOW_LightFunctions | SHOW_Tesselation);
	{
 ...
 */
UBOOL FViewportConfig_Data::OpenNewFloatingViewport(const FFloatingViewportParams& InViewportParams, INT& OutViewportIndex )
{
...
BEGIN_EVENT_TABLE( WxLevelViewportWindow, wxWindow )
	...
	EVT_MENU_RANGE( ID_CAMSPEED_SLOW, ID_CAMSPEED_FAST, WxLevelViewportWindow::OnCameraSpeedSelection )
...
void WxLevelViewportWindow::OnCameraUIUpdate(wxUpdateUIEvent& Event)
{
	...
	const INT CurSpeedID = ( ( CameraSpeed == MOVEMENTSPEED_SLOW ) ? ID_CAMSPEED_SLOW : ( ( CameraSpeed == MOVEMENTSPEED_NORMAL ) ? ID_CAMSPEED_NORMAL : ID_CAMSPEED_FAST ) );
