...
bool WxUnrealEdApp::OnInit()
{
	...
	GCallbackEvent->Register(CALLBACK_GroupsHaveChanged,this);
...
/**
 * Update the given actor's visibility, since it's group membership changed
 */
void WxUnrealEdApp::CB_GroupsHaveChanged(AActor* Actor)
{
	FGroupUtils::UpdateActorAllViewsVisibility(Actor);
}
...
void WxUnrealEdApp::Send(ECallbackEventType InType, UObject* InObject)
{
	switch( InType )
	{
		...
		case CALLBACK_GroupsHaveChanged:
			// update the actor's per-view visibility
			CB_GroupsHaveChanged(Cast<AActor>(InObject));
			break;
