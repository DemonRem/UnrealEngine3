...
FString UCookPackagesCommandlet::GetPlatformPrefix()
{
	switch (Platform)
	{
		...
		default:
			return appPlatformTypeToString(Platform);
	}	
}
...
{
	FString CookedDir = FString(TEXT("Cooked")) + appPlatformTypeToString(Platform);

	if (bCoderMode)
	{
		CookedDir += TEXT("_Coder");
	}
...
//		debugf(TEXT("PersistentFaceFX> PMap alias being processed: %s"), *UpperMapName);
	...
	UBOOL bFastMode = FALSE;
	if (ParseParam(appCmdLine(), TEXT("EXPERIMENTAL")))
	{
		bFastMode = CallerCommandlet->IsA(UCookPackagesCommandlet::StaticClass()) && CastChecked<UCookPackagesCommandlet>(CallerCommandlet)->bIsMTChild;
	}
	if (bFastMode)
	{
...
void UCookPackagesCommandlet::CookTexture( UPackage* Package, UTexture2D* Texture2D, UBOOL bIsSavedInSeekFreePackage )
{
	...
	if( !Texture2D->IsTemplate( RF_ClassDefaultObject ) )
	{
		...
		if (Texture2D->Mips.Num() > 0)
		{
			...
			if (Platform == PLATFORM_IPhone || Platform == PLATFORM_NGP)
			{
			...
			if( Platform == PLATFORM_PS3 || Platform == PLATFORM_Xenon || (Platform & PLATFORM_Mobile))
			{
				// pack mips on the xenon
				UBOOL bPackMipTails = Platform == PLATFORM_Xenon;
				...
				// IPhone needs square compressed textuers
				// what about other mobile platforms using PVRTC?
				if ((GCookingTarget & (PLATFORM_IPhone | PLATFORM_NGP)) && Texture2D->Format >= PF_DXT1 && Texture2D->Format <= PF_DXT5)
				{
...
*/
/*void UCookPackagesCommandlet::CookSeqActPlayFaceFXAnim(USeqAct_AddRemoveFaceFXAnimSet* SeqAct_AddRemoveFaceFXAnimSet)
{
	if (PersistentFaceFXAnimSetGenerator.GetGeneratePersistentMapAnimSet() && PersistentFaceFXAnimSetGenerator.IsCurrentPMapCacheValid())
	{
		for ( INT I=0; I<SeqAct_AddRemoveFaceFXAnimSet->FaceFXAnimSets.Num(); ++I )
		{
			...
			{
				// Do not allow the FaceFXAnimSetRef to cook into anything!
				SeqAct_AddRemoveFaceFXAnimSet->FaceFXAnimSets(I)->ClearFlags(RF_ForceTagExp);
				SeqAct_AddRemoveFaceFXAnimSet->FaceFXAnimSets(I)->SetFlags(RF_MarkedByCooker|RF_Transient);

				if (PersistentFaceFXAnimSetGenerator.GetLogPersistentFaceFXGeneration())
				{
					warnf(NAME_Log, TEXT("CookSeqActPlayFaceFXAnim> Cleared FaceFXAnimSet %s for %s"),
						*(SeqAct_AddRemoveFaceFXAnimSet->FaceFXAnimSets(I)->GetPathName()),
						*(SeqAct_AddRemoveFaceFXAnimSet->GetPathName()));
				}
			}
			else
			{
...
void UCookPackagesCommandlet::CookSkeletalMesh( USkeletalMesh* SkeletalMesh )
{
	...
	if ((Platform == PLATFORM_PS3) || (Platform & PLATFORM_Mobile))
	{
		...
		for (INT LodIndex=0; LodIndex<NumLODs; ++LodIndex)
		{
			...
			NewIndexArray.Reserve(LODModels[LodIndex]->DynamicIndexBuffer.Num());
			...
			for (INT SectionIndex=0; SectionIndex<NumSections; ++SectionIndex)
			{
				...
				if (Platform == PLATFORM_PS3)
				{
					...
					if (Section.TriangleSorting == TRISORT_None)
					{
						...
						LODModels[LodIndex]->DynamicIndexBuffer.GetArray(Indices);
			...
			if (Platform == PLATFORM_PS3)
			{
				...
				if (NewIndexArray.Num() < LODModels[LodIndex]->DynamicIndexBuffer.Num())
				{
				...
				LODModels[LodIndex]->DynamicIndexBuffer.CopyArray(NewIndexArray);
			}
		} // End LOD Model
	}
}
...
void UCookPackagesCommandlet::GetMaterialInstanceReferencedTextureParams(UMaterialInstance* MatInst, TMap<FName,UTexture*>& RefdTextureParamsMap)
{
	if (MatInst->Parent != NULL)
	{
		for (INT MPIdx = 0; MPIdx < MP_MAX; MPIdx++)
		{
			...
			if (MatInst->Parent->GetTexturesInPropertyChain(MaterialProp, IgnoreTextures, &RefdParamNames) == TRUE)
			{
...
void UCookPackagesCommandlet::CleanupMaterials(UPackage* Package)
{
	...
	for (INT MaterialIdx = 0; MaterialIdx < FoundMaterials.Num(); MaterialIdx++)
	{
		...
		for (INT MPIdx = 0; MPIdx < MP_MAX; MPIdx++)
		{
			...
			if (Material->GetExpressionsInPropertyChain(MaterialProp, MPRefdExpressions) == TRUE)
			{
	...
	if (Platform & (PLATFORM_Console | PLATFORM_WindowsServer))
	{
		...
		if (!bIsMTChild)  // child processes don't need to do this and the files would clash
		{
			if (Platform & PLATFORM_Console)
			{
				...
				GConfig->CoalesceFilesFromDisk(*(appGameConfigDir() + GetConfigOutputDirectory()), ShouldByteSwapData(), PlatformEngineConfigFilename, *appPlatformTypeToString(Platform), bCookCurrentLanguageOnly | bCoderMode, MultilanguageCookingMask);
				if (bCoderMode)
				{
					// Copy them to the correct directory...
					FString NonCoderCookDir = appGameDir() + FString(TEXT("Cooked")) + appPlatformTypeToString(Platform);
					FString CoderCookDir = appGameDir() + GetTrueCookedDirectory();
					FString Wildcard = NonCoderCookDir * TEXT("*.bin");
					TArray<FString> Files;
					GFileManager->FindFiles(Files, *Wildcard, TRUE, FALSE);
	
					for (INT FileIndex = 0; FileIndex < Files.Num(); FileIndex++)
					{
						FFilename SrcFile = NonCoderCookDir * Files(FileIndex);
						FString DstFile = CoderCookDir * SrcFile.GetCleanFilename();
						GFileManager->Copy(*DstFile, *SrcFile, TRUE, TRUE);
					}
				}
...
UBOOL UCookPackagesCommandlet::Init( const TCHAR* Params, UBOOL& bQuitAfterInit )
{
	...
	// For PS3 mods . . .
	if (bIsInUserMode && (Platform == PLATFORM_PS3 || Platform == PLATFORM_Xenon || Platform == PLATFORM_WindowsConsole || Platform == PLATFORM_WindowsServer))
	{
	...
	if (!GIsBuildMachine)
	{
		if (Switches.FindItemIndex(TEXT("QUICK")) != INDEX_NONE )
		{
			bQuickMode = TRUE;
		...
		if ( !bForceFullRecook )
		{
			...
			if ( !bFileExists || bFileOutOfDate )
			{
				...
				if ( bIsInUserMode && (Platform == PLATFORM_PS3 || Platform == PLATFORM_Xenon || Platform == PLATFORM_WindowsConsole || Platform == PLATFORM_WindowsServer) )
				{
					...
					const FString ShippingCookedDir = appGameDir() + GetTrueCookedDirectory() + PATH_SEPARATOR;
	...
	for (INT Pass = 0; Pass < 2; Pass++)
	{
		for( INT PackageIndex = 0; PackageIndex < PackageList.Num(); PackageIndex++ )
		{
			...
			else if ( bIsCombinedStartupPackage )
			{
				...
				if (Pass == 0)
				{
					FString StartupPackageName = FString::Printf(TEXT("%s%s\\Startup.%s"), *appGameDir(), *GetCookedDirectory(), (Platform == PLATFORM_Windows ? TEXT("upk") : TEXT("xxx")));
...
//					warnf(NAME_Log,TEXT("pkg %s"),*PackageList(PackageIndex).DstFilename);
		...
		if (bIsMTChild)
		{
			...
			Temp.bIsCombinedStartupPackage = 0;
			...
			if (Package && (!bShouldOnlyLoad || !bQuickMode) )
			{
					...
					(Platform & UE3::PLATFORM_Console))
				{
					...
					if( Package->GetFName() != NAME_Core )
					{
						...
						if (GGameCookerHelper)
						{
							GGameCookerHelper->InitializeShaderCache(this, Package, ShaderCache, bIsNativeScriptFile || bIsCombinedStartupPackage);
						}
					}
				}
				...
				if ( !bStripEverythingButTextures && !bCoderMode )
				{
					// Clean up materials???
