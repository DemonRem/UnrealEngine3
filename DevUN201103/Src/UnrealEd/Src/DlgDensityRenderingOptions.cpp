...
void WxDlgDensityRenderingOptions::OnSliderChanged(wxScrollEvent& In)
{
	...
	if (Group)
	{
		FLOAT NewValue = (FLOAT)(In.GetPosition())/10.0f;
...
void WxDlgDensityRenderingOptions::CreateControlGroup(EControlGroup ControlGroup, wxSizer* OwningSizer)
{
	...
	INT InitValue = 0;
	INT MaxValue = 1000;	// 100.0f (1000 / 10.0f)
	...
	case DRO_IdealDensity:
		...
		InitValue = appTrunc(GEngine->IdealLightMapDensity * 10);
	...
	case DRO_MaximumDensity:
		...
		InitValue = appTrunc(GEngine->MaxLightMapDensity * 10);
		Group->DataPtr = &GEngine->MaxLightMapDensity;
		break;
	case DRO_ColorScale:
		ControlName = LocalizeUnrealEd("ColorScale");
		Group = &ColorScaleSlider;
		InitValue = appTrunc(GEngine->RenderLightMapDensityColorScale * 10);
		MaxValue = 100;	// 10.0f
	...
	case DRO_GrayscaleScale:
		...
		InitValue = appTrunc(GEngine->RenderLightMapDensityGrayscaleScale * 10);
		MaxValue = 100;	// 10.0f
	...
	if (Group != NULL)
	{
		FLOAT Value = (FLOAT)InitValue / 10.0f;
		TextCtrlString = FString::Printf(TEXT("%4.1f"), Value);
	...
	UBOOL bSetSlider, UBOOL bSetText)
{
	...
	if (Group == &MaximumDensitySlider)
	{
		...
		FLOAT IdealSetting = (FLOAT)IdealSettingInt / 10.0f;
		if (NewValue <= IdealSetting + 0.1f)
		{
			NewValue = IdealSetting + 0.1f;
		}
	}
	...
	*(Group->DataPtr) = NewValue;
	...
	INT NewValueInt = (INT)(NewValue * 10.0f);
	if (bSetSlider)
	{
		NewValueInt = Clamp<INT>(NewValueInt, Group->Slider->GetMin(), Group->Slider->GetMax());
		Group->Slider->SetValue(NewValueInt);
		NewValue = (FLOAT)NewValueInt / 10.0f;
	}

	if (bSetText)
	{
		Group->TextCtrl->SetValue(*FString::Printf(TEXT("%4.1f"), NewValue));
	}
	...
	if (Group == &IdealDensitySlider)
	{
		...
		FLOAT MaxSetting = (FLOAT)MaxSettingInt / 10.0f;
		if (NewValue >= MaxSetting - 0.1f)
		{
			MaxSetting = NewValue + 0.1f;
