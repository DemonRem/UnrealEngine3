 ...
 */
void FPropertyNode::SetExpanded   (const UBOOL bInExpanded, const UBOOL bInRecurse)
{
	...
	SetExpandedInternal(bInExpanded, bInRecurse);
	FastRefreshEntireTree();
}
 ...
 */
void FPropertyNode::SetExpandedItem( const FString& PropertyName, INT ArrayIndex, const UBOOL bInExpanded )
{
	//if we can't see the arrows, do not allow the state to be toggled
	check(TopPropertyWindow);
	if (!TopPropertyWindow->IsNormalExpansionAllowed(this))
	...
	SetExpandedItemInternal(PropertyName, ArrayIndex, bInExpanded);
	FastRefreshEntireTree();
}
...
void FPropertyNode::AppendVisibleWindows (OUT TArray<WxPropertyControl*>& OutVisibleArray)
{
	...
	{
		if (HasNodeFlags(EPropertyNodeFlags::IsSeen) && HasNodeFlags(EPropertyNodeFlags::ChildControlsCreated))
		{
 ...
 */
void FPropertyNode::SetExpandedInternal   (const UBOOL bInExpanded, const UBOOL bInRecurse)
{
	...
	if (bInRecurse)
	{
		for (INT scan = 0; scan < ChildNodes.Num(); ++scan)
		{
			FPropertyNode* ScanNode = ChildNodes(scan);
			check(ScanNode);
			ScanNode->SetExpandedInternal(bInExpanded, bInRecurse);
		}
	}
}
/**
 ...
 * If it's a UArrayProperty, the propery's ArrayIndex'th item is also expanded.
 */
void FPropertyNode::SetExpandedItemInternal (const FString& InPropertyName, INT InArrayIndex, const UBOOL bInExpanded)
{
	// If the name of the property stored here matches the input name . . .
	if( Property && InPropertyName == Property->GetName() )
	{
		UBOOL bRecurse = FALSE;
		SetExpandedInternal(bInExpanded, bRecurse);
		...
		{
			ChildNodes(InArrayIndex)->SetExpandedInternal(bInExpanded, bRecurse);
		}
	}
	else
	{
		...
		for(INT i=0; i<ChildNodes.Num(); i++)
		{
			ChildNodes(i)->SetExpandedItemInternal( InPropertyName, InArrayIndex, bInExpanded );
		}
	}
}
/*-----------------------------------------------------------------------------
 ...
 */
void WxPropertyControl::SanitizePropertyDisplayName( FString& InOutPropertyDisplayName, const UBOOL bIsBoolProperty )
{
	TArray<TCHAR> Chars;
