...
void WxInterpEd::AddTrackToCurveEd( UInterpGroup* InGroup, UInterpTrack* InTrack )
{
	...
	FString CurveName = FString::Printf( TEXT("%s_%s"), *InGroup->GroupName.ToString(), *TrackTitle);
...
void WxInterpEd::UpdateCameraToGroup(const UBOOL bInUpdateStandardViewports)
{
	...
	if (bInUpdateStandardViewports)
	{
		...
		for(INT i=0; i<GApp->EditorFrame->ViewportConfigData->GetViewportCount(); i++)
		{
			...
			if(LevelVC && LevelVC->ViewportType == LVT_Perspective && LevelVC->AllowMatineePreview() )
			{
				UpdateLevelViewport(DefaultViewedActor, LevelVC, FadeAmount, ColorScale, bEnableColorScaling);
			}
		}
	}
	...
	for(TMap<FEditorLevelViewportClient*, AActor*>::TIterator ViewportIt(ExtraViewports);ViewportIt;++ViewportIt)
	{
		...
		if(LevelVC && LevelVC->ViewportType == LVT_Perspective && LevelVC->AllowMatineePreview() )
		{
			...
			UpdateLevelViewport(ViewedActor, LevelVC, FadeAmount, ColorScale, bEnableColorScaling);
		}
	}
}
 ...
 */
void WxInterpEd::UpdateLevelViewport(AActor* InActor, FEditorLevelViewportClient* InViewportClient, const FLOAT InFadeAmount, const FVector& InColorScale, const UBOOL bInEnableColorScaling )
{
