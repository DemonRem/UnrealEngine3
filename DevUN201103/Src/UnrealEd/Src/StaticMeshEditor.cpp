...
#if WITH_ACTORX
			FileMenu->Append( IDM_SME_IMPORT_LIGHTMAP_MESH, *LocalizeUnrealEd("StaticMeshEditor_ImportLightmapMesh"), TEXT("") );
#else
			FileMenu->Append( IDM_SME_IMPORT_LIGHTMAP_MESH, *LocalizeUnrealEd("StaticMeshEditor_ImportLightmapMeshFBX"), TEXT("") );
#endif
			FileMenu->AppendSeparator();
...
void FStaticMeshEditorViewportClient::Draw(const FSceneView* View,FPrimitiveDrawInterface* PDI)
{
	...
	{
		FStaticMeshRenderData* RenderData = &StaticMeshEditor->StaticMesh->LODModels(0);
	...
	if (RenderData && UINT(StaticMeshEditor->StaticMesh->LightMapCoordinateIndex) < RenderData->VertexBuffer.GetNumTexCoords())
	{
		...
		if( StaticMeshEditor->SelectedEdgeIndices.Num() > 0 )
		{
				 ...
				 ++SelectionIt )
			{
				...
				FVector2D UV1( CurRawTriangle.UVs[ EdgeIndex % 3 ][ StaticMeshEditor->StaticMesh->LightMapCoordinateIndex ] );
				FVector2D UV2( CurRawTriangle.UVs[ ( EdgeIndex + 1 ) % 3 ][ StaticMeshEditor->StaticMesh->LightMapCoordinateIndex ] );
...
BEGIN_EVENT_TABLE( WxStaticMeshEditor, WxTrackableFrame )
	...
	EVT_MENU( IDM_SME_EXPORT_LIGHTMAP_MESH,  WxStaticMeshEditor::OnExportLightmapMesh )
	EVT_MENU( IDM_SME_IMPORT_LIGHTMAP_MESH,  WxStaticMeshEditor::OnImportLightmapMesh )
...
void WxStaticMeshEditor::OnExportLightmapMesh( wxCommandEvent& In )
{
	...
	if (PromptUserForDirectory(Directory, *LocalizeUnrealEd("StaticMeshEditor_ExportToPromptTitle"), *GApp->LastDir[LD_MESH_IMPORT_EXPORT]))
	{
		...
		UBOOL bAnyErrorOccurred = ExportMeshUtils::ExportAllLightmapModels(StaticMeshArray, Directory);
...
void WxStaticMeshEditor::OnImportLightmapMesh( wxCommandEvent& In )
{
	check(StaticMesh);		//must have a mesh to perform this operation
	...
	if (PromptUserForDirectory(Directory, *LocalizeUnrealEd("StaticMeshEditor_ImportToPromptTitle"), *GApp->LastDir[LD_MESH_IMPORT_EXPORT]))
	{
		...
		UBOOL bAnyErrorOccurred = ExportMeshUtils::ImportAllLightmapModels(StaticMeshArray, Directory);
		if (bAnyErrorOccurred) {
