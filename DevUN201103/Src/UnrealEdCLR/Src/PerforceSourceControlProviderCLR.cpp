		...
		if ( Records && Records->Records->Length > 0 )
		{
			...
			for ( INT RecordIndex = 0; RecordIndex < Records->Records->Length; ++RecordIndex )
			{
				...
				if( ClientRecord->Fields->ContainsKey("depotFile") )
				{
					...
					FullPath = FullPath.Right( FullPath.Len() - appStrlen( TEXT("//depot/") ) );	// remove "//depot/" from the start of the path
					FullPath = FullPath.Left( FullPath.Len() - appStrlen( TEXT("/...") ) );			// remove "/..." from the end of the path

					BranchName = FullPath;
					bBranchNameGood = TRUE;
				}
			}
		}
		if( !bBranchNameGood )
