...
namespace UnrealBuildTool
{
	class IntelToolChain
	{
		...
		public static string GetIntelBinDirectory()
		{
			return Path.Combine(
					Environment.GetEnvironmentVariable("ICPP_COMPILER10"),
					"IA32\\Bin\\"
					);
		}

		/** Accesses the lib directory for the Intel toolchain. */
		public static string GetIntelLibDirectory()
		{
			return Path.Combine(
			Environment.GetEnvironmentVariable("ICPP_COMPILER10"),
			"IA32\\Lib\\"
		...
		static string GetCLArguments_Global(CPPEnvironment CompileEnvironment)
		{
			...
			// Specify compatibility with Microsoft* Visual Studio 2005.
			Result += " /Qvc8";
		...
		public static CPPOutput CompileCPPFiles(CPPEnvironment CompileEnvironment, List<FileItem> SourceFiles)
		{
			...
			foreach (FileItem SourceFile in SourceFiles)
			{
				...
				CompileAction.CommandPath = Path.Combine(GetIntelBinDirectory(), "icl.exe");
		...
		public static FileItem LinkFiles(LinkEnvironment LinkEnvironment)
		{
			...
			LinkAction.CommandPath = Path.Combine(GetIntelBinDirectory(), "xilink.exe");
			...
			LinkEnvironment.LibraryPaths.Add(GetIntelLibDirectory());
