...
{
	Pitch = OriginatorInput->DeviceMotionAttitude.X * 32768.0f / PI;
	Yaw = OriginatorInput->DeviceMotionAttitude.Y * 32768.0f / PI;
	Roll = OriginatorInput->DeviceMotionAttitude.Z * 32768.0f / PI;

	DeltaPitch = OriginatorInput->DeviceMotionRotationRate.X * 32768.0f / PI;
	DeltaYaw = OriginatorInput->DeviceMotionRotationRate.Y * 32768.0f / PI;
	DeltaRoll = OriginatorInput->DeviceMotionRotationRate.Z * 32768.0f / PI;
...
void USeqEvent_MobileRawInput::InputTouch(APlayerController* Originator, UINT Handle, UINT InTouchpadIndex, BYTE Type, FVector2D TouchLocation, DOUBLE DeviceTimestamp)
{
	...
	INT ActiveIndex = ZoneEvent_MAX;

	// Setup the Active Index

	switch (Type)
	{
		case ZoneEvent_Touch : ActiveIndex = 0; break;
		case ZoneEvent_Update : ActiveIndex = 1; break;
		case ZoneEvent_UnTouch : ActiveIndex = 2; break;
		case ZoneEvent_Cancelled : ActiveIndex = 3; break;
	}
...
void USeqEvent_MobileSwipe::InputTouch(APlayerController* Originator, UINT Handle, UINT InTouchpadIndex, BYTE Type, FVector2D TouchLocation, DOUBLE DeviceTimestamp)
{
	...
	if (Type == ZoneEvent_UnTouch && TouchedActors.Num()>0)
	{
...
void USeqEvent_MobileObjectPicker::InputTouch(APlayerController* Originator, UINT Handle, UINT InTouchpadIndex, BYTE Type, FVector2D TouchLocation, DOUBLE DeviceTimestamp)
{
	...
	UGameViewportClient* GameViewportClient = GEngine->GameViewport;
	if ((bCheckonTouch || Type == ZoneEvent_UnTouch) && GameViewportClient != NULL)
	{
		// Default to Fail
		INT ActiveIndex = 1;	 
...
void USeqAct_MobileAddInputZones::Activated()
{
	...
	for (FLocalPlayerIterator It(GEngine); It; ++It)
	{
		...
		if (MobileInput)
		{
			...
			{
				GameViewportClient->GetViewportSize(ViewportSize);
				MobileInput->NativeInitializeZone(ZoneCopy, ViewportSize);
			}
