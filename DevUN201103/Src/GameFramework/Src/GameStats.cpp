...
void FWeaponEvents::AddWeaponIntEvent(INT EventID, struct FWeaponIntEvent* GameEventData, INT TimePeriod)
{
	...
	if (EventsByWeaponClass.IsValidIndex(GameEventData->WeaponClassIndex))
	{
		EventsByWeaponClass(GameEventData->WeaponClassIndex).AddEvent(EventID, GameEventData->Value, TimePeriod);
	}
}
...
{
	TotalEvents.AddEvent(EventID, GameEventData->Value, TimePeriod);
	if (EventsByProjectileClass.IsValidIndex(GameEventData->ProjectileClassIndex))
	{
		EventsByProjectileClass(GameEventData->ProjectileClassIndex).AddEvent(EventID, GameEventData->Value, TimePeriod);
	}
}
...
void FDamageEvents::AddKillEvent(INT EventID, INT KillTypeID, struct FPlayerKillDeathEvent* GameEventData, INT TimePeriod)
{
	...
	if (EventsByDamageClass.IsValidIndex(GameEventData->DamageClassIndex))
	{
		EventsByDamageClass(GameEventData->DamageClassIndex).AddEvent(UCONST_GAMEEVENT_AGGREGATED_DAMAGE_KILLS, 1, TimePeriod);
		EventsByDamageClass(GameEventData->DamageClassIndex).AddEvent(KillTypeID, 1, TimePeriod);
	}
}
...
void FDamageEvents::AddDeathEvent(INT EventID, INT KillTypeID, struct FPlayerKillDeathEvent* GameEventData, INT TimePeriod)
{
	...
	if (EventsByDamageClass.IsValidIndex(GameEventData->DamageClassIndex))
	{
		EventsByDamageClass(GameEventData->DamageClassIndex).AddEvent(UCONST_GAMEEVENT_AGGREGATED_DAMAGE_DEATHS, 1, TimePeriod);
		EventsByDamageClass(GameEventData->DamageClassIndex).AddEvent(KillTypeID, 1, TimePeriod);
	}
}
...
void FDamageEvents::AddDamageIntEvent(INT EventID, struct FDamageIntEvent* GameEventData, INT TimePeriod)
{
	...
	if (EventsByDamageClass.IsValidIndex(GameEventData->DamageClassIndex))
	{
		EventsByDamageClass(GameEventData->DamageClassIndex).AddEvent(EventID, GameEventData->Value, TimePeriod);
	}
}
...
void FPawnEvents::AddPlayerSpawnEvent(INT EventID, struct FPlayerSpawnEvent* GameEventData, INT TimePeriod)
{
	...
	if (EventsByPawnClass.IsValidIndex(GameEventData->PawnClassIndex))
	{
		EventsByPawnClass(GameEventData->PawnClassIndex).AddEvent(EventID, 1, TimePeriod);
	}
}
...
void UGameStatsAggregator::PreProcessStream()
{
	...
	{
		AllTeamEvents(TeamIter).WeaponEvents.EventsByWeaponClass.AddZeroed(NumWeaponClasses);
		AllTeamEvents(TeamIter).DamageAsPlayerEvents.EventsByDamageClass.AddZeroed(NumDamageClasses);
		AllTeamEvents(TeamIter).DamageAsTargetEvents.EventsByDamageClass.AddZeroed(NumDamageClasses);
		AllTeamEvents(TeamIter).ProjectileEvents.EventsByProjectileClass.AddZeroed(NumProjectileClasses);
		AllTeamEvents(TeamIter).PawnEvents.EventsByPawnClass.AddZeroed(NumPawnClasses);
	}

	AllPlayerEvents.AddZeroed(NumPlayers);
	for (INT PlayerIter=0; PlayerIter<NumPlayers; PlayerIter++)
	{
		AllPlayerEvents(PlayerIter).WeaponEvents.EventsByWeaponClass.AddZeroed(NumWeaponClasses);
		AllPlayerEvents(PlayerIter).DamageAsPlayerEvents.EventsByDamageClass.AddZeroed(NumDamageClasses);
		AllPlayerEvents(PlayerIter).DamageAsTargetEvents.EventsByDamageClass.AddZeroed(NumDamageClasses);
		AllPlayerEvents(PlayerIter).ProjectileEvents.EventsByProjectileClass.AddZeroed(NumProjectileClasses);
		AllPlayerEvents(PlayerIter).PawnEvents.EventsByPawnClass.AddZeroed(NumPawnClasses);
	}

	AllWeaponEvents.EventsByWeaponClass.AddZeroed(NumWeaponClasses);
	AllProjectileEvents.EventsByProjectileClass.AddZeroed(NumProjectileClasses);
	AllPawnEvents.EventsByPawnClass.AddZeroed(NumPawnClasses);
	AllDamageEvents.EventsByDamageClass.AddZeroed(NumDamageClasses);
