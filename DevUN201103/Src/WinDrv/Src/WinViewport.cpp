...
void FWindowsViewport::UpdateMouseLock()
{
	...
	UBOOL bShouldMouseLock = bHasFocus && (IsFullscreen() || !bIsAnyCursorVisible || bMouseLockRequested);
            ...
            if (GameViewportClient && GameViewportClient->ScaleformInteraction)
            {
                ...
                if (Movie && Movie->pMovie && Movie->pMovie->pView)
                {
                    GFxIMEWin32Event ev(GFxIMEWin32Event::IME_Default,
                        (UPInt)Window, Message, wParam, lParam, true);
                    UInt Result = Movie->pMovie->pView->HandleEvent(ev);
                    if (Result & GFxMovieView::HE_NoDefaultAction)
					{
                        return 0;
					}
                    else
    ...
    case WM_INPUTLANGCHANGE:
        if (ViewportClient)
        {
            ...
            if (GameViewportClient && GameViewportClient->ScaleformInteraction)
            {
                ...
                if (Movie && Movie->pMovie && Movie->pMovie->pView)
                {
                    GFxIMEWin32Event ev(GFxIMEWin32Event::IME_Default,(UPInt)Window, Message, wParam, lParam, true);
                    UInt Result = Movie->pMovie->pView->HandleEvent(ev);
                    if (Result & GFxMovieView::HE_NoDefaultAction)
					{
                        return 0;
	...
	case WM_XBUTTONDOWN:
		{
			...
			if (bFakeMobileTouches)
			{
				...
				{
					Input = Cast<UPlayerInput>(GEngine->GamePlayers(0)->Actor->PlayerInput);
					FVector2D Location;
					Location.X = FLOAT(GetMouseX());
					Location.Y = FLOAT(GetMouseY());
					Input->InputTouch(1,0, Location, appSeconds());
	...
	case WM_XBUTTONUP:
		{
			...
			if (bFakeMobileTouches)
			{
				...
				if (GEngine && GEngine->GamePlayers.Num() && GEngine->GamePlayers(0) && GEngine->GamePlayers(0)->Actor && GEngine->GamePlayers(0)->Actor->PlayerInput)
				{
					...
					Input->InputTouch(1,3, Location, appSeconds());
					bIsFakingMobileTouch = false;
				}
			}
#endif
...
void FWindowsViewport::Tick(FLOAT DeltaTime)
{
	...
	if (bFakeMobileTouches)
	{
		...
		if (GEngine && GEngine->GamePlayers.Num() && GEngine->GamePlayers(0) && GEngine->GamePlayers(0)->Actor && GEngine->GamePlayers(0)->Actor->PlayerInput)
		{
			...
			if (bIsFakingMobileTouch)
			{
				FVector2D Location;
				Location.X = FLOAT(GetMouseX());
				Location.Y = FLOAT(GetMouseY());
				Input->InputTouch(1,1, Location, appSeconds());
			}
		}
	}
#endif
