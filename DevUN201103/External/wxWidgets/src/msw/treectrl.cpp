        ...
        case NM_RCLICK:
            {
				...
				// @UE3 11-21-2008: fixed difference in behavior between tree controls which allow multi-selection and those that do not - 
				// tree ctrl with multi-select was generating context menu event when right-clicking on empty area of the tree ctrl (i.e. not on an item)
				// but single-select tree was not
				//if ( TreeView_HitTest(GetHwnd(), &tvhti) )
                if ( TreeView_HitTest(GetHwnd(), &tvhti) || !HasFlag(wxTR_MULTIPLE) )
                {
					//@{
					// @UE3 11-21-2008.
