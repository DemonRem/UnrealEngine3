...
public:
    ...
    UInt                    GenerateIdString(char* pbuffer, UPInt bufferSize =9, char suffixLetter = 0) const;
    ...
    {
    public:
        UPInt operator () (const GFxResourceId& idref)
        {
...
struct GFxResourceFileInfo : public GRefCountBaseNTS<GFxResourceFileInfo, GStat_Default_Mem>
{
...
class GFxResourceLibBase : public GRefCountBase<GFxResourceLibBase, GStat_Default_Mem>
{
public:
...
public:
    ...
    static inline UInt GSTDCALL MakeTypeCode(ResourceType rt, ResourceUse ru = Use_None)
    {
...
public:
    ...
    virtual GString         GetResourceName() const { return GString(); }
    virtual GMemoryHeap*    GetResourceHeap() const { return 0; }
...
class GFxResourceLib : public GRefCountBase<GFxResourceLib, GStat_Default_Mem>
{
    friend class GFxResourceWeakLib;    
    ...
    {    
        ...
        static UPInt    GSTDCALL GetCachedHash(const C& data)                { return (((UPInt)data) >> 6) ^ ((UPInt)data); }
        static void     GSTDCALL SetCachedHash(C& data, UPInt hashValue)     { GUNUSED2(data, hashValue); }
        static C&       GSTDCALL GetValue(C& data)                           { return data; }
        static const C& GSTDCALL GetValue(const C& data)                     { return data; }
    };

    typedef GHashSetUncached<GFxResource*, GFxResourcePtrHashFunc> PinHashSet;
...
public:
    ...
    GFxResourceLib(bool debug = false);
    ~GFxResourceLib();
   ...
   // GFxResource Slots: Allocated in a separate list during a lock
    ...
    class ResourceSlot : public GRefCountBase<ResourceSlot, GStat_Default_Mem>
    {
    public:
        enum ResolveState
        {
    ...
    inline void             GetResourceArray(GArray<GPtr<GFxResource> > *presources);

    inline void             UnpinAll();

    // Obtain heap that should be used for image allocations with this resource library.
    inline GMemoryHeap*     GetImageHeap() const;
...
class GFxResourceWeakLib : public GFxResourceLibBase
{
    ...
    struct ResourceNode
    {
        ...
        {                
            ...
            UPInt  operator() (const GFxResourceKey& k) const
            {
                return GFxResourceKey::HashOp()(k);
            }
        };
...
public:
    ...
    void            GetResourceArray(GArray<GPtr<GFxResource> > *presources);

    // Obtain heap that should be used for image allocations with this resource library.
#ifdef GFX_USE_IMAGE_HEAPS
    GMemoryHeap*    GetImageHeap() const { return pImageHeap.GetPtr(); }    
#else
    GMemoryHeap*    GetImageHeap() const { return GMemory::GetGlobalHeap(); }
#endif
...
inline void GFxResourceLib::GetResourceArray(GArray<GPtr<GFxResource> > *presources)
{
...
inline GMemoryHeap* GFxResourceLib::GetImageHeap() const
{
