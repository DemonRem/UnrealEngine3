...
{
#if defined(GFC_NO_THREADSUPPORT) || defined(GFC_CPU_X86) || defined(GFC_OS_WIN32) || defined(GFC_OS_XBOX) || defined(GFC_OS_IPHONE)
...
struct GAtomicOpsRaw_4ByteImpl : public GAtomicOpsRawBase
{
    ...
    static inline UInt32   ExchangeAdd_NoSync(volatile UInt32 *i, UInt32 j)
    {
        ...
        asm volatile("1:\n\t"
                     ...
                     "beq    %[o],$0,1b\n\t"
                     "nop    \n"
                     : "+m" (*i), [r] "=&d" (ret), [o] "=&d" (dummy) : [i] "d" (i), [j] "d" (j) : "cc", "memory");
...
struct GAtomicOpsRaw_DefImpl : public O
{
    ...
    // "GAtomicOpsRaw_DefImpl<O>::" prefix in calls below.
    inline static O_T   Exchange_Sync(volatile O_T* p, O_T val)             { O_FullSync sync; GUNUSED(sync);    return GAtomicOpsRaw_DefImpl<O>::Exchange_NoSync(p, val); }
    inline static O_T   Exchange_Release(volatile O_T* p, O_T val)          { O_ReleaseSync sync; GUNUSED(sync); return GAtomicOpsRaw_DefImpl<O>::Exchange_NoSync(p, val); }
    inline static O_T   Exchange_Acquire(volatile O_T* p, O_T val)          { O_AcquireSync sync; GUNUSED(sync); return GAtomicOpsRaw_DefImpl<O>::Exchange_NoSync(p, val); }  
    inline static O_T   ExchangeAdd_Sync(volatile O_T* p, O_T val)          { O_FullSync sync; GUNUSED(sync);    return GAtomicOpsRaw_DefImpl<O>::ExchangeAdd_NoSync(p, val); }
    inline static O_T   ExchangeAdd_Release(volatile O_T* p, O_T val)       { O_ReleaseSync sync; GUNUSED(sync); return GAtomicOpsRaw_DefImpl<O>::ExchangeAdd_NoSync(p, val); }
    inline static O_T   ExchangeAdd_Acquire(volatile O_T* p, O_T val)       { O_AcquireSync sync; GUNUSED(sync); return GAtomicOpsRaw_DefImpl<O>::ExchangeAdd_NoSync(p, val); }
    inline static bool  CompareAndSet_Sync(volatile O_T* p, O_T c, O_T val)    { O_FullSync sync; GUNUSED(sync);    return GAtomicOpsRaw_DefImpl<O>::CompareAndSet_NoSync(p,c,val); }
    inline static bool  CompareAndSet_Release(volatile O_T* p, O_T c, O_T val) { O_ReleaseSync sync; GUNUSED(sync); return GAtomicOpsRaw_DefImpl<O>::CompareAndSet_NoSync(p,c,val); }
    inline static bool  CompareAndSet_Acquire(volatile O_T* p, O_T c, O_T val) { O_AcquireSync sync; GUNUSED(sync); return GAtomicOpsRaw_DefImpl<O>::CompareAndSet_NoSync(p,c,val); }
...
public:
    GLock (UInt dummy = 0)  { sceKernelCreateLwMutex(&mutex, "GLock", SCE_KERNEL_LWMA_RECURSIVE | SCE_KERNEL_LWMA_THPRI, 0, NULL); }
    ~GLock()                { sceKernelDeleteLwMutex(&mutex); }
    inline void Lock()      { sceKernelLockLwMutex(&mutex, 1, NULL); }
    inline void Unlock()    { sceKernelUnlockLwMutex(&mutex, 1); }
...
{
public:
    GLockSafe(UInt spinCount = 0) : mLock(spinCount)
    ...
    { 
        mLock.Unlock();
#ifdef GFC_BUILD_DEBUG
        GASSERT(LockCount.ExchangeAdd_NoSync(-1) > 0);
#endif
    }
