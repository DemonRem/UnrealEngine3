 ...
 */
function AppendButton(string ButtonTextMarkup, delegate<UIObject.OnClicked> ButtonDelegate)
{
	local int ButtonIdx;

	for(ButtonIdx=0; ButtonIdx<UTUIBUTTONBAR_MAX_BUTTONS; ButtonIdx++)
	{
		if(Buttons[ButtonIdx].IsVisible()==false)
		{
...
{
	assert(ButtonIndex<UTUIBUTTONBAR_MAX_BUTTONS);
