...
function PreProcessInput( float DeltaTime )
{
	...
	//`log( "aTurn: " $ aTurn $ " DeltaTime: " $ DeltaTime );
}
...
function ApplyViewAutoPitchCentering( float DeltaTime )
{
	...
	if( (WorldInfo.TimeSeconds - LastTurnTime) > AutoCenterDelay )
	{
...
function ApplyViewAutoVehiclePitchCentering( float DeltaTime )
{
	...
	if ( (UTVehicle(Pawn) != None) && !UTVehicle(Pawn).bFollowLookDir )
	{
		...
		else if ( WorldInfo.TimeSeconds - LastTurnTime > AutoCenterDelay )
		{
			...
			if ( UTVehicle(Pawn).bStickDeflectionThrottle && !bLookToSteer )
			{
...
function ApplyViewAcceleration( float DeltaTime )
{
		...
		)
	{
		...
		if( ( ViewAccel_TimeHeld > ViewAccel_TimeToHoldBeforeFastAcceleration )
			...
			&& ( !Pawn.IsFiring() ) // if we are shooting then don't do super speed up (e.g. we are prob circle straffing)
			)
		{
			...
			//`log( "aTurn *=2 ViewAccel_CurrMutliplier: " $ ViewAccel_CurrMutliplier $ " aTurn: " $ aTurn $ " ViewAccel_BaseMultiplier: " $ ViewAccel_BaseMultiplier );
		}
		else
		{
...
function ApplyTargetFriction( float DeltaTime, UTWeapon W )
{
	...
	if( FrictionTarget != None )
	{
			...
			)
		{
			...
			//`log( "FrictionMultiplier: " $ FrictionMultiplier );
...
exec function SmartDodge()
{
	...
	//`log( "I am smart dodging!!!!" );
	// dodge cancels feign death
	P = UTPawn(Pawn);
	...
	else
	{
		...
		{
			//`log( "   SmartJump back " $ RawJoyRight );
			ForcedDoubleClick = DCLICK_Back;
		}
  		else
  		{
  			//`log( "   SmartJump front " $ RawJoyRight );
