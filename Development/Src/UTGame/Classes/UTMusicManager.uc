...
function StartMusic()
{
	...
	if (UMI != None && UMI.MapMusicInfo != None)
	{
		...
		CheckCrossFadeMeasures(UMI.MapMusicInfo.MapMusic.Intro, "INTRO");
		CurrFadeFactor = MusicVolume/UMI.MapMusicInfo.MapMusic.Intro.CrossfadeToMeNumMeasuresDuration;
	}
}

// @TODO FIXMESTEVE REMOVE BEFORE SHIP
function CheckCrossFadeMeasures(MusicSegment ASegment, string SegmentString)
{
	if ( ASegment.CrossfadeToMeNumMeasuresDuration < 1 )
	{
		`warn("Bad crossfade duration for "$SegmentString$" TTP to Mike Larson with map");
		ASegment.CrossfadeToMeNumMeasuresDuration = 1;
	}
}
...
function IntroFinished(AudioComponent AC)
{
	...
	if (AC == MusicTracks[5] && CurrentTrack == AC && CurrentState == MST_Ambient)
	{
		...
		if (MusicTracks[0] != None)
		{
			...
			MusicStartTime = WorldInfo.TimeSeconds;
			CurrTempo = UMI.MapMusicInfo.MapMusic.Tempo;
			CheckCrossFadeMeasures(UMI.MapMusicInfo.MapMusic.Ambient, "AMBIENT");
...
function ChangeTrack(EMusicState NewState)
{
	...
	{
		case MST_Ambient:
			if ( MusicTracks[0] == None ) MusicTracks[0] = CreateNewTrack(UMI.MapMusicInfo.MapMusic.Ambient.TheCue);
			...
			break;
		case MST_Tension:
			if ( MusicTracks[1] == None ) MusicTracks[1] = CreateNewTrack(UMI.MapMusicInfo.MapMusic.Tension.TheCue);
			NewTrack = MusicTracks[1];
			CurrTempo = UMI.MapMusicInfo.MapMusic.Tempo;
			CheckCrossFadeMeasures(UMI.MapMusicInfo.MapMusic.Tension, "TENSION");
			CurrFadeFactor = MusicVolume/UMI.MapMusicInfo.MapMusic.Tension.CrossfadeToMeNumMeasuresDuration;
			break;
		case MST_Suspense:
			if ( MusicTracks[2] == None ) MusicTracks[2] = CreateNewTrack(UMI.MapMusicInfo.MapMusic.Suspense.TheCue);
			NewTrack = MusicTracks[2];
			CurrTempo = UMI.MapMusicInfo.MapMusic.Tempo;
			CheckCrossFadeMeasures(UMI.MapMusicInfo.MapMusic.Suspense, "SUSPENSE");
			CurrFadeFactor = MusicVolume/UMI.MapMusicInfo.MapMusic.Suspense.CrossfadeToMeNumMeasuresDuration;
			break;
		case MST_Action:
			if ( MusicTracks[3] == None ) MusicTracks[3] = CreateNewTrack(UMI.MapMusicInfo.MapMusic.Action.TheCue);
			NewTrack = MusicTracks[3];
			CurrTempo = UMI.MapMusicInfo.MapMusic.Tempo;
			CheckCrossFadeMeasures(UMI.MapMusicInfo.MapMusic.Action, "ACTION");
			CurrFadeFactor = MusicVolume/UMI.MapMusicInfo.MapMusic.Action.CrossfadeToMeNumMeasuresDuration;
			break;
		case MST_Victory:
			if ( MusicTracks[4] == None ) MusicTracks[4] = CreateNewTrack(UMI.MapMusicInfo.MapMusic.Victory.TheCue);
			NewTrack = MusicTracks[4];
			CurrTempo = UMI.MapMusicInfo.MapMusic.Tempo;
			CheckCrossFadeMeasures(UMI.MapMusicInfo.MapMusic.Victory, "VICTORY");
