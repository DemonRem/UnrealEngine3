...
simulated function bool AddInventory( Inventory NewItem, optional bool bDoNotActivate )
{
	...
	if (Role == ROLE_Authority)
	{
		...
		if (bResult && UTWeapon(NewItem) != None)
		{
			...
			if ( UTWeapon(Instigator.Weapon) == None ||
					(!bDoNotActivate &&	bAutoSwitchWeaponOnPickup &&
						Instigator != None && PlayerController(Instigator.Controller) != None &&
						UTWeapon(Instigator.Weapon).ShouldSwitchTo(UTWeapon(NewItem))) )
			{
				UTWeapon(NewItem).ClientWeaponSet(true);
			}
		}
	}
...
simulated function ChangedWeapon()
{
	...
	Wep = UTWeapon(Pawn(Owner).Weapon);
...
simulated function SwitchToBestWeapon( optional bool bForceADifferentWeapon )
{
	...
	if (BestWeapon != None)
	{
		BestWeapon.ClientWeaponSet(PlayerController(Instigator.Controller) != None);
	}
}
