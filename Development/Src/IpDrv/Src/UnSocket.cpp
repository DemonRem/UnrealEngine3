...
UBOOL FSocket::SendTo(const BYTE* Data,INT Count,INT& BytesSent,const FInternetIpAddr& Destination)
{
	...
	debugfSuppressed( NAME_DevNetTraffic, TEXT("Socket '%s' SendTo %i Bytes"), *SocketDescription, BytesSent );
	return TRUE;
}
...
UBOOL FSocket::Send(const BYTE* Data,INT Count,INT& BytesSent)
{
	...
	debugfSuppressed( NAME_DevNetTraffic, TEXT("Socket '%s' Send %i Bytes"), *SocketDescription, BytesSent );
	return TRUE;
}
...
UBOOL FSocket::RecvFrom(BYTE* Data,INT BufferSize,INT& BytesRead,FInternetIpAddr& Source)
{
	if( BytesRead > 0 )
	{
		debugfSuppressed( NAME_DevNetTraffic, TEXT("Socket '%s' RecvFrom %i Bytes"), *SocketDescription, BytesRead );
	}
	return TRUE;
}
...
UBOOL FSocket::Recv(BYTE* Data,INT BufferSize,INT& BytesRead)
{
	if( BytesRead > 0 )
	{
		debugfSuppressed( NAME_DevNetTraffic, TEXT("Socket '%s' Recv %i Bytes"), *SocketDescription, BytesRead );
	}
	return TRUE;
}
