...
#if !XBOX && !WITH_PANORAMA // PC socket subsystem creation/destruction support
 ...
 * @return TRUE if successful, FALSE otherwise
 */
UBOOL FSocketWin::Accept(FInternetIpAddr& OutAddr)
{
	INT SizeOf = sizeof(SOCKADDR_IN);
	return accept(Socket,OutAddr,&SizeOf) == 0;
}
...
FSocket* FSocketSubsystemWindows::CreateStreamSocket(void)
{
	...
	return Socket != INVALID_SOCKET ? new FSocketWin(Socket,SOCKTYPE_Streaming) : NULL;
}
