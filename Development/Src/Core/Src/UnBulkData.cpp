...
ECompressionFlags FUntypedBulkData::GetDecompressionFlags() const
{
	...
	return
		...
		COMPRESS_None;
#else	//#if WITH_LZO
	checkf(((BulkDataFlags & BULKDATA_SerializeCompressedLZO) == 0), TEXT("BulkData compressed with LZO"));
...
#else	//#if WITH_LZO
			Ar.SerializeCompressed( Data, GetBulkDataSize(), COMPRESS_ZLIB );
#endif	//#if WITH_LZO
		}
...
#else	//#if WITH_LZO
				Ar.SerializeCompressed( SerializedData.GetData(), SerializedData.Num(), COMPRESS_ZLIB);
#endif	//#if WITH_LZO
