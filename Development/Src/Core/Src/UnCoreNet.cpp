...
,	ForcedExportBasePackageName(NAME_None)
{
	...
	if (Package != NULL)
	{
		...
		if (GPackageFileCache->FindPackageFile(*Package->GetName(), NULL, PackageFile, NULL, FALSE))
		{
...
/**
 * Add the standalone seekfree package wrapper package to the package map if it exists
 *
 * @param PackageName Name of the package for which to look for the wrapper
 */
void UPackageMap::AddStandaloneSeekfreePackage(FName PackageName)
{
	// check to see if there's a standalone seekfree package for this package
	FFilename StandalonePath;
	if (GPackageFileCache->FindPackageFile(*(PackageName.ToString() + STANDALONE_SEEKFREE_SUFFIX), NULL, StandalonePath, NULL, FALSE))
	{
		FPackageInfo* StandaloneInfo = new(List) FPackageInfo(NULL);

		// grab info out of the package
		FArchive* PackageReader = GFileManager->CreateFileReader(*StandalonePath);
		...
		*PackageReader << Summary;
		...
		// fill out the "fake" package map entry
		StandaloneInfo->PackageName = FName(*(PackageName.ToString() + STANDALONE_SEEKFREE_SUFFIX));
		StandaloneInfo->Guid = Summary.Guid;
		// mark this package to download only, never open it as a real package (since the FindPackageFile redirection will take care of it)
		StandaloneInfo->PackageFlags = Summary.PackageFlags | PKG_DownloadButNoLoad;
		// @todo: Is this the right thing to do here?
		StandaloneInfo->LocalGeneration = Summary.Generations.Num();

		// close the file
		delete PackageReader;

		debugf(TEXT("Added a fake package map entry for standalone seekfree package %s [%s]"), *StandaloneInfo->PackageName.ToString(), *StandaloneInfo->Guid.String() );
	}
}
...
void UPackageMap::AddNetPackages()
{
	...
	{
		FPackageInfo* Info = new(List) FPackageInfo(NetPackages(i));
		
		// attempt to add a standalone seekfree version
		AddStandaloneSeekfreePackage(Info->PackageName);
	}
