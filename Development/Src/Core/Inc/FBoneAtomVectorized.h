...
public:
	...
	*/
	FORCEINLINE FMatrix ToMatrix() const
	{
		...
		const VectorRegister DiagonalsXYZ = VectorSubtract(Scale, DiagonalSum);
...
#if XBOX
#define VectorPermute(Vec1, Vec2, Mask) __vperm(Vec1, Vec2, Mask)
#else
...
#define VectorPermuteMask(x,y,z,w) \
		...
		// Permute masks needed to get the rotation+scaling matrix elements back in the right location
		static const VectorRegister Ax_Ay_Bx_Bz = VectorPermuteMask(0, 1, 4, 6);
		static const VectorRegister Az_By = VectorPermuteMask(2, 5, 0, 0);
		static const VectorRegister Ax_Bx_Bw_Aw = VectorPermuteMask(0, 4, 7, 3);
		static const VectorRegister Bz_Ay_By_Aw = VectorPermuteMask(6, 1, 5, 3);
		static const VectorRegister Bx_By_Az_Aw = VectorPermuteMask(4, 5, 2, 3);
