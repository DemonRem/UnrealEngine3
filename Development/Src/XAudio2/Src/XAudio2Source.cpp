...
UBOOL FXAudio2SoundSource::Init( FWaveInstance* InWaveInstance )
{
	...
	if( Buffer && Buffer->NumChannels > 0 )
	{
		...
		// Set whether to apply radio distortion
		SetRadioDistortionApplied();
	...
	case 1:
		{
			...
			if( bRadioDistortionApplied && WaveInstance->RadioFilterVolumeThreshold - AttenuatedVolume > KINDA_SMALL_NUMBER )
			{
...
INT FXAudio2SoundSource::GetDestinationVoiceIndexForEffect( SourceDestinations Effect )
{
	...
	case DEST_RADIO:
		...
		Index = bRadioDistortionApplied ? ( bReverbApplied ? DEST_RADIO : DEST_REVERB ) : -1;
		break;
