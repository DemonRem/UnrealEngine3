...
var color HitBeamColor;
var color HighHitBeamColor;
...
simulated function PostBeginPlay()
{
	...
	if (WorldInfo.NetMode != NM_DedicatedServer && Mesh != None)
	{
		...
		if (PoweredUpEffect != None && SkeletalMeshComponent(Mesh) != None)
		{
...
simulated function UpdateBeam(byte FireModeNum)
{
	...
	{
		BeamColor = HighHitBeamColor;
		BeamSystem = HighPowerSystem;
		BeamEndpointTemplate = HighPowerBeamEndpointTemplate;
		MuzzleFlashTemplate = HighPowerMuzzleFlashTemplate;
	}
	else if (FireModeNum == 5)
	{
		BeamColor = HitBeamColor;
		BeamSystem = HighPowerSystem;
		BeamEndpointTemplate = HighPowerBeamEndpointTemplate;
		MuzzleFlashTemplate = HighPowerMuzzleFlashTemplate;
	}
...
defaultproperties
{
	...
	HitBeamColor=(R=128,G=255,B=128,A=255)
	HighHitBeamColor=(R=255,G=255,B=96,A=255)
