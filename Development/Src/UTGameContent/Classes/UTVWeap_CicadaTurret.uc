...
simulated function ActiveRenderOverlays(HUD H)
{
	...
	for (i = 0; i < Missiles.length; i++)
	{
		if (Missiles[i] == None || Missiles[i].bDeleteMe)
		{
			Missiles.Remove(i, 1);
			i--;
		}
		...
		{
			Pos = H.Canvas.Project(Missiles[i].Location);
			H.Canvas.SetPos(Pos.X - 16.0, Pos.Y - 16.0);
			H.Canvas.DrawBox(32.0, 32.0);
		}
	}
}
...
defaultproperties
{
	...
	FireTriggerTags=(TurretWeapon01,TurretWeapon02)

	DefaultImpactEffect=(ParticleTemplate=ParticleSystem'VH_Cicada.Effects.P_VH_Cicada_2ndPrim_impact',Sound=SoundCue'A_Weapon.ShockRifle.Cue.A_Weapon_SR_AltFireImpact_Cue')
	BotRefireRate=0.99
	bSplashJump=false
	bSplashDamage=false
