...
QWORD FD3D9BufferedGPUTiming::GetTiming()
{
	if ( GIsSupported )
	{
		...
		if ( NumIssuedTimestamps > 0 )
		{
			...
			D3DResult = D3DRHI->GetQueryData( EndTimestamps[TimestampIndex], &EndTime, sizeof(EndTime), bBlocking );

			if ( D3DResult == S_OK )
			{
				D3DRHI->GetQueryData( StartTimestamps[TimestampIndex], &StartTime, sizeof(EndTime), bBlocking );
