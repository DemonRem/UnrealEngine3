...
/*
	...
	{
		`log("Ignoring restrictions for connections to "$self@bIsBeyondSpawnDistance);
	        for ( i=0; i< NextDestinations.Length; i++ )
	        {
		        if ( (NextDestinations[i] != None) && NextDestinations[i].AllowableDestinationFor(Agent) )
			{
				`log("BUT acceptable "$NextDestinations[i]);
			}
			else
			{
				`log(NextDestinations[i]$" restricted:  beyond distance "
						$NextDestinations[i].bIsBeyondSpawnDistance
						$" NavMesh "$NextDestinations[i].bHasNavigationMesh
						$" Enabled "$NextDestinations[i].bIsEnabled
						$" Behavior "$((Agent.CurrentBehavior != None) ? Agent.CurrentBehavior.AllowThisDestination(NextDestinations[i]) : (NextDestinations[i].bAllowAsPreviousDestination || (Agent.PreviousDestination != NextDestinations[i])))
						$" Capacity "$((Agent.MyGroup != None) ? (NextDestinations[i].CustomerCount + Agent.MyGroup.Members.Length <= NextDestinations[i].Capacity) : (NextDestinations[i].CustomerCount < NextDestinations[i].Capacity) || ((NextDestinations[i].QueueHead !=None) && NextDestinations[i].QueueHead.HasSpace()))
						$" Tagged "$NextDestinations[i].InteractionTag
						$" Restrictions "$NextDestinations[i].bHasRestrictions);
			}
		}
	}
*/
	...
	for ( i=0; i<NextDestinations.Length; i++ )
	{
		if ( (NextDestinations[i] != None) && NextDestinations[i].bHasNavigationMesh && (bIgnoreRestrictions || NextDestinations[i].bLastAllowableResult) )
		{
			// bonus to this potential destination's frequency if current destination is not visible, and destination is, and agent prefers visible destinations
			DestinationFrequencySum += NextDestinations[i].Frequency * ((!bIsVisible && Agent.bPreferVisibleDestination && (NextDestinations[i].bIsVisible || NextDestinations[i].bWillBeVisible)) ? 2.0 : 1.0);
	...
	// if ( (Agent.CurrentDestination != None) && Agent.CurrentDestination.bIsBeyondSpawnDistance ) `log("Picked dest from "$self@bIsBeyondSpawnDistance$" beyond spawndist ignoring restrictions "$bIgnoreRestrictions);
	Agent.PreviousDestination = self;
}
