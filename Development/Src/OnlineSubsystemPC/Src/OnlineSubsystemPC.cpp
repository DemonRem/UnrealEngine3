...
UBOOL UOnlineSubsystemPC::DoesProfileExist(void)
{
	...
	DWORD Attribs = GetFileAttributes(*CreateProfileName());
	return Attribs != -1 &&
		(Attribs & FILE_ATTRIBUTE_DEVICE &&
	...
	UOnlineProfileSettings* ProfileSettings)
{
	...
	if (CachedProfile == NULL)
	{
		...
		{
//@todo joeg -- Make async once the async i/o system supports saves
			...
			if (DoesProfileExist())
			{
				...
				{
					FProfileSettingsReader Reader(TRUE,Buffer.GetTypedData(),Buffer.Num());
					...
					if (Reader.SerializeFromBuffer(CachedProfile->ProfileSettings))
					{
						...
						CachedProfile->AsyncState = OPAS_None;
						Return = S_OK;
					}
					else
					{
...
//@todo joeg -- Make async once the async i/o system supports saves
			...
			FProfileSettingsWriter Writer(TRUE);
