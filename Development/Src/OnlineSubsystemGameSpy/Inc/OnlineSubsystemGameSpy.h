...
#include "pt/pt.h"
...
#if EPIC_INTERNAL
	#define WANTS_GAMESPY_PRODUCTION_URLS 0
	...
	#endif
#else
	// Licensees should default to these URLs unless directed by GameSpy to use something else
	#define WANTS_GAMESPY_PRODUCTION_URLS 1
#endif

#ifndef _GAMESPY_URLS_DEFINED
	#if WANTS_GAMESPY_PRODUCTION_URLS
	// These URLs go to the live system (not staging, real deal)
	...
	#else
	// Wants staging URLs not production
		#define SAKEI_SOAP_URL_FORMAT		"http://%s.sake." GSI_DOMAIN_NAME "/SakeStorageServerDev/StorageServer.asmx"
		#define SC_SERVICE_URL_FORMAT		"http://mwstage." GSI_DOMAIN_NAME "/CompetitionService/CompetitionService.asmx"
	#endif
