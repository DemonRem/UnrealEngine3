...
FLightRayIntersection FStaticLightingAggregateMesh::IntersectLightRay(const FLightRay& LightRay,UBOOL bFindClosestIntersection,FCoherentRayCache& CoherentRayCache) const
{
	...
	for(MeshOctreeType::TConstIterator OctreeIt(MeshOctree);OctreeIt.HasPendingNodes();OctreeIt.Advance())
	{
		const MeshOctreeType::FNode& CurrentNode = OctreeIt.GetCurrentNode();
		...
		if(!CurrentNode.IsLeaf())
		{
			FOREACH_OCTREE_CHILD_NODE(ChildX,ChildY,ChildZ)
			{
				...
				if(CurrentNode.HasChild(ChildRef))
				{
					...
					if(FLineBoxIntersection(ChildContext.BoundingBox,ClippedLightRay.Start,ClippedLightRay.End,ClippedLightRay.Direction,ClippedLightRay.OneOverDirection))
					{
		...
		const TArray<FMeshAndBounds>& Meshes = CurrentNode.GetElements();
...
FStaticLightingAggregateMesh::FStaticLightingAggregateMesh():
	MeshOctree(FBox(FVector(-HALF_WORLD_MAX,-HALF_WORLD_MAX,-HALF_WORLD_MAX),FVector(+HALF_WORLD_MAX,+HALF_WORLD_MAX,+HALF_WORLD_MAX)))
{}
