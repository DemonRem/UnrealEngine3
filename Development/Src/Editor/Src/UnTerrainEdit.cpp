		...
		for (INT Y = InMinY - StepSize; Y <= InMaxY + StepSize; Y += StepSize)
		{
			...
			for (INT X = InMinX - StepSize; X <= InMaxX + StepSize; X += StepSize)
			{
				...
				else
				{
					...
					AlphaValue[0] = Terrain->Alpha(DecoLayer ? Terrain->DecoLayers(LayerIndex).AlphaMapIndex : Terrain->Layers(LayerIndex).AlphaMapIndex,X,Y);
					AlphaValue[1] = Terrain->Alpha(DecoLayer ? Terrain->DecoLayers(LayerIndex).AlphaMapIndex : Terrain->Layers(LayerIndex).AlphaMapIndex,X + StepSize,Y);
					AlphaValue[2] = Terrain->Alpha(DecoLayer ? Terrain->DecoLayers(LayerIndex).AlphaMapIndex : Terrain->Layers(LayerIndex).AlphaMapIndex,X,Y + StepSize);
					AlphaValue[3] = Terrain->Alpha(DecoLayer ? Terrain->DecoLayers(LayerIndex).AlphaMapIndex : Terrain->Layers(LayerIndex).AlphaMapIndex,X + StepSize,Y + StepSize);
					...
					for (INT Sub = 1; Sub < StepSize; Sub++)
					{
						...
						BYTE& Alpha = Terrain->Alpha(DecoLayer ? Terrain->DecoLayers(LayerIndex).AlphaMapIndex : Terrain->Layers(LayerIndex).AlphaMapIndex,X+Sub,Y);
						Alpha = NewAlpha;
						MirrorX = X + Sub;
						if (bMirrorY)
						{
						...
						if (bMirrorX || bMirrorY)
						{
							Alpha = Terrain->Alpha(DecoLayer ? Terrain->DecoLayers(LayerIndex).AlphaMapIndex : Terrain->Layers(LayerIndex).AlphaMapIndex,MirrorX,MirrorY);
							Alpha = NewAlpha;
						}
						// Top
						TempAlpha = (WORD)(((FLOAT)AlphaValue[2] * (1.0f - Interp)) + ((FLOAT)AlphaValue[3] * Interp));
						NewAlpha = (BYTE)Clamp<WORD>(TempAlpha, 0, MAXBYTE);
						Alpha = Terrain->Alpha(DecoLayer ? Terrain->DecoLayers(LayerIndex).AlphaMapIndex : Terrain->Layers(LayerIndex).AlphaMapIndex,X+Sub,Y+StepSize);
						Alpha = NewAlpha;
						MirrorX = X + Sub;
						if (bMirrorY)
						{
					...
					for (INT Sub = 1; Sub < StepSize; Sub++)
					{
						...
						NewAlpha = (BYTE)Clamp<WORD>(TempAlpha, 0, MAXBYTE);
						BYTE& Alpha = Terrain->Alpha(DecoLayer ? Terrain->DecoLayers(LayerIndex).AlphaMapIndex : Terrain->Layers(LayerIndex).AlphaMapIndex,X,Y+Sub);
						Alpha = NewAlpha;
						MirrorX = X;
						if (bMirrorY)
						{
						...
						NewAlpha = (BYTE)Clamp<WORD>(TempAlpha, 0, MAXBYTE);
						Alpha = Terrain->Alpha(DecoLayer ? Terrain->DecoLayers(LayerIndex).AlphaMapIndex : Terrain->Layers(LayerIndex).AlphaMapIndex,X+StepSize,Y+Sub);
						Alpha = NewAlpha;
						MirrorX = X + StepSize;
						if (bMirrorY)
						{
					...
					for (INT Sub2 = 1; Sub2 < StepSize; Sub2++)
					{
						AlphaValue[0] = Terrain->Alpha(DecoLayer ? Terrain->DecoLayers(LayerIndex).AlphaMapIndex : Terrain->Layers(LayerIndex).AlphaMapIndex,X+Sub2,Y);
						AlphaValue[1] = Terrain->Alpha(DecoLayer ? Terrain->DecoLayers(LayerIndex).AlphaMapIndex : Terrain->Layers(LayerIndex).AlphaMapIndex,X+Sub2,Y+StepSize);
						for (INT Sub = 1; Sub < StepSize; Sub++)
						{
							Interp = (FLOAT)(Sub % StepSize) / (FLOAT)StepSize;
							...
							TempAlpha = (WORD)(((FLOAT)AlphaValue[0] * (1.0f - Interp)) + ((FLOAT)AlphaValue[1] * Interp));
							NewAlpha = (BYTE)Clamp<WORD>(TempAlpha, 0, MAXBYTE);
							BYTE& Alpha = Terrain->Alpha(DecoLayer ? Terrain->DecoLayers(LayerIndex).AlphaMapIndex : Terrain->Layers(LayerIndex).AlphaMapIndex,X+Sub2,Y+Sub);
							Alpha = NewAlpha;
							MirrorX = X + Sub2;
							if (bMirrorY)
							{
...
UBOOL FModeTool_TerrainSplitX::Render(ATerrain* Terrain, const FVector HitLocation, const FSceneView* View, FViewport* Viewport, FPrimitiveDrawInterface* PDI)
{
...
UBOOL FModeTool_TerrainSplitY::Render(ATerrain* Terrain, const FVector HitLocation, const FSceneView* View, FViewport* Viewport, FPrimitiveDrawInterface* PDI)
{
...
UBOOL FModeTool_TerrainMerge::Render(ATerrain* Terrain, const FVector HitLocation, const FSceneView* View, FViewport* Viewport, FPrimitiveDrawInterface* PDI)
{
	FEdModeTerrainEditing*  Mode = (FEdModeTerrainEditing*)GEditorModeTools().GetCurrentMode();
	for (INT i=0;i<Mode->ToolTerrains.Num();i++)
	{
...
UBOOL FModeTool_TerrainAddRemoveSectors::Render(ATerrain* Terrain, const FVector HitLocation, const FSceneView* View, FViewport* Viewport, FPrimitiveDrawInterface* PDI )
{
	if (abs(Direction) < KINDA_SMALL_NUMBER)
	{
		return FALSE;
...
{
	UBOOL bResult = FModeTool_Terrain::MouseMove(ViewportClient, Viewport, x, y);

	if ((bMouseLeftPressed == TRUE) && (bCtrlIsPressed == TRUE) && (bAltIsPressed == TRUE))
	{
		debugf(TEXT("MouseMove: MLB = %d - CTRL = %d - ALT = %d - Amount %3d,%3d"), 
			bMouseLeftPressed, bCtrlIsPressed, bAltIsPressed, x, y);
	}
	...
	UBOOL bMirrorX, UBOOL bMirrorY)
{
	if ((bCtrlIsPressed == TRUE) && (bAltIsPressed == FALSE))
	{
		...
		{
			debugf(TEXT("VertexEdit: BeginApplyTool - Select vertices..."));
			bSelectVertices = TRUE;
		}
		else
		if (bMouseRightPressed)
		{
			debugf(TEXT("VertexEdit: BeginApplyTool - Deselect vertices..."));
	...
	if (bCtrlIsPressed == TRUE)
	{
		...
		if (bAltIsPressed == TRUE)
		{
			if (MouseYDelta != 0)
			{
				...
				FLOAT Moved = (FLOAT)(MouseYDelta) * (Settings->Strength / 100.0f) * Settings->ScaleFactor;
				for (INT VertIndex = 0; VertIndex < Terrain->SelectedVertices.Num(); VertIndex++)
				{
					...
					WORD& Height = Terrain->Height(SelVert->X, SelVert->Y);
					INT NewHeight = Height + appTrunc(Amount);
					Terrain->Height(SelVert->X, SelVert->Y) = (WORD)Clamp<INT>(NewHeight, 0, MAXWORD);
		...
		if ((bSelectVertices == TRUE) || (bDeselectVertices == TRUE))
		{
			...
			for (INT Y = MinY; Y <= MaxY; Y += StepSize)
			{
				...
				for (INT X = MinX; X <= MaxX; X += StepSize)
				{
					...
					INT SetWeight = (INT)(100.0f * CalculatedStrength);
					SetWeight = Clamp<INT>(SetWeight, 0, 100);
...
{
	debugf(TEXT("VertexEdit: EndApplyTool!"));
	bSelectVertices = FALSE;
	bDeselectVertices = FALSE;
}

UBOOL FModeTool_TerrainVertexEdit::Render(ATerrain* Terrain, const FVector HitLocation, const FSceneView* View, FViewport* Viewport, FPrimitiveDrawInterface* PDI )
{
	...
	for (TObjectIterator<ATerrain> It; It; ++It)
	{
		...
		if (LocalTerrain->SelectedVertices.Num() > 0)
		{
			for (INT Index = 0; Index < LocalTerrain->SelectedVertices.Num(); Index++)
			{
				...
				EdMode->DrawBallAndStick( View, Viewport, PDI, LocalTerrain, Vertex, (FLOAT)(SelVert.Weight) / 100.0f );
			}
		}
	}
	return TRUE;
