...
UBOOL CImporter::ImportFromText(const FString& Filename, const TCHAR* FileStart, const TCHAR* FileEnd)
{
	...
	const FUStatus ParsingStatus = ColladaDocument->LoadFromText(FilePath, FileText.c_str());

	// Retrieve the error message and buffer it for the engine
	bParsingSuccessful = ParsingStatus.IsSuccessful();
	ErrorMessage = ParsingStatus.GetErrorString();
...
UBOOL CImporter::ImportFromFile(const TCHAR* Filename)
{
	...
	delete ColladaDocument;
	ColladaDocument = FCollada::NewTopDocument();
	FUStatus ParsingStatus = ColladaDocument->LoadFromFile(Filename);
...
void CImporter::PostImport()
{
	...
	{
		bInvertUpAxis = !IsEquivalent(ColladaDocument->GetUpAxis(), FMVector3::ZAxis);
	}
}
...
inline UBOOL _HasUnknownCameras(FCDSceneNode* ColladaSceneNode, const TCHAR* Name)
{
	...
	for ( size_t InstanceIndex = 0; InstanceIndex < ColladaInstanceCount; ++InstanceIndex )
	{
		...
		if ( ColladaInstance->GetEntityType() == FCDEntity::CAMERA )
		{
			...
			if ( Actor == NULL || Actor->bDeleteMe ) return TRUE;
			else
			{
...
UBOOL CImporter::HasUnknownCameras() const
{
	if ( !bParsingSuccessful || ColladaDocument == NULL )
		return FALSE;
	...
	for ( size_t NodeIndex = 0; NodeIndex < ColladaSceneNodeCount; ++NodeIndex )
	{
		...
		if ( _HasUnknownCameras( ColladaSceneNode, ColladaSceneNode->GetName().c_str() ) )
			return TRUE;
		...
		for ( size_t ChildIndex = 0; ChildIndex < ColladaChildCount; ++ChildIndex )
		{
			...
			if ( _HasUnknownCameras( ColladaChildNode, ColladaSceneNode->GetName().c_str() ) )
				return TRUE;
		}
	}
...
void CImporter::ImportMatineeSequence(USeqAct_Interp* MatineeSequence)
{
	...
	for (size_t NodeIndex = 0; NodeIndex < ColladaSceneNodeCount; ++NodeIndex)
	{
		...
		AActor* Actor = FindObject<AActor>( ANY_PACKAGE, ColladaSceneNode->GetName().c_str() );
...
{
	// Import the default, animatable camera properties
	...
	FCDENode* DepthOfFieldNode = ColladaCamera->GetExtra()->FindRootNode(DAEFC_CAMERA_DEPTH_OF_FIELD_ELEMENT);
	FCDENode* MotionBlurNode = ColladaCamera->GetExtra()->FindRootNode(DAEMAX_CAMERA_MOTIONBLUR_ELEMENT);
	if (DepthOfFieldNode != NULL)
	{
		...
		{
			ImportAnimatedProperty(&Actor->CamOverridePostProcess.DOF_FocusDistance, TEXT("CamOverridePostProcess.DOF_FocusDistance"), MatineeGroup, &ParameterNode->GetAnimated()->GetDummy());
		}
	...
	else if (MotionBlurNode != NULL)
	{
		...
		if (ParameterNode != NULL || ParameterNode->GetAnimated() != NULL)
		{
			ImportAnimatedProperty(&Actor->CamOverridePostProcess.MotionBlur_Amount, TEXT("CamOverridePostProcess.MotionBlur_Amount"), MatineeGroup, &ParameterNode->GetAnimated()->GetDummy());
		}
	}
}

void CImporter::ImportAnimatedProperty(FLOAT* Value, const TCHAR* ValueName, UInterpGroupInst* MatineeGroup, FLOAT* ColladaValue)
{
	if (ColladaDocument == NULL || ColladaValue == NULL || Value == NULL || MatineeGroup == NULL) return;

	// Retrieve the COLLADA animated element for this value and verify that it contains an animation curve.
	FCDAnimated* ColladaAnimated = ColladaDocument->FindAnimatedValue(ColladaValue);
	if (ColladaAnimated == NULL)
	{
		*Value = *ColladaValue;
		return;
	}
	...
	if (ColladaCurve == NULL || ColladaCurve->GetKeys().empty())
	{
		*Value = *ColladaValue;
		return;
	}
	...
	// Import the COLLADA animation curve into the Matinee track.
	FloatList& Times = ColladaCurve->GetKeys();
	FloatList& Values = ColladaCurve->GetKeyValues();
	FMVector2List& InTangents = ColladaCurve->GetInTangents();
	FMVector2List& OutTangents = ColladaCurve->GetOutTangents();
	INT KeyCount = (INT) ColladaCurve->GetKeys().size();
		...
		case FUDaeInterpolation::BEZIER:
			{
				...
				FMVector2& InTangent = ColladaCurve->GetInTangents()[KeyIndex];
				FMVector2& OutTangent = ColladaCurve->GetInTangents()[KeyIndex];
				const FLOAT& KeyValue = ColladaCurve->GetKeyValues()[KeyIndex];
				FLOAT InSlope = (KeyValue - InTangent.y) / (Key.InVal - InTangent.x);
				FLOAT OutSlope = (OutTangent.y - KeyValue) / (OutTangent.x - Key.InVal);
				Key.InterpMode = !IsEquivalent(InSlope, OutSlope) ? CIM_CurveBreak : CIM_CurveUser;
				break;
			}
	...
	// Fill in the curve keys with the correct data values.
	...
	for (INT KeyIndex = 0; KeyIndex < KeyCount; ++KeyIndex)
	{
		...
		Key.OutVal = /*(*Conversion)*/(Values[KeyIndex]);
		if (HasTangents)
		{
			...
			Key.ArriveTangent = (Key.OutVal - /*(*Conversion)*/(InTangents[KeyIndex].v)) * PreviousSpan / (Key.InVal - InTangents[KeyIndex].u);
			Key.LeaveTangent = (/*(*Conversion)*/(OutTangents[KeyIndex].v) - Key.OutVal) * NextSpan / (OutTangents[KeyIndex].u - Key.InVal);
		}
		else
		{
