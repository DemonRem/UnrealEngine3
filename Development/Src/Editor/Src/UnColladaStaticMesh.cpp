...
void CStaticMesh::ImportStaticMeshPolygons(FStaticMeshRenderData* RenderMesh, const FCDGeometryInstance* ColladaInstance, const FCDGeometryMesh* ColladaMesh)
{
	...
	for (uint32 ElementIndex = 0; ElementIndex < MeshElementCount; ++ElementIndex)
	{
		...
		FStaticMeshElement* Element = new(RenderMesh->Elements) FStaticMeshElement(EngineMaterial);
		...
		const UInt32List& VertexCounts = ColladaPolygons->GetFaceVertexCounts();
		const INT PolygonCount = (INT) VertexCounts.size();
		...
		const FCDGeometryPolygonsInputList& PolygonsInputs = ColladaPolygons->GetInputs();
		for (FCDGeometryPolygonsInputList::const_iterator InputIterator = PolygonsInputs.begin(); InputIterator != PolygonsInputs.end(); ++InputIterator)
		{
			const FCDGeometryPolygonsInput* PolygonsInput = *InputIterator;
			const UInt32List* InputIndices = ColladaPolygons->FindIndicesForIdx(PolygonsInput->idx);
			if (InputIndices == NULL || PolygonsInput->GetSource() == NULL)
			{
				continue;
			}

			const FloatList& SourceData = PolygonsInput->GetSource()->GetSourceData();
			UINT Stride = (UINT) PolygonsInput->GetSource()->GetSourceStride();
...
#			define END_FOREACH } Offset += PolygonTriangleCount + 2; } }
			...
			switch (PolygonsInput->GetSource()->GetSourceType())
			{
					...
					if (!BaseImporter->InvertUpAxis())
					{
						FOREACH_TRIANGLE
						{
							for (UINT I = 0; I < 3; ++I)
							{
								...
								Triangle->Vertices[I].Y = - SourceData[Stride * DataIndices[I] + 1];
								Triangle->Vertices[I].Z = SourceData[Stride * DataIndices[I] + 2];
							}
						} END_FOREACH
					}
					else
...
typedef vector<CStaticMeshSmoothingGroup*> CStaticMeshSmoothingGroupList;
...
typedef vector<CStaticMeshFace*> CStaticMeshPolygonList;
...
void CStaticMesh::CreateSmoothingGroups(const FCDGeometryMesh* ColladaMesh, UINTArray& PolygonSmoothingGroups)
{
	...
	size_t VertexCount = ColladaPositionSource->GetSourceData().size() / ColladaPositionSource->GetSourceStride();
	...
	for (size_t PolygonIndex = 0; PolygonIndex < ColladaPolygonsCount; ++PolygonIndex)
	{
		...
		const UInt32List* PositionIndices = ColladaPolygons->FindIndices(PositionInput);
		const UInt32List* NormalIndices = ColladaPolygons->FindIndices(NormalInput);
		if (PositionIndices == NULL || NormalIndices == NULL) continue;

		size_t ColladaPolygonsFaceCount = ColladaPolygons->GetFaceVertexCounts().size();
		size_t FaceVertexOffset = 0;
		for (size_t FaceIndex = 0; FaceIndex < ColladaPolygonsFaceCount; ++FaceIndex)
		{
			// Skip holes.
			if (ColladaPolygons->GetHoleFaces().find(FaceIndex) != ColladaPolygons->GetHoleFaces().end()) continue;
			...
			Face->Vertices.insert( Face->Vertices.end(), PositionIndices->begin() + FaceVertexOffset, PositionIndices->begin() + ( FaceVertexOffset + FaceVertexCount ) );
			Face->Normals.insert( Face->Normals.end(), NormalIndices->begin() + FaceVertexOffset, NormalIndices->begin() + ( FaceVertexOffset + FaceVertexCount ) );
	...
	vector<UInt32List> MergedSmoothingGroups; MergedSmoothingGroups.resize(SmoothingGroupCount);
	for (size_t GroupIndex = 0; GroupIndex < SmoothingGroupCount; ++GroupIndex)
	{
...
FCDGeometry* CStaticMesh::ExportStaticMesh(FStaticMeshRenderData& RenderMesh, const TCHAR* MeshName)
{
	...
	INT VertexCount = RenderMesh.VertexBuffer.GetNumVertices();
	PositionSource->GetSourceData().reserve(VertexCount * 3);
	for (INT PosIndex = 0; PosIndex < VertexCount; ++PosIndex)
	{
		...
		PositionSource->GetSourceData().push_back(Position.X);
		PositionSource->GetSourceData().push_back(-Position.Y);
		PositionSource->GetSourceData().push_back(Position.Z);
	}
	...
	NormalSource->GetSourceData().reserve(VertexCount * 3);
	...
	TexCoordSources.resize(TexCoordSourceCount, NULL);
	for (INT TexCoordSourceIndex = 0; TexCoordSourceIndex < TexCoordSourceCount; ++TexCoordSourceIndex)
	{
		...
		TexCoordSource->GetSourceData().reserve(VertexCount * 2);
		for (INT TexCoordIndex = 0; TexCoordIndex < VertexCount; ++TexCoordIndex)
		{
			const FVector2D& TexCoord = RenderMesh.VertexBuffer.VertexUV(TexCoordSourceIndex, TexCoordIndex);
			TexCoordSource->GetSourceData().push_back(TexCoord.X);
			TexCoordSource->GetSourceData().push_back(-TexCoord.Y);
		}
	}
	...
	for (INT PolygonsIndex = 0; PolygonsIndex < PolygonsCount; ++PolygonsIndex)
	{
		...
		for (INT TexCoordSourceIndex = 0; TexCoordSourceIndex < TexCoordSourceCount; ++TexCoordSourceIndex)
		{
			...
			ColladaTexCoordInput->set = TexCoordSourceIndex;
		}
		UInt32List* ColladaIndices = ColladaPolygons->FindIndices(PositionSource);
		...
		ColladaPolygons->GetFaceVertexCounts().resize(TriangleCount, 3);
