...
public:
	 ...
	 */
	void ExportStaticMesh(AActor* Actor, UStaticMeshComponent* StaticMeshComponent);
...
private:
	 ...
	 */
	CExporterActor* ExportActor(AActor* Actor);

	/**
	 * Exports the Matinee movement track into the COLLADA animation library.
	 */
	void ExportMatineeTrackMove(CExporterActor* ColladaActor, UInterpTrackMove* MoveTrack);
	 ...
	 */
	void ExportAnimatedVector(const TCHAR* Name, FCDAnimated* ColladaAnimated, INT ColladaAnimatedIndex, FInterpCurveVector* Curve, INT CurveIndex, FCDConversionFunctor* Conversion);
...
public:
	...
	CExporterAnimatableProperty* CreateProperty(const TCHAR* Name, FLOAT* ColladaValues, INT ColladaValueCount=1, FCDConversionFunctor* Conversion=NULL);
};
