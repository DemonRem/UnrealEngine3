...
#include "GFxUIGenericBrowserContextMenus.h"
...
WxMBGenericBrowserContext_GFxMovieInfo::WxMBGenericBrowserContext_GFxMovieInfo()
{
	Append(IDMN_ObjectContext_Properties,*LocalizeUnrealEd("PropertiesE"),TEXT(""));
	Reimport = Append(IDMN_ObjectContext_Reimport,*LocalizeUnrealEd(TEXT("Reimport")),TEXT(""));
	AppendObjectMenu();
}

void WxMBGenericBrowserContext_GFxMovieInfo::ToggleItemsForCookedContent(UBOOL bSomeObjectsAreCooked)
{
	Super::ToggleItemsForCookedContent( bSomeObjectsAreCooked );

	Enable( Reimport->GetId(), !bSomeObjectsAreCooked );
}
...
void UGenericBrowserType_GFxMovie::Init()
{
    ...
    SupportInfo.AddItem(FGenericBrowserTypeInfo(USwfMovie::StaticClass(), FColor(200,128,128), new WxMBGenericBrowserContext_GFxMovieInfo, 0, this));
}
