...
FGFxFile::FGFxFile(const char* const InFilename, UByte* const pBuffer, const SInt NumBytes)
:	Buffer(pBuffer),
 	Length(NumBytes),
 	Position(0),
 	ErrorCode(0)
 ...
 */
SInt FGFxFile::Write(const UByte *pbufer, SInt numBytes)
{
 ...
 */
SInt FGFxFile::Read(UByte *pbufer, SInt numBytes)
{
	if( numBytes < 0 )
		return -1;
 ...
 */
SInt FGFxFile::SkipBytes(SInt numBytes)
{
	if( numBytes < 0 )
		return -1;
...
SInt FGFxFile::Seek(SInt offset, SInt origin)
{
	switch(origin)
	{
	case Seek_Set:
...
/** Returns new position, -1 for error */  
SInt64 FGFxFile::LSeek(SInt64 offset, SInt origin)
{
...
bool FGFxFile::ChangeSize(SInt newSize)
{
	return true;
}
 ...
 */
SInt FGFxFile::CopyFromStream(GFile *pstream, SInt byteSize)
{
	return 0;
}
