...
INT UResavePackagesCommandlet::Main( const FString& Params )
{
	...
	warnf( NAME_Log, TEXT( "%d/%d packages required resaving" ), PackagesRequiringResave, PackageNames.Num() );
	return 0;
}
...
INT UAnalyzeShaderCachesCommandlet::Main( const FString& Params )
{
	...
	if( Parse(*Params, TEXT("PLATFORM="), PlatformString) )
	{
		...
		else if( PlatformString == TEXT("360") || PlatformString == TEXT("XENON") || PlatformString == TEXT("XBOX360") )
		{
			Platform = SP_XBOXD3D;
		}
	}
