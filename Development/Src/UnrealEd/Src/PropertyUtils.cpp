									 ...
									 const TArray<FString>& InValues)
{
	...
	if ( bNotifiedPreChange )
	{
		...
		{	
			...
			FString After;
			NodeProperty->ExportText( 0, After, Cur.BaseAddress - NodeProperty->Offset, Cur.BaseAddress - NodeProperty->Offset, NULL, PPF_Localized );
			...
			{
				RefreshControlValue( NodeProperty, Cur.BaseAddress );
			}
		}
	}
...
void UPropertyInputProxy::OnUseSelected(WxPropertyControl* InItem)
{
	...
	if( ObjProp || IntProp)
	{
		...
		UClass* ObjPropClass = ObjProp ? ObjProp->PropertyClass : IntProp->InterfaceClass;
...
static void SyncGenericBrowser(WxPropertyControl* InItem)
{
	...
	UInterfaceProperty* IntProp = Cast<UInterfaceProperty>( NodeProperty );
	if( ObjectProperty || IntProp )
	{
		UClass* PropertyClass = ObjectProperty ? ObjectProperty->PropertyClass : IntProp->InterfaceClass;
...
{
	// Create any buttons for the control.
	UPropertyInputArrayItemBase::CreateControls( InItem, InBaseClass, InRC, InValue );
...
{
	FLOAT EqResult = Slider->GetFloatValue();
...
void UPropertyInputCombo::SendToObjects( WxPropertyControl* InItem )
{
	...
	if ( bContainsMultipleValueString && ComboBox->GetSelection() == 0 )
	{
		return;
	}
...
void UPropertyInputEditInline::ButtonClick( WxPropertyControl* InItem, ePropButton InButton )
{
	...
	switch( InButton )
	{
		case PB_NewObject:
		{
			...
			// also see if an archetype of this class is selected in the GB
			if (ObjProp)
			{
				GCallbackEvent->Send(CALLBACK_LoadSelectedAssetsIfNeeded);
