...
namespace {
static int wxCALLBACK ResultsListSort(UPTRINT InItem1, UPTRINT InItem2, UPTRINT InSortData)
{
	...
	for( TObjectIterator<UObject> It; It; ++It )
	{
		...
		if( CurrentFilter->Supports( *It ) )
		{
			...
			if( bFoundItem )
			{
				...
				UObject* obj = *It;
				const PTRINT Data = (PTRINT)obj;
				ResultsList->SetItemPtrData( idx, Data );
			}
		}
	}

	// Sort based on the current options.
	ResultsList->SortItems( ResultsListSort, reinterpret_cast< UPTRINT >( &SearchOptions ) );

	// Misc/clean up.
	ResultsList->Thaw();
	ResultsLabel->SetLabel( *FString::Printf( LocalizeSecure(LocalizeUnrealEd("F_ObjectsFound"), ResultsList->GetItemCount(), ResultsList->GetItemCount()) ) );
}
