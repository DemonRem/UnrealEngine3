...
/**
 * @return	The timeline position of the longest track, which includes 
...
FLOAT WxInterpEd::GetLongestTrackTime() const
{
	...
	for( FGroupConstIterator GroupIter(GetGroupIterator()); GroupIter; ++GroupIter )
	{
		const UInterpGroup* CurrentGroup = *GroupIter;

		for( INT TrackIndex = 0; TrackIndex < CurrentGroup->InterpTracks.Num(); TrackIndex++ )
		{
			const FLOAT TrackEndTime = CurrentGroup->InterpTracks(TrackIndex)->GetTrackEndTime();
...
/**
 * Handler to move the grabbed marker to the beginning of the sequence.
 *
 * @param	In	The event sent by wxWidgets.
 */
...
/**
 * Handler to move the grabbed marker to the end of the sequence.
 *
 * @param	In	The event sent by wxWidgets.
 */
...
{
	MoveGrabbedMarker( GetLongestTrackTime() );
}
...
WxMBInterpEdMarkerMenu::WxMBInterpEdMarkerMenu( WxInterpEd* InterpEd, EInterpEdMarkerType MarkerType )
{
	...
	const UBOOL bIsLoopMarker = (MarkerType == ISM_LoopStart) || (MarkerType == ISM_LoopEnd);

	// Only allow loop markers to moved to the sequence end. 
	// Displaying this option for the sequence end marker is silly!
	if( bIsLoopMarker )
	{
