...
void FASVViewportClient::Draw(FViewport* Viewport, FCanvas* Canvas)
{
	...
	if( AnimSetViewer->UVSetToDisplay != -1 )
	{
		...
		if( AnimSetViewer->UVSetToDisplay < (INT)LODModel.NumTexCoords )
		{
			...
			TArray<DWORD> Indices;
			LODModel.DynamicIndexBuffer.GetArray( Indices );
			UINT NumIndices = LODModel.DynamicIndexBuffer.Num();
...
template<typename IndexBufferType> void UpdateCustomSortStrips( TArray<FSortStripData>& InStrips, FStaticLODModel& InLODModel, FSkelMeshSection& InSection, BYTE AlternateIndexMode )
{
	...
	for( INT OutputStripIndex = 0; OutputStripIndex < InStrips.Num(); ++OutputStripIndex )
	{
		...
		appMemcpy( &Temp[OutputIndex], InLODModel.DynamicIndexBuffer.GetPointerTo( SortStripData.FirstIndex ), SortStripData.NumTris * 3 * InLODModel.DynamicIndexBuffer.DataTypeSize );
	...
	appMemcpy( InLODModel.DynamicIndexBuffer.GetPointerTo( BaseIndex ), Temp, InSection.NumTriangles * 3 * InLODModel.DynamicIndexBuffer.DataTypeSize );
	delete[] Temp;

	// Tell render thread we've updated the index buffer.
	ENQUEUE_UNIQUE_RENDER_COMMAND_ONEPARAMETER(
		UpdateSkelMeshIndexBuffer,
		FRawStaticIndexBuffer16or32<IndexBufferType>*,IndexBuffer,(FRawStaticIndexBuffer16or32<IndexBufferType>*)InLODModel.DynamicIndexBuffer.IndexBuffer,
	{
		IndexBuffer->InitRHI();
	});	
}
		...
		if (Event == IE_DoubleClick)
		{
			if (HitResult)
			{
				if (HitResult->IsA(HNotifyProxy::StaticGetType()))
				{
					...
					if( NotifyNode )
					{
						NotifyNode->SetExpanded(FALSE, TRUE);
					}
					AnimSetViewer->AnimSeqProps->ExpandItem(TEXT("Notifies"),((HNotifyProxy*)HitResult)->NotifyIdx);
				}
