...
void UMaterialEditorInstance::CopyStaticParametersToSourceInstance()
{
...
BEGIN_EVENT_TABLE(WxCustomPropertyItem_MaterialInstanceParameter, WxCustomPropertyItem_ConditonalItem)
	EVT_BUTTON(ID_MATERIALINSTANCEEDITOR_RESETTODEFAULT, OnResetToDefault)
END_EVENT_TABLE()
 ...
 */
void WxCustomPropertyItem_MaterialInstanceParameter::Create(	wxWindow* InParent,
					...
					UBOOL bInSupportsCustomControls )
{
	...
	{
		ResetToDefault = new wxBitmapButton( this, ID_MATERIALINSTANCEEDITOR_RESETTODEFAULT, GPropertyWindowManager->Prop_ResetToDefaultB );
		IndentX += 15;
...
void WxCustomPropertyItem_MaterialInstanceParameter::OnResetToDefault(wxCommandEvent &Event)
{
	UMaterialEditorInstance* Instance = GetInstanceObject();
...
BEGIN_EVENT_TABLE(WxMaterialInstanceEditor, WxMaterialEditorBase)
	EVT_MENU(ID_MATERIALINSTANCEEDITOR_SYNCTOGB, OnMenuSyncToGB)
	EVT_MENU(ID_MATERIALINSTANCEEDITOR_OPENEDITOR, OnMenuOpenEditor)
	EVT_LIST_ITEM_ACTIVATED(ID_MATERIALINSTANCEEDITOR_LIST, OnInheritanceListDoubleClick)
	EVT_LIST_ITEM_RIGHT_CLICK(ID_MATERIALINSTANCEEDITOR_LIST, OnInheritanceListRightClick)
END_EVENT_TABLE()
		...
		FDockingParent(this)
{
	...
	UMaterialInstanceConstant* InstanceConstant = Cast<UMaterialInstanceConstant>(InMaterialInstance);
	...
	InheritanceList = new WxListView(this, ID_MATERIALINSTANCEEDITOR_LIST, wxDefaultPosition, wxDefaultSize, wxLC_REPORT | wxLC_SINGLE_SEL);
	RebuildInheritanceList();

	// Add docking windows.
	{
		AddDockingWindow(PropertyWindow, FDockingParent::DH_Left, *FString::Printf(*LocalizeUnrealEd("PropertiesCaption_F"), *MaterialInstance->GetPathName()), *LocalizeUnrealEd("Properties"));
		AddDockingWindow(InheritanceList, FDockingParent::DH_Left, *FString::Printf(*LocalizeUnrealEd("MaterialInstanceParent_F"), *MaterialInstance->GetPathName()), *LocalizeUnrealEd("MaterialInstanceParent"));
...
void WxMaterialInstanceEditor::NotifyPreChange(void* Src, UProperty* PropertyThatChanged)
{
...
void WxMaterialInstanceEditor::NotifyPostChange(void* Src, UProperty* PropertyThatChanged)
{
	...
	if(PropertyThatChanged->GetName()==TEXT("Parent"))
	{
		...
		if(MaterialEditorInstance->Parent==MaterialInstance)
		{
...
void WxMaterialInstanceEditor::RebuildInheritanceList()
{
	...
	InheritanceList->Freeze();
	{
		...
		if(InstanceConstant)
		{
			...
			// Add all parents
			UMaterialInstance* Parent = InstanceConstant->Parent;
...
void WxMaterialInstanceEditor::SyncSelectedParentToGB()
{
	INT SelectedItem = (INT)InheritanceList->GetFirstSelected();
	if(ParentList.IsValidIndex(SelectedItem))
	{
		UMaterialInstance* SelectedMaterialInstance = ParentList(SelectedItem);
...
void WxMaterialInstanceEditor::OpenSelectedParentEditor()
{
	INT SelectedItem = (INT)InheritanceList->GetFirstSelected();
	if(ParentList.IsValidIndex(SelectedItem))
	{
		...
		if(MaterialInstance!=SelectedMaterialInstance)
		{
			...
			else if(SelectedMaterialInstance->IsA(UMaterialInstanceConstant::StaticClass()))
			{
				...
				wxFrame* MaterialInstanceEditor = new WxMaterialInstanceEditor( (wxWindow*)GApp->EditorFrame,-1, MaterialInstanceConstant );
...
void WxMaterialInstanceEditor::OnMenuSyncToGB(wxCommandEvent &Event)
{
	SyncSelectedParentToGB();
}

/** Event handler for when the user wants to open the editor for the selected parent material. */
void WxMaterialInstanceEditor::OnMenuOpenEditor(wxCommandEvent &Event)
{
	OpenSelectedParentEditor();
}

/** Double click handler for the inheritance list. */
void WxMaterialInstanceEditor::OnInheritanceListDoubleClick(wxListEvent &ListEvent)
{
	OpenSelectedParentEditor();
}

/** Event handler for when the user right clicks on the inheritance list. */
void WxMaterialInstanceEditor::OnInheritanceListRightClick(wxListEvent &ListEvent)
{
	INT SelectedItem = (INT)InheritanceList->GetFirstSelected();
	if(ParentList.IsValidIndex(SelectedItem))
	{
		...
		if(SelectedMaterialInstance != MaterialInstance)
		{
			FString Label;
			...
			ContextMenu->Append(ID_MATERIALINSTANCEEDITOR_OPENEDITOR, *Label);
		}

		ContextMenu->Append(ID_MATERIALINSTANCEEDITOR_SYNCTOGB, *LocalizeUnrealEd("SyncGenericBrowser"));
