 ...
 */
void FInterpEdViewportClient::DrawSubTrackGroup( FCanvas* Canvas, UInterpTrack* Track, const FSubTrackGroup& InGroup, INT GroupIndex, const FInterpTrackLabelDrawParams& LabelDrawParams )
{
...
INT FInterpEdViewportClient::DrawTrack( FCanvas* Canvas, UInterpTrack* Track, UInterpGroup* Group, const FInterpTrackDrawParams& TrackDrawParams, const FInterpTrackLabelDrawParams& LabelDrawParams )
{
	...
	if( bIsTrackWithinScrollArea )
	{
		...
		// If the track is in the visible scroll alea, draw the tracks label
			...
			if( Track->SubTrackGroups.Num() > 0 )
			{
				...
				for( INT GroupIndex = 0; GroupIndex < Track->SubTrackGroups.Num(); ++GroupIndex )
				{
					...
					if( bIsGroupWithinScrollArea )
					{
						...
						DrawSubTrackGroup( Canvas, Track, SubGroup, GroupIndex, SubLabelDrawParams );
						Canvas->PopTransform();
...
void FInterpEdViewportClient::DrawTrackLabel( FCanvas* Canvas, UInterpTrack* Track, UInterpGroup* Group, const FInterpTrackDrawParams& TrackDrawParams, const FInterpTrackLabelDrawParams& LabelDrawParams )
{
	...
	if( Track->IsA(UInterpTrackFloatBase::StaticClass()) || Track->IsA(UInterpTrackVectorBase::StaticClass()) || Track->IsA(UInterpTrackMove::StaticClass()) || Track->IsA(UInterpTrackLinearColorBase::StaticClass()) )
	{
		...
		if(Canvas->IsHitTesting()) Canvas->SetHitProxy( new HInterpEdTrackGraphPropBtn( Group, Track ) );
