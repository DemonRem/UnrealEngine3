					...
					if(HitResult)
					{
						...
						else if(HitResult->IsA(HInterpEdTrackGraphPropBtn::StaticGetType()))
						{
							...
							if( Track->SubTracks.Num() > 0 )
							{
								...
								{
									InterpEd->AddTrackToCurveEd(Group, Track->SubTracks( SubTrackIndex ) );
								}
							}
							else
							{
								InterpEd->AddTrackToCurveEd(Group, Track);
							}
						}
						...
						{
							UInterpGroup* Group = ((HInterpTrackKeypointProxy*)HitResult)->Group;
							UInterpTrack* Track = ((HInterpTrackKeypointProxy*)HitResult)->Track;
						...
						{
							// A keyframe drawn on a sub group was hit
							HInterpTrackSubGroupKeypointProxy* KeyProxy = (HInterpTrackSubGroupKeypointProxy*)HitResult;
							// The time of the key.
							FLOAT KeyTime = KeyProxy->KeyTime;
							UInterpTrack* Track = KeyProxy->Track;
							UInterpGroup* Group = Track->GetOwningGroup();
							// GroupIndex indicates what sub group the key was drawn on.  INDEX_NONE means the key was drawn on the parent track
							...
							// Select all keyframes at the specified time
							if( Track->SubTracks.Num() > 0 )
							{
								if( GroupIndex == INDEX_NONE )
								{
									// The keyframe was drawn on the parent track, select all keyframes in all groups at the specified time
									for( INT SubGroupIndex = 0; SubGroupIndex < Track->SubTrackGroups.Num(); ++SubGroupIndex )
									{
										...
										for( INT TrackIndex = 0; TrackIndex < SubTrackGroup.TrackIndices.Num(); ++TrackIndex )
										{
											...
											{
												if( InterpEd->KeyIsInSelection( Group, SubTrack, KeyIndex ) )
												{
													// If the key is already selected, deselect it
													InterpEd->RemoveKeyFromSelection( Group, SubTrack, KeyIndex );
												}
												else
												{
													InterpEd->SelectTrack( Group, SubTrack, FALSE );
													InterpEd->AddKeyToSelection( Group, SubTrack, KeyIndex, !bShiftDown );
												}
											}
										}
									}
					...
					for(INT Y=0; Y < TestSizeY; Y++)
					{
						for(INT X=0; X < TestSizeX; X++)
						{
							...
							if(HitProxy && HitProxy->IsA(HInterpTrackKeypointProxy::StaticGetType()))
							{
								...
								NewSelection.AddUniqueItem( FInterpEdSelKey(Group, Track , KeyIndex) );
								...
								for(INT i=0; i<Track->GetNumKeyframes(); i++)
								{
									...
									{
										NewSelection.AddUniqueItem( FInterpEdSelKey(Group, Track, i) );
									}
								}
							}
						}
		...
		for( INT I=0; I<Interp->GroupInst.Num(); ++I )
		{
			...
			if ( AIGroupInst && AIGroupInst->AIGroup && AIGroupInst->AIGroup->HasMoveTrack()==FALSE )
			{
				...
				else if ( AIGroupInst->StageMarkActor && AIGroupInst->PreviewPawn == InObject && Interp->Position == 0.f && Interp->bIsPlaying == FALSE )
				{
					...
					AIGroupInst->StageMarkActor->SetLocation(AIGroupInst->PreviewPawn->Location);
