		...
		if (RenderObject)
		{
			...
			if ((RenderInfo != NULL) && (RenderInfo->Renderer != NULL))
			{
				...
				RenderInfo->Renderer->Draw(RenderObject, TPT_Plane, TLBIconInfo[eIcon].PositionOffset + XOffset, YPos, 
					TLBIconInfo[eIcon].Size, TLBIconInfo[eIcon].Size, Viewport, Canvas, TBT_None);
				if (Overlay)
				{
...
void WxTerrainEditor::OnChangeColor(wxCommandEvent& In)
{
	...
	if (wxColorOut.Ok())
	{
		NewColor = FColor(wxColorOut.Red(), wxColorOut.Green(), wxColorOut.Blue(), 64);
	}
	UBOOL		bRefreshTerrain	= FALSE;
...
//	EdMode->SetCurrentTool(NewTool);
	...
	for (Index = 0; Index < ETETB_MAX; Index++)
	{
		...
		case ETETB_OrientationFlip:
			Image_Inactive.LoadFile(*FString::Printf(TEXT("%swxres\\TerrainEdit_Orientation.bmp"), appBaseDir()), wxBITMAP_TYPE_BMP);
			Image_Active.LoadFile(*FString::Printf(TEXT("%swxres\\TerrainEdit_Orientation_Active.bmp"), appBaseDir()), wxBITMAP_TYPE_BMP);
...
void WxTerrainEditor::SaveData()
{
	...
	debugf(TEXT("TerrainTool ScaleFactor is now %f"), ToolSettings->ScaleFactor);
