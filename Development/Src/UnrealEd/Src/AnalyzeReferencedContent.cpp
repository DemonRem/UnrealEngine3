...
,	VertexInfluencsSize(0)
{
	...
	IndexMemorySize = SkeletalMesh->LODModels(0).DynamicIndexBuffer.GetResourceDataSize();
...
FString FAnalyzeReferencedContentStat::FAnimSequenceStats::ToCSV() const
{
	...
	return FString::Printf(TEXT("%s%1.1f,%i,%i%s"), *CSV, RotationKeySize, OverheadSize, TotalFrames, LINE_TERMINATOR);
}
...
{
	return TEXT("ResourceType,ResourceName,AnimSetName,AnimTag,ScriptReferenced,NumMapsUsedIn,ReferenceType,TransFormat,RotFormat,CompressionScheme,AnimSize,CompressionRatio,TrackTableSize,Tracks,TracksWithoutTrans,TracksWithoutRot,TransKeys,RotKeys,TransKeySize,RotKeySize,OverheadSize,TotalFrames") LINE_TERMINATOR;
}
