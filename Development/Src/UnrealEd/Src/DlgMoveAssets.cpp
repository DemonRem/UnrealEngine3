...
// text entry field that the "Create ClassMoveInfo" dialog presents the user.
...
public:
	...
	virtual bool Validate()
	{
		...
		if ( TempFinalPackageName.Len() > 0 && !FIsValidObjectName(*TempFinalPackageName, Reason) )
		{
			appMsgf( AMT_OK, *Reason );
			return 0;
		}
		// Verify the group name is empty or a valid name.
		if ( TempFinalGroupName.Len() > 0 && !FIsValidXName(*TempFinalGroupName, Reason, DLGNEW_INVALID_GROUP_CHARACTERS) )
		{
			appMsgf( AMT_OK, *Reason );
			return 0;
		}
...
public:
	...
	void GetActiveClassRelocationInfo(TArray<FClassMoveInfo>& OutActiveClassRelocationInfo) const
	{
		...
		for ( INT Index = 0 ; Index < MoveInfoCheckList->GetCount() ; ++Index )
		{
...
private:
	...
	void SetOKToAllState()
	{
		if ( ButtonOKToAll )
		{
			const bool bIsChecked = TreatNameAsSuffixCheck->IsChecked();
			ButtonOKToAll->Enable( bIsChecked );
		}
	}
	...
	void OnCreateNewMoveInfo(wxCommandEvent& In)
	{
		...
		if( Dlg.ShowModal() == wxID_OK )
		{
			...
			if ( RelocationIndex != INDEX_NONE )
			{
				const FString ClassAlreadyExists = FString::Printf(*LocalizeUnrealEd("ClassMoveInfoAlreadyExistsQ"),*Dlg.FinalClassName);
...
int WxDlgMoveAssets::ShowModal(const FString& InPackage, const FString& InGroup, const FString& InName )
{
