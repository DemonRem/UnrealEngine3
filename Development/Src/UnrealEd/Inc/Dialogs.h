...
public:
	...
	*/
	WxSuppressableWarningDialog ( const FString& InPrompt, const FString& InTitle, const FString& InIniSettingName );
...
-----------------------------------------------------------------------------*/
class WxDlgMeshLODFbxImport : public wxDialog
{
public:
	WxDlgMeshLODFbxImport( wxWindow* InParent, const TArray<FString>& InLODGroupNames, const TArray<TArray<FString>*>& LODString, const TCHAR* SrcFileName );
	virtual ~WxDlgMeshLODFbxImport() { }

	void GetLODLevels(TArray<INT>& LODLevelArray) const
	{
		INT Index;
		for ( Index = 0; Index < LODLevelList.Num(); Index++ )
		{
			LODLevelArray.AddItem(LODLevelList(Index)->GetSelection());
		}
	}

private:
	TArray<wxComboBox*> LODLevelList;
...
private:
	...
	void GetResolution(const INT DeviceIndex, INT& OutWidth, INT& OutHeight) const;

	/** Returns string associated with a particular device */
	FString GetDeviceName (const int DeviceIndex ) const;
