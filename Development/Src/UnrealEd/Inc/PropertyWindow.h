...
public:
	 ...
	 */
	void ExpandItem( const FString& InPropertyName, INT InArrayIndex=INDEX_NONE);
...
protected:
	 ...
	 */
	UBOOL MarkFavoritesInternal (FPropertyNode* InNode, const TArray<FString>& InFavoritesList);

	/**
	 * Save Favorites to ini file
	 * @param InFavoritesList - List of favorite nodes by qualified names
	 */
	void SaveFavorites(const TArray<FString>& InFavoritesList);

	/**
	 * Load Favorites from ini file
	 * @param OutFavoritesList - List of favorite nodes by qualified names
	 */
	void LoadFavorites(TArray<FString>& OutFavoritesList);
...
public:
	 ...
	 */
	void MoveFocus (const INT Direction, wxWindow* InWindow );
