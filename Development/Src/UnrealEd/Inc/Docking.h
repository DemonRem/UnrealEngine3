...
public:
	...
	/** 
	 * Overridden to prevent the iconizing (minimizing) WxFloatingFrames, such as the content browser 
	 *
	 * @note	The 'iconize' parameter type MUST be a 'bool' and not 'UBOOL' in order for the function to be overriden.
	 */
	virtual void Iconize(bool iconize = true) { /** Intentionally left blank **/ };

	/** 
	 * Overridden from WxTopLevelWindow.
	 *
	 * @note	Return value type MUST be a 'bool' and not a 'UBOOL' in order for the function to be overriden. 
	 * @return	FALSE always because WxFloatingFrames cannot be iconized
	 */
