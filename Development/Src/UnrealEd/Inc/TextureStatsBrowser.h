...
public:
	...
	*/
		...
		LastTimeRendered(FLT_MAX)
	{
		...
		// Passing 0 to GetDetailedDescription returns the max dimensions
		MaxDim = InTexture->GetDetailedDescription(0);

		CurrentKB = InTexture->CalcTextureMemorySize( TMC_ResidentMips ) / 1024.0f;
		FullyLoadedKB = InTexture->CalcTextureMemorySize( TMC_AllMips ) / 1024.0f;
