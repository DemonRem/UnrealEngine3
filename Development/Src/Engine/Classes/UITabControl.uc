 ...
 */
var(Presentation)	UIScreenValue_Extent		TabButtonPadding;
...
event bool ActivatePage( UITabPage PageToActivate, int PlayerIndex, optional bool bFocusPage=true )
{
	...
	`log(">>>" @ `location @ `showobj(PageToActivate) @ `showvar(bFocusPage) @ "(" $ `showobj(ActivePage) $ ")",,'TabControl' );
	...
	`log("<<<" @ `location @ `showobj(PageToActivate) @ `showvar(bResult) @ "(" $ `showobj(ActivePage) $ ")",,'TabControl');
	return bResult;
}
...
DefaultProperties
{
	...
	TabButtonPadding=(Value=10,ScaleType=UIEXTENTEVAL_Pixels,Orientation=UIORIENT_Horizontal)
	bUpdateLayout=true
