	...
	DWORD InterColumnOffset,const FLinearColor& StatColor)
{
	...
	for (FMemoryCounter* Stat = FirstStat; Stat != NULL; Stat = (FMemoryCounter*)Stat->Next)
	{
		...
		if (Stat->bShowStat == TRUE && (Stat->Value != 0 || Stat->bDisplayZeroStats))
		{
			...
			DrawShadowedString(Canvas,CurrX,Y,*FString::Printf(TEXT("%.2f %s"),MaxMemUsed,Units[MaxUnitIndex]),
				STAT_FONT,StatColor);
			CurrX += InterColumnOffset;
			...
			DrawShadowedString(Canvas,CurrX,Y,*FString::Printf(TEXT("%.2f%% [%d %s]"),PercentOfMax, appRound(TotalAvail), Units[UnitIndex]),
				STAT_FONT,StatColor);
