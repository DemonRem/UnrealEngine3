...
UBOOL FSkeletalMeshMerge::DoMerge()
{
	...
	// disable PackedPosition 
	MergeMesh->bUsePackedPosition = FALSE;
	...
	for( INT MeshIdx=0; MeshIdx < SrcMeshList.Num(); MeshIdx++ )
	{
		...
		if( SrcMesh )
		{
			...
			if ( SrcMesh->bUsePackedPosition )
			{
				...
				debugf(NAME_Error, TEXT("Error in merging meshes. Turn off bUsePackedPosition for Source Mesh (%s)"), *SrcMesh->GetPathName());
				return FALSE;
			}
#endif
			if( bMaxNumLODsInit )
	...
	if(Result)
	{
		...
		else
		{
			...
			for( INT LODIdx=0; LODIdx < MaxNumLODs; LODIdx++ )
			{
				...
				{
					GenerateLODModel< TGPUSkinVertexFloat16Uvs<> >( LODIdx + StripTopLODs );
				}
				else
				{
					GenerateLODModel< TGPUSkinVertexFloat32Uvs<> >( LODIdx + StripTopLODs );
				}
			}
...
void FSkeletalMeshMerge::GenerateLODModel( INT LODIdx )
{
	...
	for( INT CreateIdx=0; CreateIdx < NewSectionArray.Num(); CreateIdx++ )
	{
		...
		for( INT MergeIdx=0; MergeIdx < NewSectionInfo.MergeSections.Num(); MergeIdx++ )
		{
			...
			for( INT VertIdx=MergeSectionInfo.Chunk->BaseVertexIndex; VertIdx < MaxVertIdx; VertIdx++ )
			{
				...
				// For now just take the first UV set.  Different source models may have different amounts of UV sets
				// @todo: Perhaps take the number of UV sets common to all source lod's
				DestVert.UVs[0] = SrcLODModel.VertexBufferGPUSkin.GetVertexUV(VertIdx,0);
			...
			INT MaxIndexIdx = Min<INT>( 
				...
				SrcLODModel.DynamicIndexBuffer.Num() 
				);
			for( INT IndexIdx=MergeSectionInfo.Section->BaseIndex; IndexIdx < MaxIndexIdx; IndexIdx++ )
			{
				DWORD SrcIndex = SrcLODModel.DynamicIndexBuffer.Get(IndexIdx);
	...
	MergeLODModel.VertexBufferGPUSkin.SetUsePackedPosition(!MergeMesh->IsCPUSkinned() && MergeMesh->bUsePackedPosition);
	// Set the number of tex coords on this vertex buffer
	MergeLODModel.VertexBufferGPUSkin.SetNumTexCoords(1);

	// copy vertex resource arrays
	MergeLODModel.VertexBufferGPUSkin = MergedVertexBuffer;

	FDynamicIndexBufferData IndexBufferData;
	IndexBufferData.bNeedsCPUAccess = MergeLODModel.DynamicIndexBuffer.NeedsCPUAccess;
	IndexBufferData.bSetUpForInstancing = FALSE;
	IndexBufferData.DataTypeSize = MaxIndex < MAXWORD ? 2 : 4;
	IndexBufferData.Indices = MergedIndexBuffer;
	IndexBufferData.NumVertsPerInstance = 0;
	MergeLODModel.DynamicIndexBuffer.RebuildIndexBuffer( IndexBufferData );
}
