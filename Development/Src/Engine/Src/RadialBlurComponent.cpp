...
public:
	...
	{
		FMaterialRenderContext MaterialRenderContext(MaterialProxy, View->Family->CurrentWorldTime, View->Family->CurrentRealTime, View);
		MaterialParameters.Set(this,MaterialRenderContext);
		SceneTextureParameters.Set(View, this, SF_Bilinear);
		SetPixelShaderValue(GetPixelShader(),RadialBlurScaleParameter,RadialBlurScale);
...
UBOOL FRadialBlurSceneProxy::Draw(const class FSceneView* View,UINT DPGIndex,DWORD Flags)
{
	...
	if (IsRenderable(View,DPGIndex,Flags))
	{
		...
		PixelShader->SetParameters(View,MaterialProxy,this,CalcBlurScale(View),BlurFalloffExp,BlurOpacity);
