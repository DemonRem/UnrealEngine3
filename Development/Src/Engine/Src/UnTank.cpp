...
void USVehicleSimTank::ApplyWheels(FLOAT InLeftTrackVel, FLOAT InRightTrackVel, ASVehicle* Vehicle)
{
	...
	for (INT i = 0; i < Vehicle->Wheels.Num(); i++)
	{
		...
		// Handbrake slip is used on tanks to make the wheels slip more when turning
		Vehicle->bOutputHandbrake = (Wheel->SteerFactor > 0.0f && Vehicle->OutputSteering != 0);

		SetNxWheelShapeParams(WheelShape, Wheel);
	}
#endif
}
