...
void USkeletalMesh::UpdatePerPolyKDOPs()
{
	...
	if (LODModels(0).DynamicIndexBuffer.DataTypeSize != 2)
	{
	...
	for(INT i=0; i<PerPolyCollisionBones.Num(); i++)
	{
		...
		if(BoneIndex != INDEX_NONE)
		{
			...
			for(INT j=0; j<LODModel->DynamicIndexBuffer.Num(); j+=3)
			{
				WORD Index0 = (WORD)LODModel->DynamicIndexBuffer.Get(j+0);
				WORD Index1 = (WORD)LODModel->DynamicIndexBuffer.Get(j+1);
				WORD Index2 = (WORD)LODModel->DynamicIndexBuffer.Get(j+2);
