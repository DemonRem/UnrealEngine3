...
void AActor::FindBase()
{
	...
	GWorld->SingleLineCheck(Hit, this, ColLocation + FVector(0,0,-8), ColLocation, TRACE_AllBlocking, GetCylinderExtent());
...
FVector APawn::CheckForLedges(FVector AccelDir, FVector Delta, FVector GravDir, int &bCheckedFall, int &bMustJump )
{
	...
	if (!GWorld->SingleLineCheck(Hit, this, Destn, ColLocation, TRACE_AllBlocking|TRACE_StopAtAnyHit))
	{
...
void AProjectile::GrowCollision()
{
	if ( ZeroCollider )
	{
		...
		if ( DefaultCyl != NULL &&
				(ZeroCollider->CollisionComponent != NULL
				? ZeroCollider->CollisionComponent->PointCheck(Hit, Location, FVector(DefaultCyl->CollisionRadius,DefaultCyl->CollisionRadius,DefaultCyl->CollisionHeight), DefaultProj->bCollideComplex ? TRACE_ComplexCollision : 0)
...
void APawn::processLanded(FVector HitNormal, AActor *HitActor, FLOAT remainingTime, INT Iterations)
{
	...
	GWorld->SingleLineCheck(Hit, this, ColLocation - FVector(0,0,0.2f * CylinderComponent->CollisionHeight + 2.f*LEDGECHECKTHRESHOLD),
		ColLocation, TRACE_Actors|TRACE_World|TRACE_StopAtAnyHit, 0.9f * GetCylinderExtent());
...
UBOOL AActor::MoveWithInterpMoveTrack(UInterpTrackMove* MoveTrack, UInterpTrackInstMove* MoveInst, FLOAT CurInterpTime, FLOAT DeltaTime)
{
	...
	MoveTrack->GetLocationAtTime(MoveInst, CurInterpTime, NewPos, NewRot);
