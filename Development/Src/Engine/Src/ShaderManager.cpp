...
/** Set of all active pixel and vertex shaders in this frame. Can only be accessed by render thread! */
TSet<void*> CurrentFrameActiveShaders;
...
{
	// Add to set as it's now tracked this frame and determine whether it's first addition to set.
	UBOOL bIsAlreadyInSet = FALSE;
	CurrentFrameActiveShaders.Add( Shader, &bIsAlreadyInSet );

	// Update size stats on first encounter.
	if( !bIsAlreadyInSet )
	{
		const EMemoryStats MemStat = GetMemoryStatType(Frequency);
		INC_DWORD_STAT_BY(EMemoryStats,Size);
	}
}
