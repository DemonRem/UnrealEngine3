 ...
 */
const FPlayerInformationNew& UGameplayEvents::GetPlayerMetaData(INT PlayerIndex) const
{
...
FArchive& operator<<(FArchive& Ar, FPlayerInformationNew& PlayerInfo)
{
	BYTE IsBot;
	if (Ar.IsLoading())
	{
		appMemzero(&PlayerInfo, sizeof(FPlayerInformationNew));
		if (Ar.Ver() >= ADDED_UNIQUEID_TO_PLAYERINFO)
		{
			if (Ar.Ver() >= REMOVED_UNIQUEID_HASH)
			{
				 Ar << PlayerInfo.ControllerName << PlayerInfo.PlayerName << PlayerInfo.UniqueId << IsBot;
			}
			else
			{
				FString UnusedUniqueIDHash;
				Ar << PlayerInfo.ControllerName << PlayerInfo.PlayerName << PlayerInfo.UniqueId << UnusedUniqueIDHash << IsBot;
			}
		}
		else
		{
		   Ar << PlayerInfo.ControllerName << PlayerInfo.PlayerName << IsBot;
		}
...
FArchive& operator<<(FArchive& Ar, FWeaponClassEventData& WeaponClassEventData)
{
	...
	Ar << WeaponClassEventData.WeaponClassName;
	return Ar;
}
...
FArchive& operator<<(FArchive& Ar, FDamageClassEventData& DamageClassEventData)
{
	...
	Ar << DamageClassEventData.DamageClassName;
	return Ar;
}
...
FArchive& operator<<(FArchive& Ar, FProjectileClassEventData& ProjectileClassEventData)
{
	...
	Ar << ProjectileClassEventData.ProjectileClassName;
	return Ar;
}
...
FArchive& operator<<(FArchive& Ar, FPawnClassEventData& PawnClassEventData)
{
	...
	Ar << PawnClassEventData.PawnClassName;
	return Ar;
}
...
INT UGameplayEventsWriter::ResolvePlayerIndex(AController* Player)
{
	...
	if (Player != NULL && Player->PlayerReplicationInfo != NULL)
	{
		...
		{
			if (PlayerList(Idx).ControllerName == PlayerName)
			{
				PlayerIdx = Idx;
		...
		if (PlayerIdx == INDEX_NONE)
		{
			...
			FPlayerInformationNew& PlayerInfo = PlayerList(PlayerIdx);
			PlayerInfo.ControllerName = PlayerName;
...
INT UGameplayEventsWriter::ResolveWeaponClassIndex(UClass* WeaponClass)
{
	...
	if (WeaponClass != NULL)
	{
		const FString& WeaponClassName = WeaponClass->GetName();
		...
		if (WeaponIdx == INDEX_NONE)
		{
			...
			debugf(NAME_GameStats, TEXT("### Adding WeaponClass [%s] to WeaponList @ index %i"), *WeaponClassName, WeaponIdx);
		}
	}
...
INT UGameplayEventsWriter::ResolvePawnIndex(UClass* PawnClass)
{
	...
	if (PawnClass != NULL)
	{
		const FString& PawnClassName = PawnClass->GetName();
		...
		if (PawnClassIdx == INDEX_NONE)
		{
			...
			debugf(NAME_GameStats, TEXT("### Adding PawnClass [%s] to PawnClassList @ index %i"), *PawnClassName, PawnClassIdx);
		}
	}
...
INT UGameplayEventsWriter::ResolveDamageClassIndex(UClass* DamageClass)
{
	...
	if (DamageClass != NULL)
	{
		const FString& DamageClassName = DamageClass->GetName();
		...
		if (DamageClassIdx == INDEX_NONE)
		{
			...
			debugf(NAME_GameStats, TEXT("### Adding DamageClass [%s] to DamageClassList @ index %i"), *DamageClassName, DamageClassIdx);
		}
	}
...
INT UGameplayEventsWriter::ResolveProjectileClassIndex(UClass* ProjectileClass)
{
	...
	{
		const FString& ProjectileClassName = ProjectileClass->GetName();
		...
		if (ProjectileClassIdx == INDEX_NONE)
		{
			...
			debugf(NAME_GameStats, TEXT("### Adding ProjectileClass [%s] to ProjectileClassList @ index %i"), *ProjectileClassName, ProjectileClassIdx);
		}
	}
...
UBOOL UGameplayEventsReader::SerializeHeader()
{
	if (Archive)
	{
		   ...
		   Header.FileSize <= 0)
		{
			...
			debugf(/*NAME_GameStats,*/ TEXT("Version: %d\nStreamOffset:%d\nFooterOffset:%d\nFileSize:%d\nStreamSize:%d\nArchiveSize:%d"),
				Header.StatsWriterVersion,
				Header.StreamOffset,
				Header.FooterOffset,
				Header.FileSize,
				Header.TotalStreamSize,
				Archive->TotalSize());
			return FALSE;
		}
...
void UGameplayEventsWriter::CloseStatsFile(void)
{
	...
	{	
		...
		{
#if STATS
			FScopedLogTimer ScopeTimer(TEXT("XFlushUtilityDrive"), FALSE);
#endif
			XFlushUtilityDrive();
		}
#endif
