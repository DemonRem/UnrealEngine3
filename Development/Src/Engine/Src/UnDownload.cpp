...
//@todo JoshA -- Please fix this properly
	...
	else
	{
		...
		else
		{
			// Successful.
			Transfered += Count;
			FString Msg1 = FString::Printf( (Info->PackageFlags&PKG_ClientOptional)?*LocalizeProgress(TEXT("ReceiveOptionalFile"),TEXT("Engine")):*LocalizeProgress(TEXT("ReceiveFile"),TEXT("Engine")), *Info->PackageName.ToString() );
			FString Msg2 = FString::Printf( *LocalizeProgress(TEXT("ReceiveSize"),TEXT("Engine")), FileSize/1024, 100.f*Transfered/FileSize );
...
void UDownload::DownloadDone()
{
	...
	else
	{
		...
		if( !*Error && Transfered==0 )
			DownloadError( *FString::Printf( *LocalizeError(TEXT("NetRefused"),TEXT("Engine")), *Info->PackageName.ToString() ) );
		...
		else
		{
			...
			Connection->Driver->Notify->NotifyProgress( TEXT("Success"), *Message, 4.f );
			Connection->Driver->Notify->NotifyReceivedFile( Connection, PackageIndex, Error, 0 );
