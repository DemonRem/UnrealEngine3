...
void UCodecMovieFallback::GetFrame( class FTextureMovieResource* InTextureMovieResource )
{
		...
		InTextureMovieResource->IsInitialized() )
	{
		...
		RHISetRenderTarget(RHIGetGlobalContext(),InTextureMovieResource->GetRenderTargetSurface(),FSurfaceRHIRef());
		RHIClear(RHIGetGlobalContext(),TRUE,ClearColor,FALSE,0.f,FALSE,0);
		RHICopyToResolveTarget(InTextureMovieResource->GetRenderTargetSurface(),FALSE);
	}
}
