...
INT FLocalizedWordWrapHelper::GetNextBreakPosition( const TCHAR* pText, INT CurrentPosition )
{
	...
	{
#if 1	// @fixme ronp - this is the version that is correct for EA characters
...
#else
		TCHAR Prev=0;
		// if the current character is in the middle of a word or other a non-breakable character, we must first find the end of this word
		for ( ; pText[CurrentPosition]; CurrentPosition++ )
		{
			TCHAR Char = pText[CurrentPosition];
			if ( appCanBreakLineAt(Prev, Char) )
		...
		// at this point, pText[CurrentPosition] is pointing to whitespace or some other breakable character - now find the position of next non-breakable character
		for ( ; pText[CurrentPosition]; CurrentPosition++ )
		{
			TCHAR Char = pText[CurrentPosition];
			if ( !appCanBreakLineAt(Prev, Char) )
