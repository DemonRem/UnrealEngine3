...
void ASpeedTreeActor::CheckForErrors()
{
	...
	{
		GWarn->MapCheck_Add( MCTYPE_WARNING, this, *FString::Printf(TEXT("%s : SpeedTreeActor has legacy USpeedTree resource! Please overwrite %s with a new Speedtree 5.0."), *GetName(), *SpeedTreeComponent->SpeedTree->GetPathName() ), MCACTION_DELETE, TEXT("LegacySpeedTree") );
	}
#endif
}
#endif
...
void FSpeedTreeResourceHelper::CleanUp(BOOL bAll)
{
	if (bIsInitialized)
	{
		...
		if (bAll)
		{
			...
			BillboardElement.NumPrimitives = 0;
...
void FSpeedTreeResourceHelper::InitResources()
{
	...
	if( bHasFronds )
	{
		for (INT i = 0; i < FrondElements.Num( ); ++i)
		{
			...
			FrondElements(i).IndexBuffer = &IndexBuffer;
		}
	...
	if( bHasLeafCards )
	{
		for (INT i = 0; i < LeafCardElements.Num( ); ++i)
		{
			...
			LeafCardElements(i).IndexBuffer = &IndexBuffer;
		}
	...
	if( bHasLeafMeshes )
	{
		for (INT i = 0; i < LeafMeshElements.Num( ); ++i)
		{
			...
			LeafMeshElements(i).IndexBuffer = &IndexBuffer;
		}
...
void FSpeedTreeResourceHelper::SetupIndexedGeometry(ESpeedTreeMeshType eType)
{
	...
	for (INT LodIndex = 0; LodIndex < NumLODs; ++LodIndex)
	{
		...
		for (INT MaterialGroup = 0; MaterialGroup < IndexedGeometry.m_nNumMaterialGroups; ++MaterialGroup)
		{
			...
			// Set up mesh element
			TArray<FMeshElement>* Elements = NULL;
			switch (eThisType)
			{
			case STMT_Branches1:
			...
			(*Elements)(LodIndex).NumPrimitives		= NumIndices / 3;
...
void FSpeedTreeResourceHelper::SetupLeafCards()
{
	...
	for (INT LodIndex = 0; LodIndex < NumLODs; ++LodIndex)
	{
		...
		LeafCardElements(LodIndex).NumPrimitives	= LeafCardGeometry.m_nTotalNumCards * 2;
		LeafCardElements(LodIndex).MinVertexIndex	= StartVertex;
		LeafCardElements(LodIndex).MaxVertexIndex	= StartVertex + LeafCardGeometry.m_nTotalNumCards * 4 - 1;
...
void FSpeedTreeResourceHelper::SetupBillboards( )
{
	...
	BillboardElement.MinVertexIndex = 0;
	BillboardElement.MaxVertexIndex = 7;
...
/** Subset of a FMeshElement that needs to be saved for speedtrees. */
struct FSpeedTreeSavedMeshElement
{
	...
	FSpeedTreeSavedMeshElement(const FMeshElement& Element)
	{
		FirstIndex = Element.FirstIndex;
		NumPrimitives = Element.NumPrimitives;
		MinVertexIndex = Element.MinVertexIndex;
		MaxVertexIndex = Element.MaxVertexIndex;
		Type = (EPrimitiveType)Element.Type;
	}

	void InitializeElement(FMeshElement& OutElement) const
	{
		OutElement.FirstIndex = FirstIndex;
		OutElement.NumPrimitives = NumPrimitives;
		OutElement.MinVertexIndex = MinVertexIndex;
		OutElement.MaxVertexIndex = MaxVertexIndex;
...
/** Serializes an array of FMeshElement's. */
void SerializeSpeedtreeElements(FArchive& Ar, TArray<FMeshElement>& RenderElements)
{
	TArray<FSpeedTreeSavedMeshElement> SavedElements;
	if (Ar.IsSaving())
	{
		SavedElements.Empty(RenderElements.Num());
		SavedElements.AddZeroed(RenderElements.Num());
		for (INT i = 0; i < RenderElements.Num(); i++)
		{
			SavedElements(i) = FSpeedTreeSavedMeshElement(RenderElements(i));
		}
	}
	Ar << SavedElements;
	if (Ar.IsLoading())
	{
		...
		RenderElements.AddZeroed(SavedElements.Num());
		for (INT i = 0; i < SavedElements.Num(); i++)
		{
			SavedElements(i).InitializeElement(RenderElements(i));
		}
	}
}
...
FString USpeedTree::GetDesc()
{
	...
	if( SRH && SRH->bIsInitialized )
	{
		if( SRH->bHasBranches )
		{
			Triangles += SRH->Branch1Elements(0).NumPrimitives;	
			Triangles += SRH->Branch2Elements(0).NumPrimitives;	
		...
		{
			Triangles += SRH->LeafCardElements(0).NumPrimitives;	
			Verts += SRH->LeafCardDataBuffer.Vertices.Num( );
		}

		if( SRH->bHasLeafMeshes )
		{
			Triangles += SRH->LeafMeshElements(0).NumPrimitives;	
			Verts += SRH->LeafMeshDataBuffer.Vertices.Num( );
		}

		if( SRH->bHasBillboards )
		{
			Triangles += SRH->BillboardElement.NumPrimitives;
