...
void FLinkedObjDrawUtils::DrawTitleBar(FCanvas* Canvas, const FIntPoint& Pos, const FIntPoint& Size, const FColor& BorderColor, const FColor& BkgColor, const TCHAR* Name, const TCHAR* Comment, const TCHAR* Comment2)
{
	...
	if ( Name )
	{
		...
		{
			DrawShadowedString( Canvas, StringPos.X, StringPos.Y, Name, NormalFont, FColor(255,255,128) );
		}
	}
	...
	{
		INT XL, YL;
		StringSize( NormalFont, XL, YL, Comment2 );
		CommentY -= YL;

		const FIntPoint StringPos( Pos.X + 2, CommentY );
		if ( AABBLiesWithinViewport( Canvas, StringPos.X, StringPos.Y, XL, YL ) )
		{
			DrawString( Canvas, StringPos.X, StringPos.Y, Comment2, NormalFont, FColor(0,0,0) );
		}
		CommentY -= 2;
	}
	...
	{
		INT XL, YL;
		StringSize( NormalFont, XL, YL, Comment );
		CommentY -= YL;

		const FIntPoint StringPos( Pos.X + 2, CommentY );
		if ( AABBLiesWithinViewport( Canvas, StringPos.X, StringPos.Y, XL, YL ) )
		{
			DrawString( Canvas, StringPos.X, StringPos.Y, Comment, NormalFont, FColor(0,0,0) );
		}
	}
}
		...
		for(INT i=0; i<ObjInfo.Inputs.Num(); i++)
		{
			...
			if ( bSufficientlyZoomedIn )
			{
				...
				if ( AABBLiesWithinViewport( Canvas, StringPos.X, StringPos.Y, XL, YL ) ) 
				{
					...
					DrawShadowedString( Canvas, StringPos.X, StringPos.Y, *ObjInfo.Inputs(i).Name, NormalFont, FLinearColor::White );
				}
			}
		}
	}
		...
		for(INT i=0; i<ObjInfo.Outputs.Num(); i++)
		{
			...
			if ( bSufficientlyZoomedIn )
			{
				...
				if ( AABBLiesWithinViewport( Canvas, StringPos.X, StringPos.Y, XL, YL ) )
				{
					...
					DrawShadowedString( Canvas, StringPos.X, StringPos.Y, *ObjInfo.Outputs(i).Name, NormalFont, FLinearColor::White );
				}
			}
		}
	}
	...
	if( ObjInfo.Inputs.Num() > 0 )
	{
		...
		for(INT InputIdx=0; InputIdx < ObjInfo.Inputs.Num(); ++InputIdx )
		{
			...
			if ( bSufficientlyZoomedIn )
			{
				...
				DrawTile( Canvas, Pos.X - LO_CONNECTOR_LENGTH, DrawY - LO_CONNECTOR_WIDTH / 2, LO_CONNECTOR_LENGTH, LO_CONNECTOR_WIDTH, 0.f, 0.f, 0.f, 0.f, ConnColor);
	...
	if( ObjInfo.Outputs.Num() > 0 )
	{
		...
		for( INT OutputIdx=0; OutputIdx<ObjInfo.Outputs.Num(); ++OutputIdx )
		{
			...
			if ( bSufficientlyZoomedIn )
			{
				...
				DrawTile( Canvas, Pos.X + Size.X, DrawY - LO_CONNECTOR_WIDTH / 2, LO_CONNECTOR_LENGTH, LO_CONNECTOR_WIDTH, 0.f, 0.f, 0.f, 0.f, ConnColor);
				if( bHitTesting )
				{
					Canvas->SetHitProxy( NULL );
				}
	...
	for (INT DataIdx = 0; DataIdx < PlacementData.Num(); ++DataIdx)
	{
		...
		if ( bSufficientlyZoomedIn )
		{
			...
			else
			{
				...
				DrawTile( Canvas, DrawX + HalfXL - LO_CONNECTOR_WIDTH / 2, Pos.Y+Size.Y, LO_CONNECTOR_WIDTH, LO_CONNECTOR_LENGTH, 0.f, 0.f, 0.f, 0.f, ConnColor );
			}
...
*/
INT FLinkedObjDrawUtils::DrawStringWrapped( FCanvas* Canvas, UBOOL Draw, FLOAT CurX, FLOAT CurY, FLOAT& XL, FLOAT& YL, UFont* Font, const TCHAR* Text, FLinearColor DrawColor )
{
	const FLOAT Zoom2D = GetUniformScaleFromMatrix(Canvas->GetTransform());
	...
	if ( Zoom2D > TextMinZoom )
	{
		if ( Zoom2D > 1.f - DELTA )
		{
			::DrawStringWrapped( Canvas, TRUE, CurX+1, CurY+1, XL, YL, Font, Text, FLinearColor(0,0,0) );
		}

		::DrawStringWrapped( Canvas, TRUE, CurX, CurY, XL, YL, Font, Text, DrawColor );
