...
void USeqAct_Interp::UpdateObject()
{
	...
	{
		if (OutputLinks.Num() < DefaultOp->OutputLinks.Num())
		{
			// add room for new entries
			OutputLinks.AddZeroed(DefaultOp->OutputLinks.Num()-OutputLinks.Num());
		}
		else
		if (OutputLinks.Num() > DefaultOp->OutputLinks.Num())
		{
			...
			{
				// remove the extra entries
				OutputLinks.Remove(DefaultOp->OutputLinks.Num()-1,OutputLinks.Num()-DefaultOp->OutputLinks.Num());
			}
		}
...
void UInterpGroup::UpdateAnimWeights(FLOAT NewPosition, class UInterpGroupInst* GrInst, UBOOL bPreview, UBOOL bJump)
{
	...
	if ( UsingSlotNode && TotalSlotNodeAnimWeight <= ZERO_ANIMWEIGHT_THRESH )
	{
		debugf(TEXT("SlotName is set, but no weight is applied. Please add a key to curve editor and set weight."));
	}
...
void UInterpGroupAI::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyThatChanged)
	{
		...
		if ( PropertyName == TEXT("PreviewPawnClass") || PropertyName == TEXT("bNoEncroachmentCheck") )
		{
			// refresh variable, so that it can refresh info when next update
			bRecreatePreviewPawn = TRUE;
		}
		else if ( PropertyName == TEXT("StageMarkGroup") )
		{
			// refresh variable, so that it can refresh info when next update
	...
	if (AIGroup->PreviewPawnClass != NULL && StageMarkActor != NULL)
	{
		if (PreviewPawn == NULL)
		{
			...
			if ( PreviewPawn )
			{
				...
				PreviewPawn->PostEditMove( TRUE );

				// need to recalc initial transform for movement track
				USeqAct_Interp * SeqInterp = Cast<USeqAct_Interp>(GetOuter());
...
FVector UInterpGroupInstAI::GetStageMarkPosition(FRotator* Rotation)
{
	if (StageMarkActor)
	{
		*Rotation = StageMarkActor->Rotation;
		...
		if (AIGroup->bNoEncroachmentCheck || GWorld->SingleLineCheck( Hit, StageMarkActor, StageMarkActor->Location - FVector(0, 0, 100), StageMarkActor->Location, TRACE_World ))
		{
...
void UInterpGroupInstAI::UpdatePhysics(UBOOL bInit)
{
	...
	for ( INT I=0; I<TrackInst.Num(); ++I )
	{
		...
		if (TrackInst(I)->IsA(UInterpTrackInstAnimControl::StaticClass()))
		{
			AnimTrackExists = TRUE;
		}		
	}
	...
	if ( Pawn )
	{
		...
		else
		{
			...
			if ( AnimTrackExists )
			{
				Pawn->bNoEncroachCheck = bSavedNoEncroachCheck;			
			}
...
void    UInterpTrackHeadTracking::UpdateHeadTracking(AActor* Actor, UInterpTrackInst* TrInst, FLOAT DeltaTime)
{
	...
	if ( HeadTrackingInst->TrackControls.Num() > 0  && HeadTrackingInst->Mesh && HeadTrackingInst->Mesh->GetOwner() )
	{
		...
		if (BestCandidate)
		{
			...
			for (INT I=0; I<HeadTrackingInst->TrackControls.Num(); ++I)
			{
				HeadTrackingInst->TrackControls(I)->SetTargetLocation(TargetLoc);
			...
			GWorld->GetWorldInfo()->DrawDebugLine(MeshLocation, HeadTrackingInst->LookAtControl->DesiredTargetLocation, 255, 0, 0, TRUE);
			GWorld->GetWorldInfo()->DrawDebugLine(MeshLocation, HeadTrackingInst->LookAtControl->TargetLocation, 255, 255, 0, TRUE);
#endif
			for( TMap<class AActor*,struct FActorToLookAt*>::TIterator It(HeadTrackingInst->CurrentActorMap); It; ++It )
			{
				FActorToLookAt * ActorToLookAt = It.Value();
