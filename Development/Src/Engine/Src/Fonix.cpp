...
#if _WINDOWS
#pragma comment( lib, "..\\..\\External\\GamersSDK\\4.2.1\\lib\\Win32\\VoiceCmds.lib" )
#elif _XBOX
#pragma comment( lib, "..\\..\\External\\GamersSDK\\4.2.1\\lib\\Xenon\\VoiceCmds.lib" )
#elif PS3
#elif PLATFORM_UNIX
#else
...
void FRecogVocabulary::OutputDictionary( TArrayNoInit<struct FRecognisableWord>& Dictionary, FString& Line )
{
	...
	for( INT i = 0; i < Dictionary.Num(); i++  )
	{
		...
		Line += FString::Printf( TEXT( "\"%s\"%%%d" ), *Word.Word, Word.Id );
 ...
 */
void USpeechRecognition::PostEditChange( UProperty* PropertyThatChanged )
{
#if !CONSOLE && WITH_SPEECH_RECOGNITION
 ...
 */
void USpeechRecognition::Serialize( FArchive& Ar )
{
	Super::Serialize( Ar );

	if( Ar.Ver() < VER_DEPRECATED_FONIX_417 )
	{
		VoiceData.Empty();
		UserData.Empty();

		debugf( NAME_Warning, TEXT( "Out of date speech recognition data; please resave \'%s\'" ), *GetName() );
	}
}
...
UBOOL USpeechRecognition::GetResult( DWORD UserIndex, TArray<FSpeechRecognizedWord>& Words )
{
	...
	FnxVoicePtr FnxUserData = ( FnxVoicePtr )InstanceData[UserIndex].UserData.GetTypedData();
	FnxVoiceGetResultsGrammar( FnxVoiceData, FnxUserData, &TempWordID, &NumWords, &TempConfidence, &NumBest );
