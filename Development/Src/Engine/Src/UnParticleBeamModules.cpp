...
void UParticleModuleTypeDataBeam2::PostEditChange(UProperty* PropertyThatChanged)
{
	if (PropertyThatChanged)
	{
		...
		if (PropertyThatChanged->GetFName() == FName(TEXT("MaxBeamCount")))
		{
			...
			if (MaxBeamCount > FDynamicBeam2EmitterData::MaxBeams)
			{
				MaxBeamCount = FDynamicBeam2EmitterData::MaxBeams;
			}
		}
		...
		if (PropertyThatChanged->GetFName() == FName(TEXT("InterpolationPoints")))
		{
			...
			if (InterpolationPoints > FDynamicBeam2EmitterData::MaxInterpolationPoints)
			{
				InterpolationPoints = FDynamicBeam2EmitterData::MaxInterpolationPoints;
			}
		}
	}
...
//	BYTE ModifierType;
//	BITFIELD bScale:1 GCC_BITFIELD_MAGIC;
//	BITFIELD bLockValue:1;
//	BITFIELD bScaleTangent:1;
//	BITFIELD bLockTangent:1;
//	BITFIELD bScaleStrength:1;
//	BITFIELD bLockStength:1;
//	struct FRawDistributionVector Value;
...
void UParticleModuleBeamModifier::SetToSensibleDefaults()
{

}

void UParticleModuleBeamModifier::PostEditChange(UProperty* PropertyThatChanged)
{

}

void UParticleModuleBeamModifier::SpawnEditor(FParticleEmitterInstance* Owner, INT Offset, FLOAT SpawnTime, UParticleModule* LowerLODModule, FLOAT Multiplier)
{
	Spawn(Owner, Offset, SpawnTime);
}

void UParticleModuleBeamModifier::UpdateEditor(FParticleEmitterInstance* Owner, INT Offset, FLOAT DeltaTime, UParticleModule* LowerLODModule, FLOAT Multiplier)
{
	Update(Owner, Offset, DeltaTime);
}
