...
UBOOL FTerrainBV::LineCheck(FTerrainBVTreeLineCollisionCheck& Check, FLOAT& HitTime) const
{
...
UBOOL FTerrainBV::PointCheck(FTerrainBVTreeLineCollisionCheck& Check) const
{
...
static UBOOL CollisionInRegion(const FTerrainSubRegion& InRegion, const UTerrainComponent* TerrainComp)
{
	...
	{
		INT GlobalY = TerrainComp->SectionBaseY + Y;
		for(INT X=InRegion.XPos; X<(InRegion.XPos+InRegion.XSize) && !bContainsVisible; X++)
		{
			INT	GlobalX = TerrainComp->SectionBaseX + X;
...
void FTerrainBVNode::SplitTerrain(const FTerrainSubRegion& InRegion, const UTerrainComponent* TerrainComp, TArray<FTerrainBVNode>& Nodes)
{
	...
	if(InRegion.XSize > MAX_LEAFREGION_SIZE || InRegion.YSize > MAX_LEAFREGION_SIZE)
	{
		...
		WORD ASize = Max(InRegion.XSize/2, InRegion.YSize/2);
...
UBOOL FTerrainBVNode::LineCheck(FTerrainBVTreeLineCollisionCheck& Check) const
{
	...
	B[0]=BoundingVolume.Bounds.Min;
	B[1]=BoundingVolume.Bounds.Max;
	...
	if (bIsLeaf == 0)
	{
		...
		for(INT i=0; i<4; i++)
		{
			...
			if(n.NodeIndex[N] != NULL_NODE)
			{
				...
				FLOAT BoundsHitTime;
				UBOOL bBoundsCheck = Check.Nodes(n.NodeIndex[N]).BoundingVolume.LineCheck(Check, BoundsHitTime);
				if(bBoundsCheck && BoundsHitTime < ClosestHit)
				{
					// Line check against actual node (and subnodes).
					UBOOL bGotHit = Check.Nodes(n.NodeIndex[N]).LineCheck(Check);
					if(bGotHit)
					{
...
UBOOL FTerrainBVNode::LineCheckTriangles(FTerrainBVTreeLineCollisionCheck& Check) const
{
	...
	for(INT Y=Region.YPos; Y<(Region.YPos+Region.YSize) && (bHit == FALSE || bStopAtAnyHit == FALSE); Y++)
	{
		...
		for(INT X=Region.XPos; X<(Region.XPos+Region.XSize) && (bHit == FALSE || bStopAtAnyHit == FALSE); X++)
		{
			...
			if(Terrain->IsTerrainQuadVisible(GlobalX, GlobalY))
			{
				const FVector& A1 = TerrainVerts(VERT(X+0, Y+0));
				const FVector& A2 = TerrainVerts(VERT(X+1, Y+1));
	...
	if (bIsLeaf == 0)
	{
		...
		for(INT i=0; i<4; i++)
		{
			...
			if(n.NodeIndex[N] != NULL_NODE)
			{
				...
				FLOAT BoundsHitTime;
				FTerrainBV ExtendedBV(Check.Nodes(n.NodeIndex[N]).BoundingVolume, Check.LocalExtent);
				UBOOL bBoundsCheck = ExtendedBV.LineCheck(Check, BoundsHitTime);
				if(bBoundsCheck && BoundsHitTime < ClosestHit)
				{
					// Line check against actual node (and subnodes).
					UBOOL bGotHit = Check.Nodes(n.NodeIndex[N]).BoxCheck(Check);
					if(bGotHit)
					{
						// If we hit - set bHit to TRUE, and update ClosestHit time.
						bHit = TRUE;
	...
	if (bIsLeaf == 0)
	{
		...
		for(INT i=0; i<4; i++)
		{
			...
			if(n.NodeIndex[i] != NULL_NODE)
			{
				...
				FTerrainBV ExtendedBV(Check.Nodes(n.NodeIndex[i]).BoundingVolume, Check.LocalExtent);
...
UBOOL FTerrainBVTree::BoxCheck(FTerrainBVTreeBoxCollisionCheck& Check) const
{
	...
	FTerrainBV BV(Nodes(0).BoundingVolume, Check.LocalExtent);
	// Check against the first bounding volume and decide whether to go further
...
FTerrainBVTreeCollisionCheck::FTerrainBVTreeCollisionCheck(const UTerrainComponent* InTerrainComp) :
	...
	Nodes(BVTree.Nodes)
{
	LocalToWorld = TerrainComp->GetTerrain()->LocalToWorld();
	WorldToLocal = TerrainComp->GetTerrain()->WorldToLocal();
}
...
FVector FTerrainBVTreeLineCollisionCheck::GetHitNormal(void)
{
...
FVector FTerrainBVTreePointCollisionCheck::GetHitLocation(void)
{
