...
/**
 * IsLocallyControlled()
 TODO- remove parameter, frickin just check it directly if it is a playercontroller
 * @param PawnController - optional parameter so you can pass a controller that is associated with this pawn but is not attached to it
 * @return - TRUE if controlled by local (not network) player
 */
UBOOL APawn::IsLocallyControlled(class AController* PawnController)
{
	AController *PController = PawnController ? PawnController : Controller;

    return ( PController && ((GWorld->GetNetMode() == NM_Standalone) || !PController->GetAPlayerController() || PController->IsLocalPlayerController()) );
}
