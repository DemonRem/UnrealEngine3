 ...
 */
template<UINT DepthOfFieldFullResMode, UINT TemplTonemapperType, UBOOL bUseImageGrain, UBOOL bSeparateTranslucency>
...
public:
	...
	{
		LowResSceneBuffer.Bind(Initializer.ParameterMap, TEXT("LowResSceneBuffer"), TRUE );
	...
	virtual UBOOL Serialize(FArchive& Ar)
	{
		...
		Ar << LowResSceneBuffer << ImageAdjustments1Parameter << ImageAdjustments2Parameter
		<< ImageAdjustments3Parameter << ColorGradingLUTTextureParameter << HalfResMaskRectParameter << BokehDOFLayerTextureParameter;
...
#define VARIATION(A, B, C, D) typedef FUberPostProcessBlendPixelShader<A, B, C, D> FUberPostProcessBlendPixelShader##A##B##C##D; \
	IMPLEMENT_SHADER_TYPE2(template<>, FUberPostProcessBlendPixelShader##A##B##C##D, SF_Pixel, VER_BLOOM_AFTER_MOTIONBLUR, 0);
	//
	VARIATION(0,0,0,0) VARIATION(0,0,1,0) VARIATION(0,1,0,0) VARIATION(0,1,1,0) VARIATION(0,2,0,0) VARIATION(0,2,1,0)
	VARIATION(1,0,0,0) VARIATION(1,0,1,0) VARIATION(1,1,0,0) VARIATION(1,1,1,0) VARIATION(1,2,0,0) VARIATION(1,2,1,0)
	VARIATION(2,0,0,0) VARIATION(2,0,1,0) VARIATION(2,1,0,0) VARIATION(2,1,1,0) VARIATION(2,2,0,0) VARIATION(2,2,1,0)
	VARIATION(0,0,0,1) VARIATION(0,0,1,1) VARIATION(0,1,0,1) VARIATION(0,1,1,1) VARIATION(0,2,0,1) VARIATION(0,2,1,1)
	VARIATION(1,0,0,1) VARIATION(1,0,1,1) VARIATION(1,1,0,1) VARIATION(1,1,1,1) VARIATION(1,2,0,1) VARIATION(1,2,1,1)
	VARIATION(2,0,0,1) VARIATION(2,0,1,1) VARIATION(2,1,0,1) VARIATION(2,1,1,1) VARIATION(2,2,0,1) VARIATION(2,2,1,1)
...
public:
	...
	FShaderResourceParameter		LowResSceneBufferPointParameter;
	FShaderResourceParameter		BokehDOFLayerTextureParameter;
	...
	{
		LowResSceneBufferPointParameter.Bind(Initializer.ParameterMap, TEXT("LowResSceneBufferPoint"), TRUE );
		BokehDOFLayerTextureParameter.Bind(Initializer.ParameterMap, TEXT("BokehDOFLayerTexture"), TRUE);
	}
	...
	{
		UBOOL bShaderHasOutdatedParameters = FDOFAndBloomBlendPixelShader::Serialize(Ar);
		Ar << MotionBlurParameters << LowResSceneBufferPointParameter << BokehDOFLayerTextureParameter;
		return bShaderHasOutdatedParameters;
	}
};
...
public:
	...
	*/
		...
		,	bScaleEffectsWithViewSize(InEffect->bScaleEffectsWithViewSize)
	{
		...
		if(GBloomSize >= 0.0f)
		{
			BlurBloomKernelSize = GBloomSize;
		}
		...
		if(GBloomScale >= 0.0f)
		{
			BloomScale = GBloomScale;
		}

		// console command override
		extern FLOAT GMotionBlurSoftEdge;
		if(GMotionBlurSoftEdge >= 0.0f)
		{
			MotionBlurSoftEdgeKernelSize = GMotionBlurSoftEdge;
		}

		// console command override
		extern FLOAT GMotionBlurMaxVelocity;
		if(GMotionBlurMaxVelocity >= 0.0f)
		{
			MotionBlurParams.MaxVelocity = GMotionBlurMaxVelocity;
		}

		// console command override
		extern FLOAT GMotionBlurAmount;
		if(GMotionBlurAmount >= 0.0f)
		{
			MotionBlurParams.MotionBlurAmount = GMotionBlurAmount;
		}
		...
		if(GImageGrain >= 0.0f)
		{
			SceneImageGrainScale = GImageGrain;
		}
	...
	*/
	...
	void RenderVariationHalfRes(FViewInfo& View, EDOFType LocalDepthOfFieldType, FLOAT EffectSizeScale)
	{
		...
		if(LocalDepthOfFieldType == DOFType_SimpleDOF)
		{
			...
			RenderGatherPass(View, EGD_DepthOfField, SRTI_FilterColor0, Quality, bAlternativeMode);
			...
			if (bAlternativeMode)
			{
				...
				RenderGatherPass(View, EGD_DepthOfField, SRTI_FilterColor1, Quality, bAlternativeMode);
		...
		if(bUseSoftEdgeMotionBlur)
		{
			...
			RenderGatherPass(View, EGD_MotionBlur, SRTI_FilterColor2);
		...
		if((GVisualizeTexture - 1) != (INT)TranslucencyBuffer)
		{
			...
			SetTextureParameterDirectly(
				BlendPixelShader->GetPixelShader(),
				BlendPixelShader->LowResSceneBufferPointParameter, 
	...
	void RenderBloom(FViewInfo& View, FLOAT EffectSizeScale)
	{
		...
		RenderGatherPass(View, EGD_Bloom, SRTI_FilterColor1, BloomQuality);
	...
	* @param TemplTonemapperType see "TonemapperType" console command, is never -1
	*/
	template<UINT DepthOfFieldFullResMode, UINT TemplTonemapperType, UBOOL bSeparateTranslucency, UBOOL bImageGrain>
	void RenderVariationFullRes(FViewInfo& View, FSceneColorLDRInfo& LDRInfo, const FTextureRHIRef& ColorGradingRHI)
	{
		...
		TShaderMapRef<FUberPostProcessBlendPixelShader<DepthOfFieldFullResMode,TemplTonemapperType,bImageGrain,bSeparateTranslucency> > BlendPixelShader(GetGlobalShaderMap());
...
#if MOBILE
//		SetTextureParameterDirectly(BlendPixelShader->GetPixelShader(), BlendPixelShader->LowResSceneBuffer, TStaticSamplerState<SF_Bilinear>::GetRHI(), GSceneRenderTargets.GetHalfResPostProcessTexture() );
#else
		SetTextureParameterDirectly(BlendPixelShader->GetPixelShader(), BlendPixelShader->LowResSceneBuffer, TStaticSamplerState<SF_Bilinear>::GetRHI(), GSceneRenderTargets.GetHalfResPostProcessTexture() );
#endif
		...
		{
			const int RandomRange = 129;
			const int MinimumDistance = 40;

			static int OldRand[2] = {0, 0};
			int NewRand[2] = {0, 0};
			...
			if(AnimateNoise)
			{
				// find a 2d random position not too near to the last one
				for(int i = 0; i < 2; ++i)
				{
					do
					{
						NewRand[i] = rand() % (RandomRange - MinimumDistance);
					} while(abs(NewRand[i] - OldRand[i]) < MinimumDistance);
				}
			}
	...
	UBOOL Render(const FScene* Scene, UINT InDepthPriorityGroup, FViewInfo& View, const FMatrix& CanvasTransform, FSceneColorLDRInfo& LDRInfo)
	{
		...
		// downsample scene to half resolution, depth in alpha
		{
			...
			bool DoDownSample = View.Family->Views(0) == &View;
#if PS3
			// on PS3 we do this trick: RenderTargets[HalfResPostProcess] = RenderTargets[TranslucencyBuffer];
			// because of that we always need to recreate the half res buffer as it gets lost during rendering
			// This can be optimized further by only downsampling one view at a time, that could be the solution for all platforms
			// but on X360 it needs another resolve.
			DoDownSample = TRUE;
#endif
			if(DoDownSample)
			{
		...
		UBOOL bSeparateTranslucency = GSystemSettings.RenderThreadSettings.bAllowSeparateTranslucencyRT && GRHIShaderPlatform == SP_PCD3D_SM5;
...
#define VARIATION(A, B, C, D) \
		if(A == DepthOfFieldFullResMode && B == TonemapperType && C == bSeparateTranslucency && D == bLocalImageGrain) \
		{ \
			RenderVariationFullRes<A, B, C, D>(View, LDRInfo, ColorGradingRHI); \
		} else
		//
		VARIATION(0,0,0,0) VARIATION(0,0,1,0) VARIATION(0,1,0,0) VARIATION(0,1,1,0) VARIATION(0,2,0,0) VARIATION(0,2,1,0)
		VARIATION(1,0,0,0) VARIATION(1,0,1,0) VARIATION(1,1,0,0) VARIATION(1,1,1,0) VARIATION(1,2,0,0) VARIATION(1,2,1,0)
...
FPostProcessSceneProxy* UUberPostProcessEffect::CreateSceneProxy(const FPostProcessSettings* WorldSettings)
{
	...
	if(GTonemapperType == 0
	|| GTonemapperType == 1
	|| GTonemapperType == 2)
	{
		LocalTonemapperType = GTonemapperType;
	}
