	...
	)
{
	...
	// See if we need to use 32 bit vertex indices and create the appropriate type of index buffer
	if (Wedges.Num() < MAXWORD)
	{
		LODModel.DynamicIndexBuffer.DataTypeSize = sizeof(WORD);
		LODModel.DynamicIndexBuffer.IndexBuffer = new FRawStaticIndexBuffer16or32<WORD>(LODModel.DynamicIndexBuffer.NeedsCPUAccess);
	}
	else
	{
		LODModel.DynamicIndexBuffer.DataTypeSize = sizeof(DWORD);
		LODModel.DynamicIndexBuffer.IndexBuffer = new FRawStaticIndexBuffer16or32<DWORD>(LODModel.DynamicIndexBuffer.NeedsCPUAccess);
	}
	...
	for(INT SectionIndex = 0;SectionIndex < LODModel.Sections.Num();SectionIndex++)
	{
		...
		Section.BaseIndex = LODModel.DynamicIndexBuffer.Num();
		...
		for(INT Index = 0;Index < SectionIndexBuffer.Indices.Num();Index++)
		{
			...
			LODModel.DynamicIndexBuffer.AddItem(VertexIndex);
		}
		...
		ExtraVertInfluences->Usage == IWU_FullSwap)
	{
		...
		for(INT BaseSectionIdx=0; BaseSectionIdx < LODModel.Sections.Num(); BaseSectionIdx++)
		{
			...
			INT MaxIdx = Min<INT>(BaseSection.BaseIndex + BaseSection.NumTriangles * 3, LODModel.DynamicIndexBuffer.Num());
			for (INT Idx=BaseSection.BaseIndex; Idx < MaxIdx; Idx+=3)
			{
				...
				{
					DWORD VertexIndex = LODModel.DynamicIndexBuffer.Get(Idx+TriIdx);
...
void USkeletalMesh::BuildClothMapping()
{
	...
	for(INT i=0; i<LODModel->DynamicIndexBuffer.Num(); i+=3)
	{
		DWORD Index0 = LODModel->DynamicIndexBuffer.Get(i+0);
		DWORD Index1 = LODModel->DynamicIndexBuffer.Get(i+1);
		DWORD Index2 = LODModel->DynamicIndexBuffer.Get(i+2);
