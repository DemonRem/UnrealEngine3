...
public:
	 ...
	 */
	void DrawTile(FCommandContextRHI* Context, const class FViewInfo& View, const FMaterialRenderProxy* MaterialRenderProxy);

	/**
	 * Draw a tile at the given location and size, using the full material
	 * (UV = [0..1]
	 */
	void DrawTile(FCommandContextRHI* Context, const class FViewInfo& View, const FMaterialRenderProxy* MaterialRenderProxy, FLOAT X, FLOAT Y, FLOAT SizeX, FLOAT SizeY, UBOOL bIsHitTesting=FALSE, const FHitProxyId HitProxyId=FHitProxyId());

	/**
	 * Draw a tile at the given location and size, using tjhe given UVs
	 * (UV = [0..1]
	 */
	void DrawTile(FCommandContextRHI* Context, const class FViewInfo& View, const FMaterialRenderProxy* MaterialRenderProxy, FLOAT X, FLOAT Y, FLOAT SizeX, FLOAT SizeY, FLOAT U, FLOAT V, FLOAT SizeU, FLOAT SizeV, UBOOL bIsHitTesting=FALSE, const FHitProxyId HitProxyId=FHitProxyId());
...
private:
	 ...
	 */
	void PrepareShaders(FCommandContextRHI* Context, const class FViewInfo& View, const FMaterialRenderProxy* MaterialRenderProxy, const FMatrix& LocalToWorld, UBOOL bUseIdentityViewProjection, UBOOL bIsHitTesting=FALSE, const FHitProxyId HitProxyId=FHitProxyId());
};
