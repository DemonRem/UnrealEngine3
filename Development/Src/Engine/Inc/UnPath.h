...
public:
	...
	*/
	 ...
	 */
	virtual UBOOL IsCrossPylon(){ return FALSE; }
...
struct FNavMeshCrossPylonEdge : public FNavMeshEdgeBase
{
	...
	virtual UBOOL IsCrossPylon(){ return TRUE; }
...
#define DEFAULT_BOX_PADDING 10.f
	...
	*/
	UBOOL GetBestLocationForCyl(const FVector& CylIdealLoc, FLOAT CylRadius, FVector& out_BestLoc, UBOOL bForceInteriorPosition=FALSE);
...
public:
	...
	/**  map of edgeIDs to handles currently using them */
...
public:
	...
	{
		if(EdgePtrs.Num() > 0 && (EdgePtrs.Num() == EdgeStorageData.Num() || EdgeStorageData.Num() == 0))
		{
			return EdgePtrs(Idx);
		}
		else
	...
	*/
	...
	void BuildEdgesFromSegmentSpan(const FVector& Start, const FVector& End, TArray<FEdgeWidthPair>& out_EdgePairs, TArray<APylon*>* PylonsToCheck=NULL,BYTE EdgeGroupID=0, TArray<FNavMeshPolyBase*>* ObstacleMeshPolys=NULL);
								...
								INT* out_EdgeId)
{
	...
	if ( NewEdge != NULL && bAddDummyRefEdge )
	{
		...
		TempPolys.Swap(0,1);
		TempPolys(0)->NavMesh->AddOneWayCrossPylonEdgeToMesh<FNavMeshOneWayBackRefEdge>(StartPt,EndPt,TempPolys,SupportedEdgeWidth,EdgeGroupID,NULL,FALSE,FALSE);			
	  ...
	  /*FORCEINLINE*/ UBOOL ShouldCheckMaterial(INT MaterialIndex) const
	  {
		  ...
		  else
		  {
			  ...
			  for(INT EdgeIdx=0;EdgeIdx<LinkedPoly->GetNumEdges();EdgeIdx++)
			  {
				  ...
				  FNavMeshEdgeBase* LinkedEdge = LinkedPoly->GetEdgeFromIdx(EdgeIdx,ForNavMesh);
				  if(LinkedEdge == NULL)
				  {
					  return TRUE;
				  }
...
//				  checkSlowish(LinkedEdge->IsCrossPylon());
				  ...
				  else if( CPEdge->GetPoly0()->NavMesh->GetPylon()->bForceObstacleMeshCollision || CPEdge->GetPoly1()->NavMesh->GetPylon()->bForceObstacleMeshCollision )
				  {
					  return TRUE;
				  }
			  }
