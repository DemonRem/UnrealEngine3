	...
	virtual void GetAllocatedSize(INT& InNum, INT& InMax)
	{
		...
		if (DynamicParameterDataOffset == 0)
		{
			Size += MaxActiveParticles * sizeof(FParticleSpriteVertex);		// The vertex data array
 			Size += sizeof(FParticleVertexFactory);
		}
		else
		{
			Size += MaxActiveParticles * sizeof(FParticleSpriteVertexDynamicParameter);
			Size += sizeof(FParticleDynamicParameterVertexFactory);
		}
...
struct FParticleSpriteSubUVEmitterInstance : public FParticleEmitterInstance
{
	...
	virtual void GetAllocatedSize(INT& InNum, INT& InMax)
	{
		...
		if (DynamicParameterDataOffset == 0)
		{
			...
			Size += sizeof(FParticleSubUVVertexFactory);
		}
		else
		{
			Size += MaxActiveParticles * sizeof(FParticleSpriteSubUVVertexDynamicParameter);
			Size += sizeof(FParticleSubUVDynamicParameterVertexFactory);
		}
...
struct FParticleBeam2EmitterInstance : public FParticleEmitterInstance
{
	...
	virtual void GetAllocatedSize(INT& InNum, INT& InMax)
	{
		...
		if (DynamicParameterDataOffset == 0)
		{
			Size += MaxActiveParticles * sizeof(FParticleBeamTrailVertex);	// The vertex data array
			Size += sizeof(FParticleBeamTrailVertexFactory);
		}
		else
		{
			Size += MaxActiveParticles * sizeof(FParticleBeamTrailVertexDynamicParameter);
			Size += sizeof(FParticleBeamTrailDynamicParameterVertexFactory);
		}
...
//	virtual void UpdateStartParticles(FLOAT DeltaTime, UBOOL bFirstTime);
	...
	virtual void GetAllocatedSize(INT& InNum, INT& InMax)
	{
		...
		// Take dynamic data into account as well
		Size = sizeof(FParticleTrailEmitterInstance);
		Size += MaxActiveParticleDataSize;								// Copy of the particle data on the render thread
		Size += MaxActiveParticleIndexSize;								// Copy of the particle indices on the render thread
		if (DynamicParameterDataOffset == 0)
		{
