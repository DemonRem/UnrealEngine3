...
#define VER_MIN_SHADER	VER_INVALIDATE_SHADERCACHE1
...
/**
* Sets the value of a geometry shader texture parameter.
*/
FORCEINLINE void SetGeometryShaderTextureParameter(
...
/**
 * Sets the value of a geometry shader parameter.
 * A template parameter specified the type of the parameter value.
 */
template<class ParameterType>
void SetGeometryShaderValue(
	FGeometryShaderRHIParamRef Shader,
	const FShaderParameter& Parameter,
	const ParameterType& Value,
	UINT ElementIndex = 0
	)
...
/**
 * Flushes the shader file and Hash cache.
 */
...
public:
	...
	void Set(const FSceneView* View,FShader* PixelShader, ESamplerFilter ColorFilter, const FTexture2DRHIRef& DesiredSceneColorTexture, ESceneDepthUsage DepthUsage=SceneDepthUsage_Normal) const;
...
#if TRACK_SCENE_SHADER_STATS
#define TRACK_SHADER_USAGE( Shader, Size, Frequency ) TrackShaderUsage( Shader, Size, Frequency );
/**
 * Tracks shader usage per frame.
 *
 * @param	Shader		Pointer to shader, used to uniquely identify
 * @param	Size		Size occupied in memory
 * @param	Frequency	Determines pixel or vertex shader
 */
void TrackShaderUsage( void* Shader, INT Size, EShaderFrequency Frequency );
#else
#define TRACK_SHADER_USAGE( Shader, Size, Frequency ) __noop;
#endif
