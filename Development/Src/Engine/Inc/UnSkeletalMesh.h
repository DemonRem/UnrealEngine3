...
struct FDynamicIndexBufferData
{
...
public:
	...
	FDynamicIndexBuffer(UBOOL bNeedsCPUAccess = FALSE)
	...
	void RebuildIndexBuffer( const FDynamicIndexBufferData& InData );
	INT Num() const;
	INT AddItem(DWORD Val);
	DWORD Get(UINT Idx) const;
	void* GetPointerTo(UINT Idx);
	void GetArray( TArray<DWORD>& OutArray ) const;
	void CopyArray(const TArray<DWORD>& NewArray);
...
#if WITH_EDITOR
	void GetIndexBufferData( FDynamicIndexBufferData& OutData );
	FDynamicIndexBuffer(const FDynamicIndexBuffer& Other);
	FDynamicIndexBuffer& operator=(const FDynamicIndexBuffer& Buffer);
#endif
};
...
public:
	...
	*/
	...
	/** Constructor (default) */
	FStaticLODModel()
	:	DynamicIndexBuffer(TRUE)	// needs to be CPU accessible for CPU-skinned decals.
	,	Size(0)
	,	NumVertices(0)
	{}
