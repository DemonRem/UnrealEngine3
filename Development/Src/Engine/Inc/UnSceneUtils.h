...
#if defined(XBOX) || defined(_WINDOWS_) || PLATFORM_UNIX || PS3
...
#define PLATFORM_SUPPORTS_DRAW_MESH_EVENTS 0
	...
	struct FDrawEvent
	{
		/** Static counter to determine number of event calls/ frame. */
	...
	// Macros to allow for scoping of draw events
	#define BEGIN_DRAW_EVENT(x) { FDrawEvent Event_##x; if( 0 ) Event_##x
	#define END_DRAW_EVENT }
