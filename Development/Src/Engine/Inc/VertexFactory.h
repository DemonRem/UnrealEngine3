 ...
 */
#define IMPLEMENT_VERTEX_FACTORY_TYPE(FactoryClass,ShaderParametersType,ShaderFilename,bUsedWithMaterials,bSupportsStaticLighting,bSupportsDynamicLighting,bPrecisePrevWorldPos,InMinPackageVersion,InMinLicenseePackageVersion) \
...
public:
	...
	FVertexFactoryParameterRef(FVertexFactoryType* InVertexFactoryType,const FShaderParameterMap& ParameterMap, DWORD ShaderFrequency=SF_Vertex);
	...
	void Set(FShader* VertexShader,const FVertexFactory* VertexFactory,const FSceneView& View) const
	{
		if(Parameters)
		{
			Parameters->Set(VertexShader,VertexFactory,View);
		}
	}
	...
	void SetMesh(FShader* VertexShader,const FMeshElement& Mesh,const FSceneView& View) const
	{
		if(Parameters)
		{
			Parameters->SetMesh(VertexShader,Mesh,View);
		}
	}

	friend UBOOL operator<<(FArchive& Ar,FVertexFactoryParameterRef& Ref);
