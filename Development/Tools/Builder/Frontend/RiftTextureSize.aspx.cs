...
public partial class RiftTextureSize : BasePage
{
	...
	private Dictionary<string, int> GetFileSizes(SqlConnection Connection)
	{
		var FileSizes = new Dictionary<string, int>();
		
		string Query = "SELECT PackageName, UncompressedSize FROM Packages";
		using (SqlCommand Command = new SqlCommand(Query, Connection))
		{
			SqlDataReader Reader = Command.ExecuteReader();
			if (Reader.HasRows)
		...
		Query = "SELECT PackageName, CompressedSize FROM Packages";
		using (SqlCommand Command = new SqlCommand(Query, Connection))
		{
			SqlDataReader Reader = Command.ExecuteReader();
			if (Reader.HasRows)
	...
	protected void Page_Load( object sender, EventArgs e )
	{
		using( SqlConnection Connection = new SqlConnection( ConfigurationManager.ConnectionStrings["DevDbConnectionString"].ConnectionString ) )
		{
			...
			var ObjectCounts = GetObjectCounts( Connection, "Texture2D" );
			var PackageTotals = GetPackageTotals( Connection, ObjectCounts, "Texture2D" );
			FillSeries( "Texture2D", PackageTotals );

			var LightMapTotals = GetLightingTotals( Connection, PackageTotals, "LightMapTexture2D" );
			FillSeries( "LightMapTexture2D", LightMapTotals );

			var ShadowMapTotals = GetLightingTotals( Connection, PackageTotals, "ShadowMapTexture2D" );
			FillSeries( "ShadowMapTexture2D", ShadowMapTotals );

			FileSizes = GetFileSizes(Connection);
	...
	protected void Button_SaveAsCSV_Click( object sender, EventArgs e )
	{
		...
		var Textures = GetValues( "Texture2D" );
		var LightMaps = GetValues( "LightMapTexture2D" );
		var ShadowMaps = GetValues( "ShadowMapTexture2D" );
		...
		foreach( string Key in FileSizes.Keys )
		{
			...
			float FileSize = (float) (FileSizes[Key] / 1024.0 / 1024.0);
