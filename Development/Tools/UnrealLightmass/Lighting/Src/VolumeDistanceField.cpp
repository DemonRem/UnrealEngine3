...
void FStaticLightingSystem::BeginCalculateVolumeDistanceField()
{
	...
	if (VolumeSizeX * VolumeSizeY * VolumeSizeZ > VolumeDistanceFieldSettings.MaxVoxels)
	{
		const UINT NumVoxels = VolumeSizeX * VolumeSizeY * VolumeSizeZ;
		const INT OldSizeX = VolumeSizeX;
		const INT OldSizeY = VolumeSizeY;
		const INT OldSizeZ = VolumeSizeZ;
		const FLOAT SingleDimensionScale = appPow((FLOAT)NumVoxels / VolumeDistanceFieldSettings.MaxVoxels, 1.0f / 3.0f);
