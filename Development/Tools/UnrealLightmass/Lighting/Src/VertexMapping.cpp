				...
				if(!bIsShadowed)
				{
						...
						)
					{
						...
						if(bUseStaticLighting)
						{
							...
							const FLinearColor LightIntensity = Light->GetDirectIntensity(AdjacentSampleVertex.WorldPosition);
			...
			for(INT TriangleSampleIndex = 0; TriangleSampleIndex < NumSamples; TriangleSampleIndex++)
			{
				...
				if (bSearchForThisSample)
				{
						...
						|| !PhotonMappingSettings.bUseFinalGathering && GeneralSettings.NumIndirectLightingBounces >= 0)
					{
						...
						const FLinearColor AccumulatedDirectIllumination = .5f * CalculateIncidentDirectIllumination(VertexMapping, SampleVertex, 1, MappingContext, RandomStream, bDebugThisSample);
	...
	for (INT TriangleIndex = 0; TriangleIndex < Mesh->NumTriangles; TriangleIndex++)
	{
		...
		if (bSearchForThisTriangle)
		{
				...
				|| !PhotonMappingSettings.bUseFinalGathering && GeneralSettings.NumIndirectLightingBounces >= 0)
			{
				...
				//@todo - why is the .5 factor needed to match up with deposited irradiance photons?
				const FLinearColor AccumulatedDirectIllumination = .5f * CalculateIncidentDirectIllumination(VertexMapping, V0, 1, MappingContext, RandomStream, bDebugThisSample);
