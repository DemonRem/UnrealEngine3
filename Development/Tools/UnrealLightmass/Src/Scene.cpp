 ...
 */
FLinearColor FLight::GetDirectIntensity(const FVector4& Point) const
{
	// @lmtodo: UE3 was checking bEnabled, but I assume non enabled lights won't even make it to LM
	return FLinearColor(Color) * Brightness;
}
...
{
	const FLinearColor LightPower = GetDirectIntensity(FVector4(0,0,0)) * IndirectLightingScale * (FLOAT)PI * SceneBounds.SphereRadius * SceneBounds.SphereRadius;
 ...
 * Computes the intensity of the direct lighting from this light on a specific point.
 */
FLinearColor FPointLight::GetDirectIntensity(const FVector4& Point) const
{
	FLOAT RadialAttenuation = appPow( Max(1.0f - ((Position - Point) / Radius).SizeSquared(),0.0f), FalloffExponent );
	return FLight::GetDirectIntensity(Point) * RadialAttenuation;
}
 ...
 * Computes the intensity of the direct lighting from this light on a specific point.
 */
FLinearColor FSpotLight::GetDirectIntensity(const FVector4& Point) const
{
	FLOAT	ClampedInnerConeAngle = Clamp(InnerConeAngle,0.0f,89.0f) * (FLOAT)PI / 180.0f,
			ClampedOuterConeAngle = Clamp(OuterConeAngle * (FLOAT)PI / 180.0f,ClampedInnerConeAngle + 0.001f,89.0f * (FLOAT)PI / 180.0f + 0.001f),
	...
	FLOAT RadialAttenuation = appPow( Max(1.0f - ((Position - Point) / Radius).SizeSquared(),0.0f), FalloffExponent );
	return FLight::GetDirectIntensity(Point) * RadialAttenuation * SpotAttenuation;
}
 ...
 * Computes the intensity of the direct lighting from this light on a specific point.
 */
FLinearColor FMeshAreaLight::GetDirectIntensity(const FVector4& Point) const
{
	...
	return AccumulatedPower / TotalSurfaceArea;
}
