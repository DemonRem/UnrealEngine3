...
namespace P4ChangeParser
{
	...
	public class P4ParsedChangelist
	{
		...
		/// </summary>
		private P4Changelist mBaseChangelist;
		...
		/// Base changelist information on which the parsed changelist is populated
		/// </summary>
		public P4Changelist BaseChangelist
		{
		...
		#region Constructors
		/// <summary>
		/// Construct a P4ParsedChangelist from the provided P4Changelist
		/// </summary>
		/// <param name="InChangelist">Changelist to use to populate the parsed changelist</param>
		public P4ParsedChangelist(P4Changelist InChangelist)
		{
			mBaseChangelist = InChangelist;
		...
		public static bool ChangelistSupportsTagging(P4Changelist InChangelist)
		{
		...
		private void WriteHtml(HtmlTextWriter HtmlWriter)
		{
			...
			HtmlWriter.RenderEndTag();
			HtmlWriter.RenderBeginTag(HtmlTextWriterTag.Td);
			HtmlWriter.Write(mBaseChangelist.Number);
			HtmlWriter.RenderEndTag();
			HtmlWriter.RenderEndTag();
			HtmlWriter.RenderBeginTag(HtmlTextWriterTag.Tr);
			HtmlWriter.RenderBeginTag(HtmlTextWriterTag.Td);
			...
			HtmlWriter.RenderEndTag();
			HtmlWriter.RenderBeginTag(HtmlTextWriterTag.Td);
			HtmlWriter.Write(mBaseChangelist.Date.ToLocalTime().ToShortDateString() + " " + mBaseChangelist.Date.ToLocalTime().ToShortTimeString());
			HtmlWriter.RenderEndTag();
			HtmlWriter.RenderEndTag();
			HtmlWriter.RenderEndTag();
