...
namespace ConsoleInterface
{
	...
	String^ OptionallyUppercase( TOCSettings^ BuildSettings, String^ In )
	{
		if( BuildSettings->bUppercasePathNames )
		{
			return( In->ToUpper() );
		}
	...
	List<TOCInfo^>^ Platform::GenerateTOC(String^ TagSetName, TOCSettings^ BuildSettings, String^ Language)
	{
		...
		TOCPath = OptionallyUppercase( BuildSettings, TOCPath );
	...
	List<TOCInfo^>^ Platform::TocBuild(String ^TOCName, TOCSettings ^BuildSettings, array<Tag^> ^SyncTags, String ^Language)
	{
		...
		{
			ProcessFileGroups(TOC, mSharedSettings->FileGroups, BuildSettings, SyncTags, NotPlatforms, NotLanguages, NotGames, Language);
		}

		if(BuildSettings->GameOptions != nullptr && BuildSettings->GameOptions->FileGroups != nullptr)
		{
			ProcessFileGroups(TOC, BuildSettings->GameOptions->FileGroups, BuildSettings, SyncTags, NotPlatforms, NotLanguages, NotGames, Language);
		}
		...
		try
		{
			if(TOCName->Length > 0)
			{
				...
				TocAddFile(TOC, BuildSettings, gcnew FileInfo( TOCPath ), RootFolder, true, true);
			}
		}
	...
	void Platform::ProcessFileGroups(List<TOCInfo^> ^TOC, array<FileGroup^> ^FileGroups, TOCSettings ^BuildSettings, array<Tag^> ^SyncTags, List<String^> ^NotPlatforms, List<String^> ^NotLanguages, List<String^> ^NotGames, String ^Language)
	{
			...
			if (IsMatchingPlatform( Group ) )
			{
				...
				if(bTagsMatch)
				{
					for each (FileSet ^CurSet in Group->Files)
					{
						...
						TocIncludeFiles(TOC, BuildSettings, Path, Wildcard, CurSet, Group, NotPlatforms, NotLanguages, NotGames, Language);
					}
				}
			}
		}
	...
	void Platform::TocAddFile(List<TOCInfo^> ^TOC, TOCSettings ^BuildSettings, FileInfo ^SrcFile, String^ CurrentDir, bool bIsForSync, bool bIsForTOC)
	{
		...
		TargetFile = OptionallyUppercase(BuildSettings, TargetFile);
	...
	void Platform::TocIncludeFiles(List<TOCInfo^> ^TOC, TOCSettings ^BuildSettings, String ^SourceFolder, String ^SourcePattern, FileSet ^CurSet, FileGroup ^Group, List<String^> ^NotPlatforms, List<String^> ^NotLanguages, List<String^> ^NotGames, String ^Language)
	{
		...
		if(mType == PlatformType::Xbox360)
		{
			...
			{
				TocIncludeFiles(TOC, BuildSettings, SourceFolder->Replace("%PLATFORM%", L"Xbox360"), SourcePattern, CurSet, Group, NotPlatforms, NotLanguages, NotGames, Language);
				TocIncludeFiles(TOC, BuildSettings, SourceFolder->Replace("%PLATFORM%", L"Xenon"), SourcePattern, CurSet, Group, NotPlatforms, NotLanguages, NotGames,Language);
				return;
			}
			if(SourcePattern->Contains("%PLATFORM%"))
			{
				TocIncludeFiles(TOC, BuildSettings, SourceFolder, SourcePattern->Replace("%PLATFORM%", L"Xbox360"), CurSet, Group, NotPlatforms, NotLanguages, NotGames, Language);
				TocIncludeFiles(TOC, BuildSettings, SourceFolder, SourcePattern->Replace("%PLATFORM%", L"Xenon"), CurSet, Group, NotPlatforms, NotLanguages, NotGames, Language);
				return;
			}
		}
		...
		try
		{
			...
			for each(FileInfo ^SrcFile in Files)
			{
				...
				if (!bWasFilteredOut)
				{
					TocAddFile(TOC, BuildSettings, SrcFile, RootFolder, Group->bIsForSync, Group->bIsForTOC);
					if( BuildSettings->VerboseOutput )
					{
