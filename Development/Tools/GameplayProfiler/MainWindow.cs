...
namespace GameplayProfiler
{
    public partial class MainWindow : Form
    {
		...
		private void ChangeFileInternal( string FileName )
		{
			...
			FunctionInfo.DumpFunctionInfoMapToList(CurrentProfilerStream, CurrentProfilerStream.AggregateNameToFunctionInfoMap, AggregateFunctionListView, 1, true);
			FunctionTreeViewParser.Parse(AggregateFunctionCallGraph, CurrentProfilerStream, 0, false, 0 );
            // Set the filename currently being viewed so the user knows which file it is
            Text = "Gameplay Profiler - " + FileName;
            // This will force an update just like previously
		...
		private void DisplayFrame( int SelectedFrame )
		{
			...
			ActorTreeViewParser.Parse(PerActorTreeView, PerClassTreeView, CurrentProfilerStream, SelectedFrame, Threshold);
			FunctionInfo.DumpFunctionInfoMapToList(CurrentProfilerStream,CurrentProfilerStream.Frames[SelectedFrame].NameToFunctionInfoMap, FrameFunctionListView, Threshold, false);
			ClassHierarchyTreeViewParser.Parse(ClassHierarchyTreeView, CurrentProfilerStream, SelectedFrame, Threshold);
			FunctionTreeViewParser.Parse(FrameFunctionCallGraph, CurrentProfilerStream, SelectedFrame, true, 0);
		}
        ...
        void RecursiveGatherFunctions(TreeNode Root, HashSet<string> FuncNames)
        {
            FunctionTreeViewParser.NodePayload payload = Root.Tag as FunctionTreeViewParser.NodePayload;
            FuncNames.Add(payload.FunctionName);
            foreach (TreeNode N in Root.Nodes)
            {
                RecursiveGatherFunctions(N, FuncNames);
            }
        }
