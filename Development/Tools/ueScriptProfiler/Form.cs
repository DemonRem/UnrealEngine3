...
namespace ueScriptProfiler
{
	public class ueScriptProfiler : System.Windows.Forms.Form
	{
		...
		private System.Windows.Forms.MenuItem CopySelectedRows;
		...
		private void InitializeComponent()
		{
			...
			this.CopySelectedRows = new System.Windows.Forms.MenuItem();
			...
			// CopySelectedRows
			// 
			this.CopySelectedRows.Index = 0;
			this.CopySelectedRows.Shortcut = System.Windows.Forms.Shortcut.CtrlC;
			this.CopySelectedRows.Text = "&Copy";
			this.CopySelectedRows.Click += new System.EventHandler(this.CopySelectedRows_Click);
			// 
			// menuItem2
			// 
		...
		/// <summary>
		/// Copies the selected text from the view
		/// </summary>
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void CopySelectedRows_Click(object sender, System.EventArgs e)
		{
			// Grab the highlighted rows from the list view and put on the clip board
			if (tabControl1.SelectedTab == tabSortedView)
			{
				string RowData = new string(' ',0);
				// Append the headers so that the columns make sense
				foreach (ColumnHeader col in SortedView.Columns)
				{
					RowData += col.Text + "\t";
				}
				// Move to next line
				RowData += "\r\n";
				// For each selected row, format that row and append to our string
				foreach (ListViewItem item in SortedView.SelectedItems)
				{
					// Append subitems to the string we'll place on the clipboard
					foreach (ListViewItem.ListViewSubItem subItem in item.SubItems)
					{
						RowData += subItem.Text + "\t";
					}
					// Move to next line
					RowData += "\r\n";
				}
				Clipboard.SetDataObject(RowData);
			}
			// Grab the highlighted tree item
			else if (tabControl1.SelectedTab == tabCallGraph)
			{
				if (CallGraph.SelectedNode != null)
				{
					Clipboard.SetDataObject(CallGraph.SelectedNode.Text + "\r\n");
				}
			}
			// Grab the highlighted tree item
			else if (tabControl1.SelectedTab == tabExpensiveFunctions)
			{
				if (ExpensiveFunctions.SelectedNode != null)
				{
					Clipboard.SetDataObject(ExpensiveFunctions.SelectedNode.Text + "\r\n");
				}
			}
		}
