...
namespace UnrealFrontend
{
	public class WorkerThread
	{
		...
		public void QueueWork( Delegate InWorkFunc, params object[] InWorkParams )
		{
			lock(mWorkQueue)
			{
				mWorkQueue.Enqueue(new WorkItem { FuncToCall = InWorkFunc, Params = InWorkParams });
			}
		...
		{
			this.QueueWork(InVoidTask, null);
		}
		...
		public void QueuePlatformRefresh(Delegate InWorkFunc, params object[] InWorkParams)
		{
			lock (mWorkQueue)
			{
				mPlatformRefreshRequest = new WorkItem { FuncToCall = InWorkFunc, Params = InWorkParams };
			}
			WakeUpWorkerThread.Set();
		}
