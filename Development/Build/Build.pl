...
$developer_devenv                        = 'C:\\Program Files\\Microsoft Visual Studio 8\\Common7\\IDE\\devenv.exe';
...
$visualStudioVersion = "VS2005 SP1 8.0.50727.762"; # the version of visual stuido that we are using
...
$xenonXDKVersion     = "5426"; # the version of the Xenon XDK that we are using

$doBuildPS3          = 1;    # build PS3? effectively 0 if doBuild is 0
$PS3SDKVersion       = "150.004"; # the version of the PS3 SDK that we are using

$doBuildStatsViewer  = 1;    # Build StatsViewer?  Effectively 0 if doBuild is 0.

$doBuildVariousTools  = 1;    # Build our various tools  Effectively 0 if doBuild is 0.

$doBuildPlatformToolsDLLs = 1; # Build <platform>Tools.dll?  Effectively 0 if doBuild is 0.
...
$addrGearExternal      = 'gears-list@epicgames.com';
...
$devenv				 = 'C:\\Program Files (x86)\\Microsoft Visual Studio 8\\Common7\\IDE\\devenv.exe';
...
sub setVersion
{
	...
	$announceSubjectWarfare = $announceSubject;
        $announceSubjectGear = $announceSubject;
...
sub setAnnounceMailHeaderText
{
  ...
  $announceMailHeaderText = 
    ...
    "        VisualStudio version used: $visualStudioVersion\n".
    "    Xenon:     $doBuildXenon\n".
...
sub checkoutAll
{
	foreach $file (@checkoutFiles)
	{
		...
		{
			announceFailure("Perforce checkout","$edit[0]");
...
sub submitAll
{
	...
	# No need to revert unchanged files.  Submitting unchanged files does no harm. 
        ...
        #
        #     -R can be used with 2006.2 p4 and 2006.1 p4d.  this should revert files that have not changed
	#system( "p4 -c $clientspec revert -a 2>&1" );

	open( SUBMITCMD, "|p4 -c \"$clientspec\" submit -f revertunchanged -i > output.txt 2>&1 " );
	...
	if (open(OUTPUT,"output.txt") )
	{
        ...
        # we were not able to open the output.txt so something bad has occurred
	else
	{
		$foundSuccess = 0;
	}
	...
	foreach $change ( @depotChanges )
	{
		...
		if ($change =~ /Change ([0-9]+) on/)
		{
			...
			if ($desc ne "" )
			{
				...
				if ( $wasContentChange == 1 )
				{
					...
					if ( $contentChange[1] == 1 )    {$changelistWarfare{$best_cat} .= $desc."\n";}
                                        ...
                                        elsif ( $contentChange[5] == 1 ) { $changelistGear{$best_cat} .= $desc."\n"; }
				}
				else
				{
			...
			if ($project_desc ne "")
			{
				...
				elsif ($reading_project == 4)
				{
					$changelist{"ExampleGame"} .= $project_desc."\n";
				}
                                elsif ($reading_project == 5)
                                {
                                        $changelist{"Gear"} .= $project_desc."\n";
                                }
			}
...
sub announceGearChanges
{
	...
	while ($success == 0 && $attempts < 10)
	{
		open( SMTP, "|\"$blat\" - -t $addrGearExternal -f $addrFrom -s \"$announceSubjectGear\" -server $mailserver > output.txt" );

		setAnnounceMailHeaderText;
		print SMTP "$announceMailHeaderText";

		# Output Gear code changes.
		if ($changelist{"Gear"} ne "")
		{
			print SMTP "GEARS CODE CHANGES:\n\n";
			print SMTP $changelist{"Gear"}."\n";
			print SMTP $lineSpacer;
		}
                else
                {
                        print SMTP "NO GEARS CODE CHANGES\n\n";
                        print SMTP $lineSpacer;
                }

		# Output Gear content changes.
		%clarg = %changelistGear;
		assembleHashChangeLog( 0, 0 );
		if ( $commonChangeLog ne "" )
		{
			print SMTP "GEARS CONTENT CHANGES:\n\n";
			print SMTP $commonChangeLog;
			print SMTP $lineSpacer;
		}
	...
	if ( open(CHANGELOG,"> ".$buildLogDir."changes_$version.txt") )
	{
		...
		if ( $commonChangeLog ne "" )
		{
			...
			print CHANGELOG $lineSpacer;
                ...
                if ($changelist{"Gear"} ne "")
                {
                        print CHANGELOG "GEARS CODE CHANGES:\n\n";
                        print CHANGELOG $changelist{"Gear"}."\n";
                        print CHANGELOG $lineSpacer;
                }
                ...
                if ( $commonChangeLog ne "" )
                {
                        print CHANGELOG "GEARS CONTENT CHANGES:\n\n";
                        print CHANGELOG $commonChangeLog;
                        print CHANGELOG $lineSpacer;
                }
...
sub parseNativeLog
{
	...
	while (<COMPILELOG>)
	{
		...
		# check for gcc error format (warnings as errors)
		#if ( /Error/ )
		if ( /] Error ([0-9]+)/ )
		{
			if ( $1 != 0 )
...
#############################################################
# compilePS3ToolsDLL
# - compile the PS3Tools.dll
#############################################################

sub compilePS3ToolsDLL
{
	#local $build_game = $_[0];
	print "- Building PS3Tools.dll\n";
	system("\"$devenv\" \"$root_dir\\Development\\Src\\PS3\\PS3Tools\\PS3Tools.sln\" /rebuild \"Release\" /out $buildlog");
...
#############################################################
# compileXeToolsDLL
# - compile the XeTools.dll
#############################################################

sub compileXeToolsDLL
{
	#local $build_game = $_[0];
	print "- Building XeTools.dll\n";
	system("\"$devenv\" \"$root_dir\\Development\\Src\\Xenon\\XeTools\\XeTools.sln\" /rebuild \"Release\" /out $buildlog");
...
#############################################################
# compileUnrealConsole
# - compile the UnrealConsole.exe
#############################################################

sub compileUnrealConsole
{
	#local $build_game = $_[0];
	print "- Building UnrealConsole.exe\n";
	system("\"$devenv\" \"$root_dir\\Development\\Tools\\UnrealConsole\\UnrealConsole.sln\" /rebuild \"Release\" /out $buildlog");
...
#############################################################
# compileUnrealFrontend
# - compile the compileUnrealFrontend exes
#############################################################

sub compileUnrealFrontend
{
	#local $build_game = $_[0];
	print "- Building UnrealFrontend .exes\n";
	system("\"$devenv\" \"$root_dir\\Development\\Tools\\UnrealFrontend\\UnrealFrontend.sln\" /rebuild \"Release\" /out $buildlog");
...
sub setDeveloper
{
	...
	if ( $useDeveloperSettings == 1 )
	{
		...
		print "devenv was $old_devenv, now $devenv\n";
	}
	else
	{
		print "None found\n";
...
sub dobuild
{
	...
	if ( $doBuild == 1 )
	{
		...
		# Build <platform>Tools.dll
		if( $doBuildPlatformToolsDLLs == 1 )
		{
			clock "BUILD PS3TOOLSDLL";

			$compilelog = compilePS3ToolsDLL;
			#print "PS3Tools.dll compilelog is $compilelog\n";

			if( parseNativeLog( $compilelog ) == 0 )
			{
			  # compile failed.  Copy the log over the failed log directory.
			  system( "copy $compilelog $failedLogDir /y" );
			
			  # Unlock if we did a checkout.
			  revertAll();
			
			  announceFailure( "PS3Tools.dll compile has failed", $failedLogDirLink  );
			  return;
			
			}
			unclock "BUILD PS3TOOLSDLL";


			clock "BUILD XETOOLSDLL";

			$compilelog = compileXeToolsDLL;
			#print "XeTools.dll compilelog is $compilelog\n";
			...
			if( parseNativeLog( $compilelog ) == 0 )
			{
			  ...
			  announceFailure( "XeTools.dll compile has failed", $failedLogDirLink  );
			  return;
			
			}
			unclock "BUILD XETOOLSDLL";
   	       }

		# Build our various tool .exe
		if( $doBuildVariousTools == 1 )
		{
			clock "BUILD UNREALCONSOLE";

			$compilelog = compileUnrealConsole;
			#print "compileUnrealConsole compilelog is $compilelog\n";
			...
			if( parseNativeLog( $compilelog ) == 0 )
			{
			  ...
			  announceFailure( "UnrealConsole.exe compile has failed", $failedLogDirLink  );
			  return;
			
			}
			unclock "BUILD UNREALCONSOLE";


			clock "BUILD UNREALFRONTEND";

			$compilelog = compileUnrealFrontend;
			#print "compileUnrealFrontend compilelog is $compilelog\n";
			...
			if( parseNativeLog( $compilelog ) == 0 )
			{
			  ...
			  announceFailure( "UnrealFrontend compile has failed", $failedLogDirLink  );
			  return;
			
			}
			unclock "BUILD UNREALFRONTEND";
		}
		...
		if( $doBuildPS3 == 1 )
		{
		    ...
		    clock "BUILD CPP FOR PS3 EXAMPLEGAME";
		    ...
		    unclock "BUILD CPP FOR PS3 EXAMPLEGAME";


		    clock "BUILD CPP FOR PS3 UTGAME";
		    ...
		    unclock "BUILD CPP FOR PS3 UTGAME";
	...
	clock "CALL_PRECOMPILEDSHADER_BAT";
	system( "$root_dir\\Development\\Build\\TriggerBuild_PreCompileShaders_Full.bat" );
	unclock "CALL_PRECOMPILEDSHADER_BAT";
...
sub parseCommandLineArgs
{
	...
	#	    $doAQABuild = 1;
		...
		elsif( $curArg eq 'ps3CompileTest' )
		{
		  setDeveloper();
		  setVersion();
