...
public:
	...
	inline static FMVector4 ToVector4(const string& value, float lengthFactor=1.0f) { return ToVector4(value.c_str(), lengthFactor); } /**< See above. */
#ifdef UNICODE
	...
	static fstring ToFString(const FMVector4& p, float lengthFactor=1.0f);
	...
	/** Converts a 2D vector into a string.
		...
		@param lengthFactor An optional factor that will scale the vector.
		@return The string containing the converted vector. */
	static string ToString(const FMVector2& value, float lengthFactor=1.0f);
	static fstring ToFString(const FMVector2& value, float lengthFactor=1.0f);
