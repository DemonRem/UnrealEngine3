...
FMVector4 FUStringConversion::ToVector4(const fchar** value, float lengthFactor)
{
	FMVector4 p;
	if(value != NULL && *value != NULL && **value != 0)
	{
		...
		p.y = ToFloat(value) * lengthFactor;
		p.z = ToFloat(value) * lengthFactor;
		p.w = ToFloat(value) * lengthFactor;
	}
	return p;
}
...
FMVector4 FUStringConversion::ToVector4(const char** value, float lengthFactor)
{
	FMVector4 p;
	if(value != NULL && *value != NULL && **value != 0)
	{
		...
		//in the special case where we try to read 3 components, the last one should often be 1.0f
		//ex: alpha value
		if (*value == NULL || **value == 0)
			p.w = 1.0f;
		else
...
void FUStringConversion::ToString(FUSStringBuilder& builder, const FMVector2& p, float lengthFactor)
{
	VAL(p.u * lengthFactor); SPACE; VAL(p.v * lengthFactor);
}

void FUStringConversion::ToFString(FUStringBuilder& builder, const FMVector2& p, float lengthFactor)
{
	VAL(p.u * lengthFactor); SPACE; VAL(p.v * lengthFactor);
}

string FUStringConversion::ToString(const FMVector2& p, float lengthFactor)
{
	globalSBuilder.clear();
...
fstring FUStringConversion::ToFString(const FMVector2& p, float lengthFactor)
{
	globalBuilder.clear();
...
{
	VAL(p.x * lengthFactor); SPACE; VAL(p.y * lengthFactor); SPACE; VAL(p.z * lengthFactor); SPACE; VAL(p.w * lengthFactor);
}

void FUStringConversion::ToFString(FUStringBuilder& builder, const FMVector4& p, float lengthFactor)
{
	VAL(p.x * lengthFactor); SPACE; VAL(p.y * lengthFactor); SPACE; VAL(p.z * lengthFactor); SPACE; VAL(p.w * lengthFactor);
}
...
fstring FUStringConversion::ToFString(const FMVector4& p, float lengthFactor)
{
	globalBuilder.clear();
	ToFString(globalBuilder, p, lengthFactor);
	return globalBuilder.ToString();
