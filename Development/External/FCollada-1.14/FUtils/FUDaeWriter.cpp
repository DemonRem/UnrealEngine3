...
namespace FUDaeWriter
{
	...
	const char* FUDaeAccessor::XY[] = { "X", "Y", 0 };
	...
	xmlNode* AddArray(xmlNode* parent, const char* id, const FMVector2List& values, float lengthFactor)
	{
		// Reserve the necessary space within the string builder
		globalSBuilder.clear();
		size_t valueCount = values.size();
		globalSBuilder.reserve(valueCount * 2 * FLOAT_STR_ESTIMATE);
		if (valueCount > 0)
		{
		...
		return AddArray(parent, id, DAE_FLOAT_ARRAY_ELEMENT, globalSBuilder.ToCharPtr(), valueCount * 2);
	}
	...
	xmlNode* AddArray(xmlNode* parent, const char* id, const FMVector4List& values, float lengthFactor)
	{
		// Reserve the necessary space within the string builder
		globalSBuilder.clear();
		size_t valueCount = values.size();
		globalSBuilder.reserve(valueCount * 4 * FLOAT_STR_ESTIMATE);
		if (valueCount > 0)
		{
			// Write out the values
		...
		// Create the typed array node.
		return AddArray(parent, id, DAE_FLOAT_ARRAY_ELEMENT, globalSBuilder.ToCharPtr(), valueCount * 4);
	}
	...
	xmlNode* AddSourcePosition(xmlNode* parent, const char* id, const FMVector2List& values, float lengthFactor)
	{
		xmlNode* sourceNode = AddChild(parent, DAE_SOURCE_ELEMENT);
		AddAttribute(sourceNode, DAE_ID_ATTRIBUTE, id);
		FUSStringBuilder arrayId(id); arrayId.append("-array");
	...
	xmlNode* AddSourcePosition(xmlNode* parent, const char* id, const FMVector4List& values, float lengthFactor)
	{
		xmlNode* sourceNode = AddChild(parent, DAE_SOURCE_ELEMENT);
		AddAttribute(sourceNode, DAE_ID_ATTRIBUTE, id);
		FUSStringBuilder arrayId(id); arrayId.append("-array");
	...
	fstring CleanName(const fchar* c)
	{
		...
		{
			// First character: alphabetic or '_'.
			if ((*c >= 'a' && *c <= 'z') || (*c >= 'A' && *c <= 'Z') || *c == '_') globalBuilder += *c;
			...
			for (++c; *c != 0; ++c)
			{
				if ((*c >= 'a' && *c <= 'z') || (*c >= 'A' && *c <= 'Z') || (*c >= '0' && *c <= '9') || *c == '_' || *c == '-') globalBuilder += *c;
				else { globalBuilder += '_'; }
			}
		}
	...
	string AddNodeSid(xmlNode* node, const char* wantedSid)
	{
	...
	void AddNodeSid(xmlNode* node, string& subId)
	{
	...
	{
		subId = TO_FSTRING(AddNodeSid(node, TO_STRING(subId).c_str()));
	}
#endif
};
