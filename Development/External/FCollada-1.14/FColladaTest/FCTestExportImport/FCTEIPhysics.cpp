...
namespace FCTestExportImport
{
	...
	static string physicsMaterialId1 = "PMat";
	static string physicsMaterialId2 = "PMat";
	static string physicsMaterialId3 = "PMat";
	...
	static string physicsModelId1, physicsModelId2, physicsModelId3;
	...
	{
		rigidBody->AddParameter(DAE_MASS_ELEMENT, 0.5f);
		return true;
	}

	bool CheckPhysicsRigidBody(FULogFile& fileOut, FCDPhysicsRigidBody* rigidBody)
	{
		FCDPhysicsParameter<float>* parameter = (FCDPhysicsParameter<float>*) rigidBody->FindParameterByReference(DAE_MASS_ELEMENT);
		PassIf(IsEquivalent(*parameter->GetValue(), 0.5f));
		return true;
	}
	...
	{
		PassIf(rigidConstraint->GetEnabled() == false);
		PassIf(rigidConstraint->GetInterpenetrate() == false)
		return true;
	}
};
