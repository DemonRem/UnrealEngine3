...
namespace FCTestExportImport
{
	...
	bool FillGeometryMesh(FULogFile& fileOut, FCDGeometryMesh* mesh)
	{
		...
		PassIf(FillExtraTree(fileOut, dummySource->GetExtra()));
		...
		PassIf(pInput1 != NULL && pInput1->GetSource() == colorSource && pInput1->idx == 1);
		FCDGeometryPolygonsInput* pInput2 = polys2->AddInput(colorSource, 1);
		PassIf(polys2->GetInputCount() == 2);
		PassIf(pInput2 != NULL && pInput2->GetSource() == colorSource && pInput2->idx == 1);
		FCDGeometryPolygonsInput* pInput3 = polys1->AddInput(dummySource, 2);
		PassIf(pInput3 != NULL && pInput3->GetSource() == dummySource && pInput3->idx == 2);
	...
	bool FillGeometrySpline(FULogFile& UNUSED(fileOut), FCDGeometrySpline* spline)
	{
		spline->SetType(FUDaeSplineType::BEZIER);
		spline->AddSpline(new FCDBezierSpline(spline->GetDocument()));
		/*
		FCDCVs cvs;
	...
	bool CheckGeometryMesh(FULogFile& fileOut, FCDGeometryMesh* mesh)
	{
		...
		PassIf(CheckExtraTree(fileOut, dummySource->GetExtra()));
