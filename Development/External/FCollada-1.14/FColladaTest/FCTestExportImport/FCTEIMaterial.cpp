...
static string surfaceParameterImageId;
...
namespace FCTestExportImport
{
	...
	bool FillEffectStandard(FULogFile& fileOut, FCDEffectStandard* profile)
	{
		...
		FCDETechnique* technique1 = texture1->GetExtra()->AddTechnique(DAEMAYA_MAYA_PROFILE);
		...
		FCDETechnique* technique2 = texture2->GetExtra()->AddTechnique(DAEMAYA_MAYA_PROFILE);
	...
	bool FillEffectFX(FULogFile& fileOut, FCDEffectProfileFX* profile)
	{
		...
		FCDEffectParameterBool* booleanParameter = (FCDEffectParameterBool*) profile->GetParameters()->AddParameter(FCDEffectParameter::BOOLEAN);
		booleanParameter->SetSemantic("BOOL_P");
		booleanParameter->SetValue(true);
		FCDEffectParameterFloat2* floatParameter = (FCDEffectParameterFloat2*) technique->GetParameterList()->AddParameter(FCDEffectParameter::FLOAT2);
		floatParameter->SetSemantic("FLOAT2_P");
		floatParameter->SetValueX(1.5f);
		floatParameter->SetValueY(5.2f);
		...
		FCDEffectParameterSurface* surface = (FCDEffectParameterSurface*) profile->GetParameters()->AddParameter(FCDEffectParameter::SURFACE);
		...
		FCDEffectParameterSampler* sampler = (FCDEffectParameterSampler*) profile->GetParameters()->AddParameter(FCDEffectParameter::SAMPLER);
	...
	bool CheckEffectStandard(FULogFile& fileOut, FCDEffectStandard* profile)
	{
		...
		FCDETechnique* technique1 = texture1->GetExtra()->FindTechnique(DAEMAYA_MAYA_PROFILE);
		FCDETechnique* technique2 = texture2->GetExtra()->FindTechnique(DAEMAYA_MAYA_PROFILE);
	...
	bool CheckEffectFX(FULogFile& fileOut, FCDEffectProfileFX* profile)
	{
		...
		PassIf(profile->GetParameters()->size() == 3);
		...
		{
			FCDEffectParameter* p = profile->GetParameters()->at(i);
		...
		PassIf(technique->GetParameterList()->size() == 1);
		FCDEffectParameter* float2Parameter = technique->GetParameterList()->front();
		FailIf(float2Parameter == NULL);
		PassIf(float2Parameter->GetType() == FCDEffectParameter::FLOAT2);
		PassIf(IsEquivalent(((FCDEffectParameterFloat2*) float2Parameter)->GetValueX(), 1.5f));
		PassIf(IsEquivalent(((FCDEffectParameterFloat2*) float2Parameter)->GetValueY(), 5.2f));
