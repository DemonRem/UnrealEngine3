...
static string transformChildId = "TMChild";
...
namespace FCTestExportImport
{
	...
	bool FillVisualScene(FULogFile& fileOut, FCDSceneNode* scene)
	{
		...
		PassIf(controllerInstance->GetType() == FCDEntityInstance::GEOMETRY);
	...
	bool FillGeometryInstance(FULogFile& fileOut, FCDGeometryInstance* instance)
	{
		...
		size_t materialCount = instance->GetDocument()->GetMaterialLibrary()->GetMaterialCount();
		...
		for (size_t i = 0; i < mesh->GetPolygonsCount(); ++i)
		{
			...
			FCDMaterial* material = instance->GetDocument()->GetMaterialLibrary()->GetMaterial(rand() % materialCount);
	...
	bool CheckVisualScene(FULogFile& fileOut, FCDSceneNode* imported)
	{
		...
		vector<FCDEntityInstance*> instances;
		...
		bool foundLight = false, foundCamera = false, foundMesh = false, foundController = false;
		...
		PassIf(foundLight && foundCamera && foundMesh && foundController);
		return true;
	}
	...
	bool CheckGeometryInstance(FULogFile& fileOut, FCDGeometryInstance* instance)
	{
		...
		for (size_t i = 0; i < 3; ++i)
		{
			...
			else if (b->semantic == "Toto") { PassIf(!foundToto); foundToto = true; PassIf(b->target == "Targeted"); }
			else if (b->semantic == "Gallant") { PassIf(!foundGallant); foundGallant = true; PassIf(b->target == "Pierto"); }
			else Fail;
		}
