...
void FCDSkinController::AddJoint(FCDSceneNode* joint, const FMMatrix44& bindPose)
{
	...
	if (joint != NULL) joint->SetJointFlag(true);
	SetDirtyFlag();
}
...
FUStatus FCDSkinController::LoadFromXML(xmlNode* skinNode)
{
	...
	if (isCollada1_3)
	{
		...
		FCDGeometry* clonedTarget = ((FCDGeometry&)*target).Clone(bindShapePositions, positionStride, bindShapeNormals, normalsStride);
		clonedTarget->SetDaeId(target->GetDaeId() + "-cloned");
		GetDocument()->GetGeometryLibrary()->AddEntity(clonedTarget);
		target = clonedTarget;
...
xmlNode* FCDSkinController::WriteToXML(xmlNode* parentNode) const
{
	...
	string bindShapeMatrixString = FUStringConversion::ToString(bindShapeTransform, GetDocument()->GetLengthUnitConversion());
	...
	AddSourceMatrix(skinNode, jointBindSourceId.ToCharPtr(), jointBindPoses, GetDocument()->GetLengthUnitConversion());
