...
public:
	...
	FCDAnimationCurve* Clone(bool includeClips = true);
	...
	inline void SetTargetQualifier(const string& q) { targetQualifier = q; SetDirtyFlag(); }

	/** [INTERNAL] Updates the offset for a given animation clip.
		@param offset The new offset. 
		@param clip The animation clip to associate with the offset. */
	void SetClipOffset(const float offset, const FCDAnimationClip* clip);
};
