...
FUStatus FCDGeometryPolygonsSpecial::LoadFromXML(xmlNode* baseNode)
{
	FUStatus status;
	...
	{
		return status.Warning(FS("Primitive count for mesh node doesn't match actual number of primitives found in <p> element(s) in geometry: ") + TO_FSTRING(parent->GetDaeId()), baseNode->line);
	}
