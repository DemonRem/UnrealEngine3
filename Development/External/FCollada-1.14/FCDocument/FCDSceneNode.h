...
typedef vector<FCDEntityInstance*> FCDEntityInstanceList; /**< A dynamically-sized array of entity instances. */
...
private:
	...
	// An optional asset attached to the node
	FCDAsset* asset;
...
public:
	...
	inline FCDAsset* GetAsset() const { return asset; }
	...
	/** Copies the entity information into a clone.
		...
		@param clone The empty clone. If this pointer is NULL, a new entity
			will be created and you will need to release this pointer manually.
		@return The clone. */
	virtual FCDEntity* Clone(FCDEntity* clone = NULL) const;
