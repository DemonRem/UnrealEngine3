...
public:
	...
	void ParseVersionNumbers(string v);

	/** Comparison functions for version numbers.
		@param v A string containing the version numbers
		@return The result of the comparison */
	bool IsLowerThan(string v) const;
	bool IsHigherThan(string v) const;
	bool Is(string v) const;
};
...
typedef map<FCDAnimated*, FCDAnimated*> FCDAnimatedSet; /**< A set of animated values. */
...
public:
	...
	void SetStartTime(float time) { startTime = time; hasStartTime = true; UpdateAnimationClips(time); }
	...
	/** Retrieves a specific entity layer contained within the document.
		...
		@return The entity layer. This pointer will be NULL if the index
			is out-of-bounds. */
	FCDLayer* GetLayer(size_t index) { FUAssert(index < GetLayerCount(), return NULL); return layers.at(index); }
	const FCDLayer* GetLayer(size_t index) const { FUAssert(index < GetLayerCount(), return NULL); return layers.at(index); }
	...
	FCDForceFieldLibrary* GetForceFieldLibrary() { return forceFieldLibrary; }
	const FCDForceFieldLibrary* GetForceFieldLibrary() const { return forceFieldLibrary; }
	...
	FCDEntity* FindEntity(const string& daeId);
	...
	FCDLayer* FindLayer(const string& name);
	...
	FCDPhysicsScene* FindPhysicsScene(const string& daeId);
	...
	/** Retrieves the force field that matches the given COLLADA id. 
		@param daeId A valid COLLADA id.
		@return The visual force field. This pointer will be NULL if no matching force field was found. */
	FCDForceField* FindForceField(const string& daeId);
...
private:
	void UpdateAnimationClips(const float newStart);
};
