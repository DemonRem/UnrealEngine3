...
public:
	...
	void SetSourceData(const float* _sourceData, uint32 _sourceStride, size_t count, size_t offset=0);

	/** Sets the stride for the source data.
		@param stride The stride for the source data. */
	void SetSourceStride(uint32 stride) { sourceStride = stride; SetDirtyFlag(); }
	...
	void SetSourceNode(xmlNode* _sourceNode) { sourceNode = _sourceNode; SetDirtyFlag(); }
