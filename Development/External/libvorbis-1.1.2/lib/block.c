...
int vorbis_block_init( vorbis_dsp_state* v, vorbis_block* vb )
{
	...
	if( v->analysisp )
	{
		...
		for( i = 0; i < PACKETBLOBS; i++ )
		{
			...
			{
				vbi->packetblob[i] = ( oggpack_buffer* )_ogg_calloc( 1, sizeof( oggpack_buffer ) );
			}
...
void* _vorbis_block_alloc( vorbis_block* vb, long bytes )
{
	...
	if( bytes + vb->localtop > vb->localalloc )
	{
		...
		/* highly conservative */
		vb->localalloc = bytes;
		vb->localstore = ( void* )_ogg_malloc( vb->localalloc );
		vb->localtop = 0;
	}
...
void _vorbis_block_ripcord(vorbis_block *vb)
{
	...
	if(vb->totaluse)
	{
		vb->localstore = ( void* )_ogg_realloc( vb->localstore, vb->totaluse + vb->localalloc );
...
static int _vds_shared_init( vorbis_dsp_state* v, vorbis_info* vi, int encp )
{
	...
	/* MDCT is tranform 0 */
	b->transform[0][0] = ( vorbis_look_transform* )_ogg_calloc( 1, sizeof( mdct_lookup ) );
	b->transform[1][0] = ( vorbis_look_transform* )_ogg_calloc( 1, sizeof( mdct_lookup ) );
	...
	{ 
		...
		for( i = 0; i < ci->psys; i++ )
		{
			_vp_psy_init( b->psy + i, ci->psy_param[i], &ci->psy_g_param, ci->blocksizes[ci->psy_param[i]->blockflag] / 2, vi->rate );
		}
...
int vorbis_analysis_init( vorbis_dsp_state* v, vorbis_info* vi )
{
	...
	/* added by aoyumi */
	b->nblock = ( float* )_ogg_calloc( ( 128 * vi->channels ), sizeof( *b->nblock ) );
	b->tblock = ( float* )_ogg_calloc( ( 128 * vi->channels ), sizeof( *b->tblock ) );
	b->lownoise_compand_level = ( float* )_ogg_calloc( vi->channels, sizeof( *b->lownoise_compand_level ) );
...
void vorbis_dsp_clear( vorbis_dsp_state* v )
{
	...
	if( v )
	{
		...
		if( b )
		{
			...
			if( b->lownoise_compand_level ) 
			{
				_ogg_free( b->lownoise_compand_level );
			}
...
static void _preextrapolate_helper( vorbis_dsp_state* v )
{
	...
	if( v->pcm_current - v->centerW > 32 * 2 )
	{ 
		/* safety */
		for( i = 0; i < v->vi->channels; i++ )
		{
			...
			/* prime as above */
			vorbis_lpc_from_data( work, lpc, v->pcm_current - v->centerW, 32 );

			/* run the predictor filter */
			vorbis_lpc_predict( lpc, work + v->pcm_current - v->centerW - 32, 32, work + v->pcm_current - v->centerW, v->centerW );
	...
	if( vals <= 0 )
	{
		...
		for( i = 0; i < vi->channels; i++ )
		{
			if( v->eofflag > 32 * 2 )
			{
				...
				vorbis_lpc_from_data( v->pcm[i] + v->eofflag - n, lpc, n, 32 );

				/* run the predictor filter */
				vorbis_lpc_predict( lpc, v->pcm[i] + v->eofflag - 32, 32, v->pcm[i] + v->eofflag, v->pcm_current - v->eofflag );
			}
			else
			{
...
int vorbis_analysis_blockout( vorbis_dsp_state* v, vorbis_block* vb )
{
	...
	long beginW = v->centerW - ci->blocksizes[v->W] / 2, centerNext, bp, blockbound;
	...
	bp = _ve_envelope_search( v );
	if( bp == -1 )
	{
		if( v->eofflag == 0 )
		{
			/* not enough data currently to search for a full long block */
			return( 0 ); 
		}
	...
	blockbound = centerNext + ci->blocksizes[v->nW] / 2;
	if( v->pcm_current < blockbound )
	{
		/* not enough data yet; although this check is less strict that the _ve_envelope_search,
	...
	for( i = 0; i < vi->channels; i++ )
	{
		vbi->pcmdelay[i] = _vorbis_block_alloc( vb, ( vb->pcmend + beginW ) * sizeof( *vbi->pcmdelay[i] ) );
#ifdef __SSE__											/* SSE Optimize */
		{
			float* d = ( float* )( vbi->pcmdelay[i] );
			float* s = ( float* )( v->pcm[i] );
			xmm_copy_forward( d, s, vb->pcmend + beginW );
		}
#else													/* SSE Optimize */
	...
	/* advance storage vectors and clean up */
	{
		...
		if( movementW > 0 )
		{
...
#ifdef __SSE__											/* SSE Optimize */
			{
				...
				if( s >= d )
				{
					xmm_copy_forward( d, s, v->pcm_current );
				}
				else
				{
					xmm_copy_backward( d, s, v->pcm_current );
				}
#else													/* SSE Optimize */
...
/* Unlike in analysis, the window is only partially applied for each block.  The time domain envelope is not yet handled at the point of
...
int vorbis_synthesis_blockin( vorbis_dsp_state* v, vorbis_block* vb )
{
	...
	if( vb->pcm )
	{
		...
		/* v->pcm is now used like a two-stage double buffer.  We don't want to have to constantly shift *or* adjust memory usage.  Don't
		...
		/* deal with initial packet state; we do this using the explicit pcm_returned==-1 flag otherwise we're sensitive to first block
		   being short or long */
		if( v->pcm_returned == -1 )
		{
		...
		{ 
			...
			if( b->sample_count > v->granulepos )
			{
				...
				if( vb->eofflag )
				{
					...
					/* granulepos could be -1 due to a seek, but that would result in a long count, not short count */
...
/* intended for use with a specific vorbisfile feature; we want access to the [usually synthetic/postextrapolated] buffer and lapping at
   the end of a decode cycle, specifically, a half-short-block worth. This funtion works like pcmout above, except it will also expose
...
int vorbis_synthesis_lapout( vorbis_dsp_state* v, float*** pcm )
{
	...
	/* centerW was advanced by blockin; it would be the center of the *next* block */
	if( v->centerW == n1 )
	{
