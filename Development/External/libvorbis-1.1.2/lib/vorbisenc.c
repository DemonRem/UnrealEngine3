...
/* high level configuration information for setting things up step-by-step with the detailed vorbis_encode_ctl interface.
   There's a fair amount of redundancy such that interactive setup does not directly deal with any vorbis_info or codec_setup_info
   initialization; it's all stored (until full init) in this highlevel setup, then flushed out to the real codec setup structs later. */
...
static void vorbis_encode_floor_setup( vorbis_info* vi, double s, int block, static_codebook*** books, vorbis_info_floor1* in, int* x )
{
	int i, k, is = ( int )s;
...
static void vorbis_encode_global_psych_setup( vorbis_info* vi, double s, vorbis_info_psy_global* in, double* x )
{
	int i, is = ( int )s;
	double ds = s - is;
	codec_setup_info* ci = vi->codec_setup;
	...
	for( i = 0; i < 4; i++ )
	{
		g->preecho_thresh[i] = ( float )( in[is].preecho_thresh[i] * ( 1.0f - ds ) + in[is + 1].preecho_thresh[i] * ds );
		g->postecho_thresh[i] = ( float )( in[is].postecho_thresh[i] * ( 1.0f - ds ) + in[is + 1].postecho_thresh[i] * ds );
	}

	g->ampmax_att_per_sec = ( float )ci->hi.amplitude_track_dBpersec;
}

static void vorbis_encode_global_stereo( vorbis_info* vi, highlevel_encode_setup* hi, adj_stereo* p )
{
	...
	if( p )
	{
		...
		if( hi->managed )
		{
			...
			{
				float kHz = p[is].kHz[i] * ( 1.0f - ds ) + p[is + 1].kHz[i] * ds;
				g->coupling_pointlimit[0][i] = ( int )( kHz * 1000.0f / vi->rate * ci->blocksizes[0] );
				g->coupling_pointlimit[1][i] = ( int )( kHz * 1000.0f / vi->rate * ci->blocksizes[1] );
				g->coupling_pkHz[i] = ( int )kHz;

				kHz = p[is].lowpasskHz[i] * ( 1.0f - ds ) + p[is + 1].lowpasskHz[i] * ds;
				g->sliding_lowpass[0][i] = ( int )( kHz * 1000.0f / vi->rate * ci->blocksizes[0] );
				g->sliding_lowpass[1][i] = ( int )( kHz * 1000.0f / vi->rate * ci->blocksizes[1] );
			}
		}
		else
		{
			float kHz = p[is].kHz[PACKETBLOBS / 2] * ( 1.0f - ds ) + p[is + 1].kHz[PACKETBLOBS / 2] * ds;
			for( i = 0; i < PACKETBLOBS; i++ )
			{
				g->coupling_pointlimit[0][i] = ( int )( kHz * 1000.0f / vi->rate * ci->blocksizes[0] );
				g->coupling_pointlimit[1][i] = ( int )( kHz * 1000.0f / vi->rate * ci->blocksizes[1] );
			...
			kHz = p[is].lowpasskHz[PACKETBLOBS / 2] * ( 1.0f - ds ) + p[is + 1].lowpasskHz[PACKETBLOBS / 2] * ds;
			for( i = 0; i < PACKETBLOBS; i++ )
			{
				g->sliding_lowpass[0][i] = ( int )( kHz * 1000.0f / vi->rate * ci->blocksizes[0] );
				g->sliding_lowpass[1][i] = ( int )( kHz * 1000.0f / vi->rate * ci->blocksizes[1] );
...
static void vorbis_encode_psyset_setup( vorbis_info* vi, double s, int* nn_start, int* nn_partition, double* nn_thresh, int block )
{
	codec_setup_info* ci = vi->codec_setup;
...
static void vorbis_encode_tonemask_setup( vorbis_info* vi, double s, int block, att3* att, int* max, vp_adjblock* in )
{
	int i, is = ( int )s;
	float ds = ( float )s - is;
	codec_setup_info* ci = vi->codec_setup;
	...
	p->tone_masteratt[0] = att[is].att[0] * ( 1.0f - ds ) + att[is + 1].att[0] * ds;
	p->tone_masteratt[1] = att[is].att[1] * ( 1.0f - ds ) + att[is + 1].att[1] * ds;
	p->tone_masteratt[2] = att[is].att[2] * ( 1.0f - ds ) + att[is + 1].att[2] * ds;
	p->tone_centerboost = att[is].boost * ( 1.0f - ds ) + att[is + 1].boost * ds;
	p->tone_decay = att[is].decay * (1.0f - ds ) + att[is + 1].decay * ds;

	p->max_curve_dB = max[is] * ( 1.0f - ds ) + max[is + 1] * ds;

	for( i = 0; i < P_BANDS; i++ )
	{
		p->toneatt[i] = in[is].block[i] * ( 1.0f - ds ) + in[is + 1].block[i] * ds;
	}
}

static void vorbis_encode_compand_setup( vorbis_info* vi, double s, int block, compandblock* in, double* x )
{
	int i, is = ( int )s, ishcm, hcm_stop = 5; // high compander limit
	...
	{
		p->noisecompand[i] = ( float )( in[is].data[i] * ( 1.0 - ds ) + in[is + 1].data[i] * ds );
	}
	...
	if( s < ( float )hcm_stop )
	{
		for( i = 0; i < NOISE_COMPAND_LEVELS; i++ )
		{
			p->noisecompand_high[i] = ( float )( in[ishcm].data[i] * ( 1.0 - dshcm ) + in[ishcm + 1].data[i] * dshcm );
		}
	}
	else
	{ 
		/* disable high compander */
		for( i = 0; i < NOISE_COMPAND_LEVELS; i++ )
		{
			p->noisecompand_high[i] = 0.0f;
		}
	}
}

static void vorbis_encode_peak_setup( vorbis_info* vi, double s, int block, int* suppress )
{
	int is = ( int )s;
	double ds = s - is;
	codec_setup_info* ci = vi->codec_setup;
	...
	p->tone_abs_limit = ( float )( suppress[is] * ( 1.0 - ds ) + suppress[is + 1] * ds );
}

static void vorbis_encode_noisebias_setup( vorbis_info* vi, double s, int block, int* suppress, noise3* in, noiseguard* guard, double userbias )
{
	int i, is = ( int )s, j;
	double ds = s - is;
	codec_setup_info* ci = vi->codec_setup;
	...
	p->noisemaxsupp = ( float )( suppress[is] * ( 1.0 - ds ) + suppress[is + 1] * ds );
	...
	for( j = 0; j < P_NOISECURVES; j++ )
	{
		for( i = 0; i < P_BANDS; i++ )
		{
			p->noiseoff[j][i] = ( float )( in[is].data[j][i] * ( 1.0 - ds ) + in[is + 1].data[j][i] * ds );
		}
	}
...
static void vorbis_encode_blocksize_setup( vorbis_info* vi, double s, int* shortb, int* longb )
{
	codec_setup_info* ci = vi->codec_setup;
	int is = ( int )s;
...
static void vorbis_encode_residue_setup( vorbis_info* vi, int number, int block, vorbis_residue_template* res )
{
	codec_setup_info* ci = vi->codec_setup;
	int i, n;

	vorbis_info_residue0* r = ( vorbis_info_residue0* )_ogg_malloc( sizeof( *r ) );
	...
	{
		double freq = ci->hi.lowpass_kHz * 1000.0;
		/* by convention */
		vorbis_info_floor1* f = ci->floor_param[block]; 
		double nyq = vi->rate / 2.0;
		...
		/* in the floor, the granularity can be very fine; it doesn't alter	the encoding structure, only the samples used to fit the floor approximation */
		f->n = ( int )( freq / nyq * blocksize );  

		/* this res may by limited by the maximum pointlimit of the mode, not the lowpass. the floor is always lowpass limited. */
		if( res->limit_type )
		{
			if( ci->hi.managed )
			{
				freq = ci->psy_g_param.coupling_pkHz[PACKETBLOBS - 1] * 1000.0;
			}
			else
			{
				freq = ci->psy_g_param.coupling_pkHz[PACKETBLOBS / 2] * 1000.0;
			}
		...
		/* in the residue, we're constrained, physically, by partition boundaries.  We still lowpass 'wherever', but we have to round up
		here to next boundary, or the vorbis spec will round it *down* to previous boundary in encode/decode */
		if( ci->residue_type[block] == 2 )
		{
			/* round up only if we're well past */
			r->end = ( int )( ( freq / nyq * blocksize * 2 ) / r->grouping + 0.9 ) * r->grouping;
		}
		else
		{
			/* round up only if we're well past */
			r->end = ( int )( ( freq / nyq * blocksize ) / r->grouping + 0.9 ) * r->grouping;
		}
	}
}      

/* we assume two maps in this encoder */
static void vorbis_encode_map_n_res_setup( vorbis_info* vi, double s, vorbis_mapping_template* maps )
{
	codec_setup_info* ci = vi->codec_setup;
	...
	for( i = 0; i < modes; i++ )
	{
		ci->map_param[i] = ( vorbis_info_mapping* )_ogg_calloc( 1, sizeof( *map ) );
...
static double setting_to_approx_bitrate( vorbis_info* vi )
{
	...
	return( ( r[is] * ( 1.0 - ds ) + r[is + 1] * ds ) * ch );  
}

static void get_setup_template( vorbis_info* vi, long ch, long srate, double req, int q_or_bitrate )
{
	int i = 0, j;
	codec_setup_info* ci = vi->codec_setup;
	highlevel_encode_setup* hi = &ci->hi;
	...
	while( setup_list[i] )
	{
		if( setup_list[i]->coupling_restriction == -1 || setup_list[i]->coupling_restriction == ch )
		{
			if( srate >= setup_list[i]->samplerate_min_restriction && srate <= setup_list[i]->samplerate_max_restriction )
			{
				...
				double* map = ( q_or_bitrate ? setup_list[i]->rate_mapping : setup_list[i]->quality_mapping );
...
/* encoders will need to use vorbis_info_init beforehand and call vorbis_info clear when all done */

/* two interfaces; this, more detailed one, and later a convenience layer on top */
...
int vorbis_encode_setup_init( vorbis_info* vi )
{
	...
	/* again, bound this to avoid the app shooting itself int he foot too badly */
	if( hi->amplitude_track_dBpersec > 0.0 )
	{
		hi->amplitude_track_dBpersec = 0.0;
	}

	if( hi->amplitude_track_dBpersec < -99999.0 )
	{
		hi->amplitude_track_dBpersec = -99999.0;
	}
	...
	hi->set_in_stone = 1;
	/* choose block sizes from configured sizes as well as paying attention to long_block_p and short_block_p.  If the configured
	short and long blocks are the same length, we set long_block_p and unset short_block_p */
	vorbis_encode_blocksize_setup( vi, hi->base_setting, setup->blocksize_short, setup->blocksize_long );
	if( ci->blocksizes[0] == ci->blocksizes[1] )
	{ 
		singleblock = 1; 
	}
	...
	vorbis_encode_floor_setup( vi, hi->short_setting, 0, setup->floor_books, setup->floor_params, setup->floor_short_mapping );
	if( !singleblock )
	{
	...
	vorbis_encode_global_psych_setup( vi, hi->trigger_setting, setup->global_params, setup->global_mapping );
	vorbis_encode_global_stereo( vi, hi, setup->stereo_modes );

	/* basic psych setup and noise normalization */
	vorbis_encode_psyset_setup( vi, hi->short_setting, setup->psy_noise_normal_start[0], setup->psy_noise_normal_partition[0], setup->psy_noise_normal_thresh, 0 );
	vorbis_encode_psyset_setup( vi, hi->short_setting, setup->psy_noise_normal_start[0], setup->psy_noise_normal_partition[0], setup->psy_noise_normal_thresh, 1 );
	if( !singleblock )
	{
		vorbis_encode_psyset_setup( vi, hi->long_setting, setup->psy_noise_normal_start[1], setup->psy_noise_normal_partition[1], setup->psy_noise_normal_thresh, 2 );
		vorbis_encode_psyset_setup( vi, hi->long_setting, setup->psy_noise_normal_start[1], setup->psy_noise_normal_partition[1], setup->psy_noise_normal_thresh, 3 );
	}

	/* tone masking setup */
	vorbis_encode_tonemask_setup( vi, hi->block[i0].tone_mask_setting, 0, setup->psy_tone_masteratt, setup->psy_tone_0dB, setup->psy_tone_adj_impulse );
	vorbis_encode_tonemask_setup( vi, hi->block[1].tone_mask_setting, 1, setup->psy_tone_masteratt, setup->psy_tone_0dB, setup->psy_tone_adj_other );
	if( !singleblock )
	{
		vorbis_encode_tonemask_setup( vi, hi->block[2].tone_mask_setting, 2, setup->psy_tone_masteratt, setup->psy_tone_0dB, setup->psy_tone_adj_other );
		vorbis_encode_tonemask_setup( vi, hi->block[3].tone_mask_setting, 3, setup->psy_tone_masteratt, setup->psy_tone_0dB, setup->psy_tone_adj_long );
	}

	/* noise companding setup */
	vorbis_encode_compand_setup( vi, hi->block[i0].noise_compand_setting, 0, setup->psy_noise_compand, setup->psy_noise_compand_short_mapping );
	vorbis_encode_compand_setup( vi, hi->block[1].noise_compand_setting, 1, setup->psy_noise_compand, setup->psy_noise_compand_short_mapping );
	if( !singleblock )
	{
		vorbis_encode_compand_setup( vi,hi->block[2].noise_compand_setting, 2, setup->psy_noise_compand, setup->psy_noise_compand_long_mapping );
		vorbis_encode_compand_setup( vi,hi->block[3].noise_compand_setting, 3, setup->psy_noise_compand, setup->psy_noise_compand_long_mapping );
	}
	...
	if( !singleblock )
	{
		...
		vorbis_encode_peak_setup( vi, hi->block[3].tone_peaklimit_setting, 3, setup->psy_tone_dBsuppress );
	}

	/* noise bias setup */
	vorbis_encode_noisebias_setup( vi, hi->block[i0].noise_bias_setting, 0, setup->psy_noise_dBsuppress, setup->psy_noise_bias_impulse, setup->psy_noiseguards, ( i0 == 0 ? hi->impulse_noisetune : 0.0 ) );
	vorbis_encode_noisebias_setup( vi, hi->block[1].noise_bias_setting, 1, setup->psy_noise_dBsuppress, setup->psy_noise_bias_padding, setup->psy_noiseguards, 0.0 );
	if( !singleblock )
	{
		vorbis_encode_noisebias_setup( vi, hi->block[2].noise_bias_setting, 2, setup->psy_noise_dBsuppress, setup->psy_noise_bias_trans, setup->psy_noiseguards, 0.0 );
		vorbis_encode_noisebias_setup( vi, hi->block[3].noise_bias_setting, 3, setup->psy_noise_dBsuppress, setup->psy_noise_bias_long, setup->psy_noiseguards, 0.0 );
	}
...
static int vorbis_encode_setup_setting( vorbis_info* vi, long channels, long rate )
{
	int ret = 0, i, is;
	codec_setup_info* ci = vi->codec_setup;
	highlevel_encode_setup* hi = &ci->hi;
	...
	hi->lowpass_kHz = setup->psy_lowpass[is] * ( 1.0 - ds ) + setup->psy_lowpass[is + 1] * ds;  

	hi->ath_floating_dB = setup->psy_ath_float[is] * ( 1.0 - ds ) + setup->psy_ath_float[is + 1] * ds;
	hi->ath_absolute_dB = setup->psy_ath_abs[is] * ( 1.0 - ds ) + setup->psy_ath_abs[is + 1] * ds;

	hi->amplitude_track_dBpersec = -6.0;
...
int vorbis_encode_setup_managed( vorbis_info* vi, long channels, long rate, long max_bitrate, long nominal_bitrate, long min_bitrate )
{
	...
	if( nominal_bitrate <= 0.0 )
	{
		if( max_bitrate > 0. )
		{
			...
			{
				nominal_bitrate = ( max_bitrate * 7 ) >> 3;
			}
		}
		else
		{
...
{
	int ret = vorbis_encode_setup_managed( vi, channels, rate, max_bitrate, nominal_bitrate, min_bitrate );
	if( ret )
	{
		vorbis_info_clear( vi );
		return( ret );
	...
	if( vi )
	{
		...
		switch( number )
		{
			...
			case OV_ECTL_LOWPASS_SET:
			{
				...
				if( hi->lowpass_kHz < 2.0 )
				{
					hi->lowpass_kHz = 2.0;
				}
				if( hi->lowpass_kHz > 99.0 )
				{
					hi->lowpass_kHz = 99.0;
				}
			}
			...
			case OV_ECTL_IBLOCK_SET:
			{
				...
				if( hi->impulse_noisetune > 0.0 )
				{
					hi->impulse_noisetune = 0.0;
				}
				if( hi->impulse_noisetune < -15.0 )
				{
					hi->impulse_noisetune = -15.0;
				}
			}
			return( 0 );      
		}
