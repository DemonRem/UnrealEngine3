...
/* 2.0.5: provides an extensible way to pass additional parameters.
  ...
  typedef struct
  {
    ...
    char *xshow;	/* if (flags & gdFTEX_XSHOW)
			   ...
			   NB. The caller is responsible for free'ing
			   the xshow string after use and before the next call.
			 */
    char *fontpath;	/* the resolved fontpath 

			   NB good until freed from fontcache
			   i.e. the caller is expected to use/copy immediately
...
#define gdFTEX_FONTPATHNAME 32
	/* if HAVE_LIBFONTCONFIG & ! gdFTEX_FONTPATHNAME
