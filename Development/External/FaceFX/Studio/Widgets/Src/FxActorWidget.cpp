	...
	if( _actor && _animGroupDropdown && _animDropdown )
	{
		...
		if( animGroupIndex != FxInvalidIndex )
		{
			...
			if( selAnimString != _("<No animation selected>") )
			{
				...
				{
					const FxInt32 noAnimSelStringIndex = _animDropdown->FindString(_("<No animation selected>"));
					if ( noAnimSelStringIndex != wxNOT_FOUND )
					{
						_animDropdown->Delete(noAnimSelStringIndex);
					}
				}
