...
class   GWaitable : public GRefCountBase<GWaitable, GStat_Default_Mem>
{
public:
...
protected:
    ...
    // it is null.
    struct HandlerArray : public GNewOverrideBase<GStat_Default_Mem>
    {
        typedef GArrayConstPolicy<0, 16, true> SizePolicyType; // MinCapacity=0, Granularity=16, NeverShrink
        typedef GArray<HandlerStruct, GStat_Default_Mem, SizePolicyType> HandlerArrayType;
...
public:
    ...
    GEXPORT          GThread(UPInt stackSize = 128 * 1024, int processor = -1);
    ...
    explicit GEXPORT GThread(const CreateParams& params);
    ...
    GEXPORT static  void GCDECL FinishAllThreads();
    ...
    GINLINE OSThread *       GetOSHandle() const { return ThreadData ? const_cast<OSThread*>(&ThreadData->ThreadHandle) : 0; }
#elif defined(GFC_OS_PS3)
    GINLINE sys_ppu_thread_t GetOSHandle() const { return ThreadHandle; }
#elif defined(GFC_OS_PSP)
    GINLINE SceUID           GetOSHandle() const { return ThreadHandle; }
#else
    ...
    // Sleep secs seconds
    GEXPORT static bool    Sleep(UInt secs);
    // Sleep msecs milliseconds
    GEXPORT static bool    MSleep(UInt msecs);
...
private:
    ...
    friend class GThreadList;

#elif defined(GFC_OS_PS3)
    friend void GThread_PpuThreadStartFn(uint64_t phandle);

#elif defined(GFC_OS_PSP)
    friend int GThread_PspThreadStartFn(SceSize size, void* phandle);
...
public: // for Wii compiler 4.3 145
    struct ThreadDataWii : public GListNode<ThreadDataWii>
    {
