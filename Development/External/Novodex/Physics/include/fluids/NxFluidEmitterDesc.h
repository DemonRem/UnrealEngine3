...
enum NxFluidEmitterFlag
	...
	*/
	...
	the actor that the emitter is attached to.
	*/
	NX_FEF_FORCE_ON_ACTOR		= (1<<2),
	
	/**
	\brief If set, the velocity of the emitter is added to the emitted particle velocity.
...
class NxFluidEmitterDesc
	...
	/**
	\brief The emitter's pose relative to the frameActor.
	
	relPose is relative to the actors body frame for dynamic actors or relative to the shape frame for static actors.

	If frameActor is NULL then relPose is relative to the origin.
	...
	*/
	...
	/**
	\brief A pointer to the NxActor to which the emitter is attached to.
	
	If this pointer is set to NULL, the emitter is attached to the world frame. The actor 
	...
	*/
	...
	Note that the maximal particle velocity is limited by NxFluid::motionLimitMultiplier.
