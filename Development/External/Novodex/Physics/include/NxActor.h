...
\li #NX_VISUALIZE_BODY_LIN_MOMENTUM
\li #NX_VISUALIZE_BODY_ANG_MOMENTUM
\li #NX_VISUALIZE_BODY_LIN_ACCEL
\li #NX_VISUALIZE_BODY_ANG_ACCEL
\li #NX_VISUALIZE_BODY_LIN_FORCE
\li #NX_VISUALIZE_BODY_ANG_FORCE
\li #NX_VISUALIZE_BODY_REDUCED
\li #NX_VISUALIZE_BODY_JOINT_GROUPS
\li #NX_VISUALIZE_BODY_CONTACT_LIST
\li #NX_VISUALIZE_BODY_JOINT_LIST
\li #NX_VISUALIZE_BODY_DAMPING
\li #NX_VISUALIZE_BODY_SLEEP
...
//@{
	...
	*/
	...
	If you want to compute the mass properties based on a total mass specify a total mass and set 
	density to zero. Alternatively specify a density and set totalMass to zero.

	If a density is supplied the mass is calculated based on the total mass of the shapes multiplied 
	by the supplied density.

	The mass of each shape is either the shape's local density (default 1.0) multiplied by the shape's 
	volume or a directly specified shape mass. 
	...
	\param[in] density Density of the shapes belonging to the actor(or zero). <b>Range:</b> (0,inf)
...
//@{
	...
	@see isGroupSleeping() isSleeping() getSleepLinearVelocity() getSleepAngularVelocity() wakeUp() putToSleep()
	*/
	...
	<b>Platform:</b>
	\li PC SW: Yes
	\li PPU  : Yes (Sleep behavior on hardware differs)
	\li PS3  : Yes
	\li XB360: Yes

	@see isGroupSleeping() isSleeping() getSleepLinearVelocity() getSleepAngularVelocity() wakeUp() putToSleep() NxBodyDesc.sleepEnergyThreshold
	*/
	...
	@see isGroupSleeping() isSleeping() getSleepLinearVelocity() getSleepAngularVelocity() wakeUp() putToSleep() setSleepLinearVelocity()
	*/
