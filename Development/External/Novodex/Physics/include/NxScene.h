...
enum NxThreadWait
	...
	*/
	...
	/**
	\brief Wait until NxScene.shutdownThreads() is called.

	@see NxScene.shutdownThreads
	*/
	NX_WAIT_SHUTDOWN			= 2,
...
enum NxSweepFlags
	...
	NX_SF_ASYNC			= (1<<2),	//!< Asynchronous sweeps (else synchronous)
...
//@{
	...
	*/
	...
	Note: releasing an actor will affect any joints that are connected to the actor.
...
//@{
	...
	*/
	...
	/**
	\brief Similar to getPairFlags(), but for a pair of shapes.
...
//@{
	...
	*/
	...
	/**
	\brief Deletes the specified fluid. 
	
	The fluid must be in this scene.
	Do not keep a reference to the deleted instance.
	Avoid release calls while the scene is simulating (in between simulate() and fetchResults() calls).
...
/************************************************************************************************/
	...
	*/
	...
	#resetSimulation() method is called.
	...
	@see resetSimulation() NxThreadWait NxThreadPollResult
	*/
	...
	*/
	...
	Note: This also applies to SDK managed threads. So calling shutdownThreads will release 
	all threads which are waiting using NX_WAIT_SHUTDOWN, which includes internal SDK threads.

	When internal threads are released by shutdownThreads() they will exit.
