...
/**
	...
	be specifyied for the static actor when it is created.
...
class NxActorDescBase
	...
	*/
	...
	/**
	\brief Denisty used during mass/inertia computation.
	...
	*/
	...
	/**
	\brief The managed scene index

	When hardware is present, an actor can be optionally be created in a managed hardware scene (these are created via NxSceneDesc::numManagedHwRBScenes).
	If the actor cannot be created in the requested scene, creation fails.  0 means to create in the software scene.
