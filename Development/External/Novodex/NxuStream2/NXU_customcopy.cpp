...
//** Questions:
//**
//**              Are there any sample apps that show how to use 'NxCompartments'?
//**							Are there any sample apps that use material indices for triangle meshes?
//**						  Are there any sample apps that use varying strides?
//**
//*************************************************************
//*************************************************************
...
CUSTOM: mId                            of type const char *           DONE
CUSTOM: mUserProperties                of type const char *           DONE
CUSTOM: materialIndices                of type NxArray< NxU32 >       **NOT DONE
...
void CustomCopy::customCopyTo(::NxParticleData &cdesc,const NxParticleData &desc)
{
	if ( desc.mBufferPos.size() )
	{
		cdesc.numParticlesPtr = &cdesc.maxParticles;
		cdesc.maxParticles    = desc.mBufferPos.size();
 		cdesc.bufferPos       = (NxF32 *)&desc.mBufferPos[0].x;
 		cdesc.bufferPosByteStride = sizeof(NxVec3);

 		if ( desc.mBufferVel.size() == cdesc.maxParticles )
 		{
    ...
    if ( desc.mBufferLife.size() == cdesc.maxParticles )
    {
    ...
    if ( desc.mBufferDensity.size() == cdesc.maxParticles )
    {
...
#if NX_SDK_VERSION_NUMBER >= 260
    if ( desc.mBufferId.size() == cdesc.maxParticles )
    {
    ...
    if ( desc.mBufferFlag.size() == cdesc.maxParticles )
    {
