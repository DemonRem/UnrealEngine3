...
public:
	...
	*/
	...
	\param effectr  A pointer to the effector that was created.
	\param userProperties An optional ASCIIZ string that contains arbitrary 'user properties' defined in the source data.  This pointer is persistent even after the NxuPhysicsCollection has been released.
	*/
...
*/
bool 										instantiateCollection(NxuPhysicsCollection	*c,	NxPhysicsSDK &sdk, NxScene *scene, NxMat34 *rootNode,	NXU_userNotify *un);
...
*/
bool 										coreDump(NxPhysicsSDK *sdk, const char	*fname,NXU_FileType type,	bool save_defaults,bool cook);
...
*/
NxuPhysicsCollection *	extractCollectionSDK(NxPhysicsSDK	*sdk,const char *collectionId=0);
...
/**
\brief  Assigns a specific ID string to a colleciton.
...
\return true if successful.
*/
bool 										addEntireScene(NxuPhysicsCollection &c,    	NxScene &s,const char *userProperties=0,const char *sceneId=0);
...
\param c The NxuPhysicsCollection to add data to.
\param sceeneId   The id of the instance created.
\param sceneName  The name of the scene to be instantiated.
\param rootNode   The root node transform to instantiate the scene relative to.
\param ignorePlane Whether or not to ignore ground planes upon instantiation.  True by default.
\return true if successful.
...
/**
\brief Sets a global behavior for the instantiator class whether or not to attempt to load cloth in its actively simulated state.  Default is false.
