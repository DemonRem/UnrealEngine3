...
bool instantiateCollection(NxuPhysicsCollection	*c,	NxPhysicsSDK &sdk, NxScene *scene, NxMat34 *rootNode,	NXU_userNotify *ur)
{
	bool ret = false;
...
bool coreDump(NxPhysicsSDK *sdk, const char	*fname,	NXU_FileType type, bool	save_default,	bool cook)
{
	bool ret = false;

	NxuPhysicsCollection *c	=	extractCollectionSDK(sdk);
	if (c)
	{
...
NxuPhysicsCollection *extractCollectionSDK(NxPhysicsSDK	*sdk,const char *collectionId)// extract the contents	of the SDK into	this collection.
{
...
bool addEntireScene(NxuPhysicsCollection &c,                  NxScene &scene,const char *userProperties,const char *sceneId)
{
	bool ret = false;
...
#if NX_SDK_VERSION_NUMBER >= 260
/**
\brief Adds a compartment to the physics colleciton

\param c The NxuPhysicsCollection to add data to.
\param cmp A reference to the compartment to be added tot the current scene in this collection.
  					...
  					switch ( shape->mType )
  					{
  						...
  						case SC_NxBoxShapeDesc:
  							if ( 1 )
  							{
  								...
  								if ( !found )
  								{
    								createBox(b->dimensions,0,g,shrink,maxv);
  						...
  						case SC_NxSphereShapeDesc:
  							if ( 1 )
  							{
  								...
  								if ( !found )
  								{
    								createSphere(b->radius,16,0,g,shrink,maxv);
    								radius = b->radius;
    							}
  							}
  							break;
  						...
  						case SC_NxCapsuleShapeDesc:
  							if ( 1 )
  							{
  								...
  								if ( !found )
  								{
    								createCapsule(b->radius,b->height,1,16,0,g,shrink,maxv);
    								radius = b->radius;
    								height = b->height;
    							}
  							}
  							...
  							if ( 1 )
  							{
  								NxConvexShapeDesc *cv = (NxConvexShapeDesc *) shape;
 	  							...
 	  							if ( cmesh && cmesh->mPoints.size() )
 		  						{
										...
										{
 			  						  createHull( cmesh->mPoints.size(), &cmesh->mPoints[0].x, 0, g, shrink, maxv);
										}
  				  			}
  							}
  							break;
 					...
 					switch ( shape->getType() )
 					{
 						...
 						case NX_SHAPE_CONVEX:
 							if ( 1 )
 							{
  	  						...
  	  						{
		  						  createHull( temp.mPoints.size(), &temp.mPoints[0].x, 0, g, shrink, maxv);
  								}
 				  			}
 							}
 							break;
