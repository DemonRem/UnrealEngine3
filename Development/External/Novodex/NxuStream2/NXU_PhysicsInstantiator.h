...
public:
		 ...
		 */
		void instantiate(NxPhysicsSDK	&sdk,	NXU_userNotify *notify = 0,	NxScene	*defaultScene	=	0, NxMat34 *defaultSceneOffset = 0);

		NxScene *instantiateScene(const	char *sceneName,	// null	means	the	'root' scene.
		NX_BOOL	ignore_plane,	 //	don't	instantiate	'planes'
		NxPhysicsSDK &sdk, NXU_userNotify	*notify	=	0, NxScene *defaultScene = 0,	NxMat34	*defaultSceneOffset	=	0);
...
private:
    ...
    void instanceCompartments(NxPhysicsSDK &sdk,NxSceneDesc &sd);
#endif
