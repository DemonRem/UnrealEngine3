...
bool createBox(NxVec3	&dimensions, const NxMat34 *localPose, NxuGeometry &g,float shrink,unsigned int maxV)
{
	...
	indices	=	addTri(0,	1, 2,	indices);
	indices	=	addTri(0,	2, 3,	indices);
	indices	=	addTri(4,	5, 6,	indices);
	indices	=	addTri(4,	6, 7,	indices);
	indices	=	addTri(0,	3, 4,	indices);
	indices	=	addTri(4,	3, 7,	indices);
	indices	=	addTri(7,	3, 2,	indices);
	indices	=	addTri(7,	2, 6,	indices);
	indices	=	addTri(0,	1, 5,	indices);
	indices	=	addTri(0,	5, 4,	indices);
	indices	=	addTri(1,	2, 5,	indices);
	indices	=	addTri(6,	5, 2,	indices);
...
bool createSphere(float	r, int stepsize, const NxMat34 *localPose, NxuGeometry &g,NxReal shrink,unsigned int maxV)
{
	...
	bool ret = createHull(vcount,	points,	localPose,	g, shrink, maxV);
...
bool createCapsule(float radius, float height, int axis, int stepsize, const NxMat34 *localPose, NxuGeometry &g,NxReal shrink,unsigned int maxV)
{
	unsigned int vcount;
...
bool createHull(unsigned int vcount, const float *points,	const NxMat34 *localPose, NxuGeometry &g,NxReal shrink,unsigned int maxv)
{
	bool ret = false;
